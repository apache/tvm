...
class FrontendLegalizer : public StmtExprMutator {
 public:
  FrontendLegalizer() = default;
 ...
 private:
  ...
  PrimExpr VisitExpr_(const VarNode* node) final {
    if (let_bindings_.count(node)) {
      return StmtExprMutator::VisitExpr(let_bindings_[node]);
    } else {
  ...
  Stmt VisitStmt_(const LetStmtNode* node) final {
    if (parallel_for_scope_ > 0) {
      ...
      return StmtExprMutator::VisitStmt(node->body);
    }
  ...
  PrimExpr VisitExpr_(const LetNode* node) final {
    if (parallel_for_scope_ > 0) {
      let_bindings_[node->var.get()] = node->value;
      return StmtExprMutator::VisitExpr(node->body);
    }
    return StmtExprMutator::VisitExpr_(node);
  }
...
Pass FrontendLegalize() {
  auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
    ...
    fptr->body = FrontendLegalizer()(f->body);
    return f;
  };
