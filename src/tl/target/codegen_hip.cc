...
CodeGenTLHip::CodeGenTLHip() { restrict_keyword_ = "__restrict__"; }

void CodeGenTLHip::PrintFuncPrefix(std::ostream& os) { os << "extern \"C\" __global__ "; }
...
void CodeGenTLHip::PrintExtraAttrs(const PrimFunc& f, std::ostream& os) {
...
std::string CodeGenTLHip::Finish() {
...
void CodeGenTLHip::VisitStmt_(const tir::ForNode* op) {
...
void CodeGenTLHip::BindThreadIndex(const IterVar& iv) {
  ICHECK(!var_idmap_.count(iv->var.get()));
  var_idmap_[iv->var.get()] = CastFromTo(iv->thread_tag, DataType::UInt(32), iv->var.dtype());
}

void CodeGenTLHip::PrintType(DataType t, std::ostream& os) {  // NOLINT(*)
  int lanes = t.lanes();
  if (t.is_handle()) {
  ...
  if (t == tl::cuTensorMapType()) {
    // TODO(lei): HIP doesn't support cuTensorMap, 
    // need to find a way to replace it.
    os << "CUtensorMap";
    return;
  }
...
void CodeGenTLHip::PrintVecBinaryOp(const std::string& op, DataType t, PrimExpr lhs, PrimExpr rhs,
...
void CodeGenTLHip::PrintVecElemLoad(const std::string& vec, DataType t, int i,
                                 std::ostream& os) {  // NOLINT(*)
  if (t.is_scalar()) {
    os << vec;
    return;
...
void CodeGenTLHip::PrintVecElemStore(const std::string& vec, DataType t, int i,
...
void CodeGenTLHip::PrintStorageSync(const CallNode* op) {
...
void CodeGenTLHip::PrintStorageScope(const std::string& scope, std::ostream& os) {  // NOLINT(*)
...
std::string CodeGenTLHip::CastFromTo(std::string value, DataType from, DataType target) {
...
void CodeGenTLHip::VisitExpr_(const CastNode* op, std::ostream& os) {
...
void CodeGenTLHip::PrintCallExtern(Type ret_type, String global_symbol, const Array<PrimExpr>& args,
...
std::string CodeGenTLHip::GetBufferRef(DataType t, const BufferNode* buffer, PrimExpr index) {
...
void CodeGenTLHip::VisitExpr_(const CallNode* op, std::ostream& os) {
  ...
  if (op->op.same_as(builtin::ptx_cp_async())) {
    ...
    os << "rocwmma::wmma::load_matrix_sync(";
    this->PrintExpr(op->args[0], os);
    os << "[";
    this->PrintExpr(op->args[4], os);
    os << "], ";
    ...
    need_wmma_h_ = true;
    ICHECK_EQ(op->args.size(), 8U);
    os << "rocwmma::wmma::store_matrix_sync(";
    this->PrintExpr(op->args[5], os);
    os << ", ";
    this->PrintExpr(op->args[0], os);
    os << "[";
    ...
    if (const StringImmNode* str = op->args[7].as<StringImmNode>()) {
      os << ", rocwmma::wmma::mem_" << str->value;
    } else {
...
void CodeGenTLHip::VisitStmt_(const AttrStmtNode* op) {
...
void CodeGenTLHip::VisitStmt_(const AllocateNode* op) {
...
void CodeGenTLHip::VisitExpr_(const RampNode* op, std::ostream& os) {
...
void CodeGenTLHip::VisitExpr_(const BroadcastNode* op, std::ostream& os) {  // NOLINT(*)
  int lanes = static_cast<int>(Downcast<IntImm>(op->lanes)->value);
...
inline void PrintConst(const FloatImmNode* op, std::ostream& os, CodeGenTLHip* p) {  // NOLINT(*)
...
void CodeGenTLHip::VisitExpr_(const FloatImmNode* op, std::ostream& os) {  // NOLINT(*)
  PrintConst(op, os, this);
}

void CodeGenTLHip::HandleVolatileLoads(const std::string& value, const BufferLoadNode* op,
                                    std::ostream& os) {
...
void CodeGenTLHip::PrintVecElemLoadExpr(DataType t, int i, const std::string& value,
                                     std::ostream& os) {
  ICHECK_GT(t.lanes(), 1);
  if (t.bits() == 8 && (t.is_int() || t.is_uint())) {
    if (!(t.lanes() == 2 || t.lanes() == 3)) {
...
void CodeGenTLHip::AddFunction(const PrimFunc& f) {
