...
__device__ dim3 rasterization2DRow() {
  ...
  const int stride = panel_idx + 1 < total_panel ? panel_width : (grid_size - panel_idx * panel_size) / gridDim.x;
  const int col_idx = (panel_idx & 1) ? gridDim.x - 1 - panel_offset / stride : panel_offset / stride;
...
__device__ dim3 rasterization2DColumn() {
  ...
  const int panel_idx = block_idx / panel_size;
  const int total_panel = cutlass::ceil_div(grid_size, panel_size);
  const int stride = panel_idx + 1 < total_panel ? panel_width : (grid_size - panel_idx * panel_size) / gridDim.y;
  const int row_idx = (panel_idx & 1) ? gridDim.y - 1 - panel_offset / stride : panel_offset / stride;
