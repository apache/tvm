...
struct MatrixCoreIntrinGroup {
  ...
  /*! \brief Create MatrixCoreIntrinGroup from config in a map. The map should contains the
   * following keys:
   *  - init
   *  - load_a
   *  - load_b
   ...
   */
  static MatrixCoreIntrinGroup FromConfig(const Map<String, String>& config);
};

MatrixCoreIntrinGroup MatrixCoreIntrinGroup::FromConfig(const Map<String, String>& config) {
  ...
  MatrixCoreIntrinGroup intrin_group;
...
class MatrixCoreStateNode : public StateNode {
 public:
  ...
  static constexpr const char* _type_key = "meta_schedule.MatrixCoreState";
  TVM_DECLARE_FINAL_OBJECT_INFO(MatrixCoreStateNode, StateNode);
};

class MatrixCoreState : public State {
 public:
  explicit MatrixCoreState(MatrixCoreIntrinGroup intrin_group,
                           tir::AutoTensorizeMappingInfo mapping_info, Schedule sch,
                           BlockRV block_rv, Array<Array<tir::LoopRV>> tiles = {});

  TVM_DEFINE_MUTABLE_OBJECT_REF_METHODS(MatrixCoreState, State, MatrixCoreStateNode);
};

TVM_REGISTER_OBJECT_TYPE(MatrixCoreStateNode);

MatrixCoreState::MatrixCoreState(MatrixCoreIntrinGroup intrin_group,
                                 tir::AutoTensorizeMappingInfo mapping_info, Schedule sch,
                                 BlockRV block_rv, Array<Array<LoopRV>> tiles) {
  ObjectPtr<MatrixCoreStateNode> node = make_object<MatrixCoreStateNode>();
...
State MatrixCoreStateNode::Copy() const {
  ObjectPtr<MatrixCoreStateNode> node = make_object<MatrixCoreStateNode>(*this);
  node->sch = sch->Copy();
  return State(node);
}
 ...
 * intrinsics.
 */
class MultiLevelTilingMatrixCoreNode : public MultiLevelTilingNode {
 private:
  // SubRule: Add tensorization-related transformations
  inline std::vector<State> TransformForTensorization(MatrixCoreState state) const;
  // Subrule: Transform the layout of the output. This is necessary for efficient cache write the
  // output in the shared memory.
  std::vector<State> TransformIntermediateOutputLayout(MatrixCoreState state);
  // Subrule: Add tensorized load
  inline std::vector<State> AddReadReuseMatrixCore(MatrixCoreState state) const;
  // Subrule: Add tensorized store
  inline std::vector<State> AddWriteReuseMatrixCore(MatrixCoreState state) const;
  // Subrule: Add software pipeline
  inline std::vector<State> AddSoftwarePipeline(MatrixCoreState state) const;
  ...
  ScheduleRule Clone() const final {
    ObjectPtr<MultiLevelTilingMatrixCoreNode> n =
        make_object<MultiLevelTilingMatrixCoreNode>(*this);
   ...
   */
  Optional<LoopRV> TransformWithTensorIntrin(MatrixCoreStateNode* state,
 ...
 public:
  ...
  std::vector<MatrixCoreIntrinGroup> intrin_groups;
  /*! \brief Whether to use software pipeline */
  bool use_software_pipeline = false;
  static constexpr const char* _type_key = "meta_schedule.MultiLevelTilingMatrixCore";
  TVM_DECLARE_FINAL_OBJECT_INFO(MultiLevelTilingMatrixCoreNode, MultiLevelTilingNode);

 private:
};

// Entry of the mega rule; Inherited from ScheduleRuleNode
Array<Schedule> MultiLevelTilingMatrixCoreNode::Apply(const Schedule& sch,
  ...
  for (const auto& kv : intrin_group_to_mapping_info) {
    const MatrixCoreIntrinGroup& intrin_group = intrin_groups[kv.first];
    ...
    initial_states.push_back(MatrixCoreState(intrin_group, mapping_info, new_sch, block_rv));
  }
  Array<Schedule> results;
  for (auto&& state : ApplySubRules(initial_states)) {
    TVM_PY_LOG(INFO, logger) << "Sketch " << results.size() << ": tensorizing with "
                             << state.as<MatrixCoreStateNode>()->intrin_group.compute_intrin;
...
std::vector<State> MultiLevelTilingMatrixCoreNode::ApplySubRules(std::vector<State> states) {
  states = SubRule(std::move(states), [&](State state) {
    return TransformForTensorization(Downcast<MatrixCoreState>(state));
  });
  states = SubRule(std::move(states), [&](State state) { return TileLoopNest(state); });
  states = SubRule(std::move(states), [&](State state) {
    return TransformIntermediateOutputLayout(Downcast<MatrixCoreState>(state));
  });
  states = SubRule(std::move(states), [&](State state) { return AddWriteReuse(state); });
  states = SubRule(std::move(states), [&](State state) {
    return AddWriteReuseMatrixCore(Downcast<MatrixCoreState>(state));
  });
...
void MultiLevelTilingMatrixCoreNode::TileAndAnnotateTensorize(Schedule* sch,
                                                              const BlockRV& block_rv,
                                                              const String& intrin_name) const {
...
std::vector<State> MultiLevelTilingMatrixCoreNode::TransformIntermediateOutputLayout(
    MatrixCoreState state) {
...
std::vector<State> MultiLevelTilingMatrixCoreNode::AddWriteReuseMatrixCore(
    MatrixCoreState state) const {
...
std::vector<State> MultiLevelTilingMatrixCoreNode::AddReadReuseMatrixCore(
    MatrixCoreState state) const {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (dtype.is_float16()) {
      ...
      sch->StorageAlign(cache_read, 0, -2, 64, 16);
    } else {
...
std::vector<State> MultiLevelTilingMatrixCoreNode::AddSoftwarePipeline(
    MatrixCoreState state) const {
...
Optional<LoopRV> MultiLevelTilingMatrixCoreNode::TransformWithTensorIntrin(
    MatrixCoreStateNode* state, const String& intrin_name) const {
...
inline std::vector<State> MultiLevelTilingMatrixCoreNode::TransformForTensorization(
    MatrixCoreState state) const {
...
ScheduleRule ScheduleRule::MultiLevelTilingMatrixCore(
  ...
  auto node = MultiLevelTilingInitCommon<MultiLevelTilingMatrixCoreNode>(
  ...
  CHECK(node->reuse_write_.req == ReuseType::kMustReuse &&
      ...
      << "ValueError: Shared memory write reuse must be enabled for MultiLevelTilingMatrixCore.";

  node->intrin_groups.reserve(intrin_groups.size());
  for (const auto& intrin_group_config : intrin_groups) {
    node->intrin_groups.emplace_back(MatrixCoreIntrinGroup::FromConfig(intrin_group_config));
  }
  node->use_software_pipeline = use_software_pipeline;
  return ScheduleRule(node);
}

TVM_REGISTER_NODE_TYPE(MultiLevelTilingMatrixCoreNode);
TVM_REGISTER_GLOBAL("meta_schedule.ScheduleRuleMultiLevelTilingMatrixCore")
    .set_body_typed(ScheduleRule::MultiLevelTilingMatrixCore);
