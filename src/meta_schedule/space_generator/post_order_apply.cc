...
class PostOrderApplyNode : public SpaceGeneratorNode {
 public:
  ...
  Array<tir::Schedule> GenerateDesignSpace(const IRModule& mod) final {
    ...
    // std::cout << "GenerateDesignSpace sch_rules size:" << sch_rules.value().size() << std::endl;
    for (ScheduleRule sch_rule : sch_rules.value()) {
      ...
      while (!stack.empty()) {
        ...
        // std::cout << "sch_rule.Apply" << std::endl << std::flush;
    ...
    // std::cout << "GenerateDesignSpace result size:" << result.size() << std::endl;
    return result;
  }
