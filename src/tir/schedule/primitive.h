 ...
 * block so that the buffer region generated by the producer block could cover those regions read
 * by the consumers. It requires:
 * 1) The scope block has stage-pipeline property
 * 2) The given block's subtree of the scope block satisfies compact dataflow condition.
 * i.e. all the blocks in the scope's subtree must be either complete block or reduction block
 * 3) `block` and `loop` are under the same scope, `loop` is not the ancestor of `block`
 * 4) The block is not an output block,
 * i.e. the buffer regions written by the block are allocated under the current scope
 ...
 * 4) The block is not an output block,
 * i.e. the buffer regions written by the block are allocated under the current scope
 * 5) All the producers of the block are under the given loop
 * \param self The schedule state
 * \param block_sref The block to be moved
 * \param loop_sref The loop where the block to be moved to
 * \param preserve_unit_loops Whether to keep the trivial loops whose extents are 1
