...
void ArgBinder::BindDLTensor(const Buffer& buffer, const PrimExpr& device_type,
  ...
  if (Bind_(buffer->data, TVMArrayGet(DataType::Handle(), handle, builtin::kArrData),
    ...
    PrimExpr is_aligned = tvm::floormod(tvm::reinterpret(DataType::UInt(vptr->dtype.bits()), vptr),
                                        buffer->data_alignment) == 0;

    std::ostringstream alignment_err_msg;
    alignment_err_msg << arg_name << ".data is expected to be aligned to " << buffer->data_alignment
                      << " bytes";

    // Mark alignment of external bufs.
    asserts_.emplace_back(AssertStmt(!interpretable_as_pointer || is_aligned,
                                     StringImm(alignment_err_msg.str()), nop));
