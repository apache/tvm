...
class BuiltinLower : public StmtExprMutator {
 public:
  ...
  PrimExpr MakeArray(const CallNode* op) {
    ...
    PrimExpr expr = StmtExprMutator::VisitExpr_(op);
    op = expr.as<CallNode>();
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrData, op->args[0]));
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrShape, op->args[1]));
    PrimExpr strides = op->args[2];
    if (!strides.defined() || is_zero(strides)) {
      strides = make_zero(DataType::Handle());
    }
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrStrides, strides));
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrNDim, op->args[3]));
    DataType dtype = op->args[4].dtype();
    prep_seq_.emplace_back(
        TVMStructSet(scope.stack_array_, idx, builtin::kArrTypeCode,
                     make_const(DataType::UInt(8), static_cast<int>(dtype.code()))));
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrTypeBits,
                                        make_const(DataType::UInt(8), dtype.bits())));
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrTypeLanes,
    ...
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrByteOffset,
                                        cast(DataType::UInt(64), byte_offset)));
    ICHECK(device_type_.defined()) << "Unknown device type in current IR";
    ICHECK(device_id_.defined()) << "Unknown device id in current IR";
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrDeviceId,
                                        cast(DataType::Int(32), device_id_)));
    prep_seq_.emplace_back(TVMStructSet(scope.stack_array_, idx, builtin::kArrDeviceType,
                                        cast(DataType::Int(32), device_type_)));
    return TVMStructGet(DataType::Handle(), scope.stack_array_, idx, builtin::kArrAddr);
  }
  // call packed.
  PrimExpr MakeCallPacked(const CallNode* op) {
    ...
    scope.run_arg_stack_ += op->args.size();
  ...
  PrimExpr MakeCallTracePacked(const CallNode* op) {
    ...
    scope.run_arg_stack_ = arg_stack_begin + args_size - 1;
    Array<PrimExpr> packed_args = {op->args[0], scope.stack_value_, scope.stack_tcode_,
                                   ConstInt32(arg_stack_begin),
                                   ConstInt32(arg_stack_begin + op->args.size() - 1),
                                   // Pass traced value.
