...
public: 
    ...
    Stmt VisitStmt_(const AllocateNode* allocate) final{
      ...
      if (!has_buffer_1){
        ...
        predicate_buffer = decl_buffer({IntImm(DataType::Int(32), 1)}, DataType::Bool(1), "predicate", "local");
      }
    ...
    Stmt VisitStmt_(const BufferStoreNode* store) final{
      ...
      if (call != nullptr){
        ...
        if (op != nullptr && op->name == "tir.if_then_else"){
          ...
          new_lhs = BufferLoad(load->buffer, {BufferLoad(addr_buffer, {IntImm(DataType::Int(32), 0)})});
