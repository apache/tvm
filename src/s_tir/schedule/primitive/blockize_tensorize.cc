...
void Tensorize(ScheduleState self, const StmtSRef& sref, const TensorIntrin& intrin,
  ...
  for (int i = 0, n = intrin_desc->params.size(); i < n; ++i) {
    const BufferVar& desc = tirx::BufferParamMap(intrin_desc->params)[intrin_desc->params[i]];
    const BufferVar& impl = tirx::BufferParamMap(intrin_impl->params)[intrin_impl->params[i]];
    impl2desc[impl] = desc;
  }
  ...
  for (int i = 0, n = intrin_impl->params.size(); i < n; ++i) {
    const BufferVar& impl = tirx::BufferParamMap(intrin_impl->params).at(intrin_impl->params[i]);
