...
inline void PythonAPICall::AsPythonString(const Any& obj, std::ostream& os) {
  if (obj == nullptr) {
    ...
    os << *opt_str;
  } else if (const auto opt_int_imm = obj.try_cast<prim::IntImm>()) {
    os << (*opt_int_imm)->value;
  } else if (const auto opt_float_imm = obj.try_cast<prim::FloatImm>()) {
    os.precision(17);
...
#endif  // TVM_S_TIR_SCHEDULE_INSTRUCTION_TRAITS_H_
