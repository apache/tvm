...
ffi::String GetRuleKindFromTarget(const Target& target) {
  if (target->kind->name == "llvm") {
    ...
    if ((target_json.at("feature.has_dotprod")).as_or_throw<IntImm>()->value) {
      return "dotprod";
    }
    if ((target_json.at("feature.has_asimd")).as_or_throw<IntImm>()->value) {
      return "asimd";
    }
    return "llvm";
  }
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef()
      .def_method("s_tir.meta_schedule.SpaceGeneratorInitializeWithTuneContext",
