...
class PipelineInjector : private StmtExprMutator {
 ...
 private:
  ...
  Stmt VisitStmt_(const ForNode* op) final {
    ...
    auto pipeline_stages = (op->annotations.at(s_tir::attr::software_pipeline_stage))
                               .as_or_throw<ffi::Array<int64_t>>();
    auto pipeline_orders = (op->annotations.at(s_tir::attr::software_pipeline_order))
                               .as_or_throw<ffi::Array<int64_t>>();
  ...
  Stmt VisitStmt_(const SBlockNode* op) final {
    ...
    if (it != op->annotations.end()) {
      int buffer_index = static_cast<int>(((*it).second).as_or_throw<IntImm>()->value);
