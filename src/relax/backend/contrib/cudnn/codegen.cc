...
class cuDNNJSONSerializer : public JSONSerializer {
 public:
  ...
  NodeEntries HandleAttention(const CallNode* call_node, const Function& fn,
    ...
    auto q_shape = ((root_call->args[0]->ty).as_or_throw<TensorType>()->shape.value())
                       .as_or_throw<ShapeExpr>();
    auto k_shape = ((root_call->args[1]->ty).as_or_throw<TensorType>()->shape.value())
                       .as_or_throw<ShapeExpr>();
    auto v_shape = ((root_call->args[2]->ty).as_or_throw<TensorType>()->shape.value())
                       .as_or_throw<ShapeExpr>();
