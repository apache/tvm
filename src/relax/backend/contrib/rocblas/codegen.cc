...
/*!
 * \file src/relax/backend/contrib/rocblas/codegen.cc
 * \brief Implementation of the ROCBLAS JSON serializer.
 */
...
class RocblasJSONSerializer : public JSONSerializer {
 public:
  RocblasJSONSerializer(Map<Constant, String> constant_names, Map<Var, Expr> bindings)
...
Array<runtime::Module> RocblasCompiler(Array<Function> functions, Map<String, ObjectRef> /*unused*/,
                                      Map<Constant, String> constant_names) {
  Array<runtime::Module> compiled_functions;

  for (const auto& func : functions) {
    RocblasJSONSerializer serializer(constant_names, AnalyzeVar2Value(func));
    serializer.serialize(func);
    auto graph_json = serializer.GetJSON();
    auto constant_names = serializer.GetConstantNames();
    const auto* pf = runtime::Registry::Get("runtime.RocblasJSONRuntimeCreate");
    ICHECK(pf != nullptr) << "Cannot find ROCBLAS runtime module create function.";
...
TVM_REGISTER_GLOBAL("relax.ext.rocblas").set_body_typed(RocblasCompiler);
