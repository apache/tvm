 ...
 private:
  ...
  void SetCallNodeAttribute(std::shared_ptr<JSONGraphNode> node, const CallNode* call) {
    ...
    if (call && call->op.as<OpNode>()) {
      ...
      if (!call->args.empty()) {
        auto input_sinfo = GetStructInfo(call->args[0]);
        if (auto tensor_sinfo = input_sinfo.as<TensorStructInfoNode>()) {
          if (tensor_sinfo->shape.defined()) {
            std::vector<std::string> shape_strs;
            for (const auto& dim : tensor_sinfo->shape.value()) {
              if (auto int_imm = dim.as<IntImmNode>()) {
                shape_strs.push_back(std::to_string(int_imm->value));
              } else {
                shape_strs.push_back("-1");  // Dynamic shape
              }
            }
          ...
          std::vector<std::string> dtype_strs{tensor_sinfo->dtype.code() == kDLFloat ? "float32" : "int32"};
...
Array<runtime::Module> GNACompiler(Array<Function> functions, Map<String, ffi::Any> /*unused*/,
  ...
  for (const auto& func : functions) {
    ...
    // Create JSON runtime module for GNA execution
    const auto pf = tvm::ffi::Function::GetGlobalRequired("runtime.JSONRuntimeCreate");
    auto func_name = GetExtSymbol(func);
    compiled_functions.push_back(pf(func_name, graph_json, constant_names_used).cast<runtime::Module>());
  }
  
  return compiled_functions;
}

TVM_FFI_REGISTER_GLOBAL("relax.ext.gna").set_body_typed(GNACompiler);
...
TVM_REGISTER_TARGET_KIND("gna", kDLExtDev)
    .set_default_keys({"gna"});
