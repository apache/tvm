...
class PatternKindAnalyzer : public StmtExprVisitor {
 ...
 private:
  ...
  ffi::Optional<VisitInterrupt> Visit_(const BufferStoreNode* op) final {
    ...
    if (store_.has_value() && !IsSameArray(op->indices, store_.value()->indices)) {
      kind_ = kOpaque;
      return std::nullopt;
    }
  ...
  ffi::Optional<VisitInterrupt> Visit_(const SBlockNode* op) final {
    if (op->name_hint == "root") {
      // Skip the root block
      if (auto interrupt = StmtExprVisitor::Visit(op->body)) return interrupt;
      return std::nullopt;
    }
...
bool HasReshapePattern(const PrimFunc& func) {
  class ReshapeDetector : public StmtExprVisitor {
   public:
   ...
   private:
    ffi::Optional<VisitInterrupt> Visit_(const ForNode* loop) final {
      ...
      if (auto interrupt = this->Visit(loop->body)) return interrupt;
    ...
    ffi::Optional<VisitInterrupt> Visit_(const SBlockRealizeNode* block_realize) final {
      ...
      if (auto interrupt = this->Visit(block)) return interrupt;
    ...
    ffi::Optional<VisitInterrupt> Visit_(const SBlockNode* block) final {
      ...
      if (block->body->IsInstance<ForNode>()) {
        if (auto interrupt = this->Visit(block->body)) return interrupt;
        return std::nullopt;
      }
