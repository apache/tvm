...
TVM_FFI_STATIC_INIT_BLOCK() {
  namespace refl = tvm::ffi::reflection;
  refl::GlobalDef().def_packed("relax.op.hint_on_device", [](ffi::PackedArgs args, ffi::Any* rv), {
    Expr data = args[0].cast<Expr>();
    Device device = args[1].cast<Device>();
    if (args.size() == 3) {
      String scope = args[2].cast<ffi::String>();
