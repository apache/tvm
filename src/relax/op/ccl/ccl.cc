...
/* relax.ccl.broadcast_from_zero */
Expr broadcast_from_zero(Expr x) {
  static const Op& op = Op::Get("relax.ccl.broadcast_from_zero");
  return Call(op, {std::move(x)}, {}, {});
}

TVM_REGISTER_GLOBAL("relax.op.ccl.broadcast_from_zero").set_body_typed(broadcast_from_zero);
...
TVM_REGISTER_OP("relax.ccl.broadcast_from_zero")
    .set_num_inputs(1)
