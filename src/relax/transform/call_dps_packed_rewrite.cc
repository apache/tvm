 ...
 */
/*!
 * \file src/relax/transform/call_dps_packed_rewrite.cc
    ...
    if (call->op == call_dps_packed_op) {
      ...
      if (const auto& _tensor_sinfo = MatchStructInfo<TensorStructInfo>(expr)) {
        ...
        ICHECK(tensor_sinfo->shape.defined())
            << "the TensorStructInfo shape of call_dps_packed has not populated";
        outs.push_back(
        ...
        for (size_t i = 0; i < tuple_sinfo->fields.size(); ++i) {
          ...
          ICHECK(field->IsInstance<TensorStructInfoNode>())
              << "call_dps_packed expects Tuple of TensorStructInfo, but got " << field
              << " as an element of TupleStructInfo";
          const auto& field_tensor = Downcast<TensorStructInfo>(field);
          ICHECK(field_tensor->shape.defined())
              << "call_dps_packed expects all TensorStructInfo has shape, but got " << field_tensor
              << " as an element of TupleStructInfo";
          outs.push_back(
              builder_->Emit(Call(alloc_tensor_op,
        ...
        LOG(FATAL)
            << "TypeError: The struct info of call_dps_packed expects to be TensorStructInfo or "
...
Expr CallDPSPackedRewrite(const Expr& e) { return CallDPSPackedMutator().VisitExpr(e); }

namespace transform {

Pass CallDPSPackedRewrite() {
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
      [=](Function f, IRModule m, PassContext pc) {
        return Downcast<Function>(CallDPSPackedRewrite(f));
      };
  return CreateFunctionPass(pass_func, 0, "CallDPSPackedRewrite", {});
}

TVM_REGISTER_GLOBAL("relax.transform.CallDPSPackedRewrite").set_body_typed(CallDPSPackedRewrite);
