...
std::pair<BufferTouch, ffi::Map<Var, Range>> ControlFlowGraph::ControlFlowBlock::MakeBufferTouch(
    const tirx::Buffer& buf, ffi::Array<Var> index_variables, ffi::Array<PrimExpr> indices,
...
ControlFlowGraph::ControlFlowGraph(const tirx::Stmt& stmt, int64_t max_simplification_steps,
                                   size_t max_revisits)
...
void ControlFlowGraph::RemoveStore(const tirx::BufferStore& store) {
...
PrimExpr BufferState::SubstituteKnownBufferValues(
    PrimExpr expr, const ffi::Map<tirx::Buffer, ffi::Array<tirx::Var>>& axis_var_lookup,
...
void BufferState::Substitute(const ffi::Map<Var, PrimExpr>& var_remap, Analyzer* analyzer) {
  if (var_remap.size()) {
    for (auto& prior : constraints_) {
      PrimExpr updated = tvm::tirx::Substitute(prior.predicate, var_remap);
...
void BufferState::BackpropUnusedIndices(const ffi::Map<Buffer, ffi::Array<Var>>& axis_var_lookup,
  ...
  for (auto [buffer, predicate] : regions_written) {
    constraints_.push_back(
        BufferTouch{buffer, predicate, tirx::Call(buffer->dtype, builtin::undef(), {})});
  }
...
bool ControlFlowGraph::IsOverwrittenWithoutEffect(const tirx::BufferStore& store,
...
PrimExpr ControlFlowGraph::SimplifyInContext(PrimExpr expr, const tirx::Stmt& context,
                                             Analyzer* analyzer) const {
  size_t context_index = [&]() {
    auto it = control_flow_lookup_.find(context.get());
    TVM_FFI_ICHECK(it != control_flow_lookup_.end())
