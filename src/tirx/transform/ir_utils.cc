...
class IRConvertSSA final : public StmtExprMutator {
 ...
 private:
  ...
  void PopVarRemap(const Var& old_var, const Var& new_var) {
    ...
    if (auto it = buf_remap_.find(old_var.get()); it != buf_remap_.end() && it->second.size()) {
      it->second.pop_back();
    }
  ...
  void PopAllRemapsInCurrentScope() {
    ...
    while (current.size()) {
      ...
      if (auto it = buf_remap_.find(remap.old_var.get());
          it != buf_remap_.end() && it->second.size()) {
        it->second.pop_back();
      }
      current.pop_back();
    }
  ...
  struct ScopeLevel {
    ...
    ~ScopeLevel() {
      ...
      while (remaps.size()) {
        ...
        if (auto it = parent->buf_remap_.find(remap.old_var.get());
            it != parent->buf_remap_.end() && it->second.size()) {
          it->second.pop_back();
        }
        remaps.pop_back();
      }
