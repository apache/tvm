 ...
 public:
  ...
  PrimExpr MutateIfThenElseExpr_(const CallNode* op) {
    ...
    if (cond.same_as(op->args[0]) && t.same_as(op->args[1]) && f.same_as(op->args[2])) {
      ...
      if (is_scalable) {
        return Call(op->dtype.with_scalable_vscale_factor(lanes), op->op, {cond, t, f}, op->span,
                    op->annotations);
      } else {
        return Call(op->dtype.with_lanes(lanes), op->op, {cond, t, f}, op->span, op->annotations);
      }
    }
  }
  // Reinterpret expr
  PrimExpr MutateReinterpretExpr_(const CallNode* op) {
    ...
    if (value.same_as(op->args[0])) {
      ...
      if (value.dtype().is_scalable_vector()) {
        return Call(op->dtype.with_scalable_vscale_factor(lanes), op->op, {value}, op->span,
                    op->annotations);
      } else {
        int new_lanes = (op->dtype != DataType::Float4E2M1FN() &&
                         op->args[0].dtype() != DataType::Float4E2M1FN())
                            ...
                            : value.dtype().lanes();
        return Call(op->dtype.with_lanes(new_lanes), op->op, {value}, op->span, op->annotations);
      }
    }
  }
  // Call
  PrimExpr VisitExpr_(const CallNode* op) final {
    if (op->op.same_as(builtin::if_then_else())) {
      ...
      return Call(op->dtype.with_lanes(lane), op->op, new_args, op->span, op->annotations);
