...
TVM_STATIC_IR_FUNCTOR(IRDocsifier, vtable)
    .set_dispatch<tirx::PrimFunc>("", [](tirx::PrimFunc func, ffi::reflection::AccessPath p, IRDocsifier d) -> Doc {
      ...
      for (int i = 0; i < n_args; ++i) {
        ...
        ffi::reflection::AccessPath var_p = p->Attr("params")->ArrayItem(i);
        if (d->cfg->syntax_sugar && CountVarOccurrence(func, var) == 2 &&
          ...
          if (IsSimpleBuffer(buffer) && buffer_data_counter.at(buffer->data.get()) == 1) {
            ffi::reflection::AccessPath buffer_p = p->Attr("buffer_map")->MapItem(var);
      ...
      if (d->cfg->syntax_sugar && implicit_root_block) {
        ...
        ffi::reflection::AccessPath root_block_p = p->Attr("body")->Attr("block");
        ...
        for (int i = 0, n = root_block->alloc_buffers.size(); i < n; ++i) {
          ...
          ffi::reflection::AccessPath buffer_p = root_block_p->Attr("alloc_buffers")->ArrayItem(i);
          IdDoc lhs = DefineBuffer(buffer, *f, d);
      ...
      if (!func->attrs.defined() || !func->attrs->dict.count(tvm::attr::kGlobalSymbol)) {
        ...
        decorator = decorator->Call(pos_args, {"private"},
                                    {LiteralDoc::Boolean(true, ffi::Optional<ffi::reflection::AccessPath>())});
      }
...
TVM_STATIC_IR_FUNCTOR(IRDocsifier, vtable)
    .set_dispatch<tvm::GlobalVar>(                                            //
        "tirx", [](tvm::GlobalVar n, ffi::reflection::AccessPath n_p, IRDocsifier d) -> Doc {  //
...
TVM_STATIC_IR_FUNCTOR(IRDocsifier, vtable)
    .set_dispatch<tvm::IRModule>(                                              //
        "tirx", [](tvm::IRModule mod, ffi::reflection::AccessPath n_p, IRDocsifier d) -> Doc {  //
