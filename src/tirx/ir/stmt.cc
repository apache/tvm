...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef()
      .def("tirx._structured_msg", [](ffi::AnyView /*ctx*/, AssertStmt node) -> ffi::Array<ObjectRef> {
      ...
      .def("tirx._evaluate_is_return", [](ffi::AnyView /*ctx*/, Evaluate self) -> bool {
        if (auto* call = self->value.as<tirx::CallNode>()) {
          return call->op.same_as(tirx::builtin::ret()) && call->args.size() == 1;
        }
        return false;
      })
      .def("tirx._evaluate_expr", [](ffi::AnyView /*ctx*/, Evaluate self) -> PrimExpr {
        if (auto* call = self->value.as<tirx::CallNode>()) {
      ...
      .def("tirx._evaluate_kind", [](ffi::AnyView /*ctx*/, Evaluate self) -> ffi::Optional<ffi::String> {
...
static ::tvm::ffi::pyast::NodeAST PrintSBlockRealize(
    SBlockRealize realize, ::tvm::ffi::pyast::IRPrinter printer,
    ::tvm::ffi::pyast::AccessPath path) {
