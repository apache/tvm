...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef()
      .def("tirx._tir_call_callee",
          [](tirx::Call call, ffi::ir::text::IRPrinter printer, ffi::ObjectRef /*frame*/) -> ffi::Any {
  ...
  refl::GlobalDef()
      ...
      .def("tirx._var_type_or_null", [](Var node) -> ffi::Optional<Type> {
      ...
      .def("tirx._sizevar_type_or_null", [](SizeVar node) -> ffi::Optional<Type> {
        if (!node->type_annotation.defined()) return ffi::Optional<Type>();
        if (const auto* tt = node->type_annotation.as<TupleTypeNode>()) {
          if (tt->fields.empty()) return ffi::Optional<Type>();
        }
      ...
      // IterVar args
      .def("tirx._iter_var_args", [](IterVar node) -> ffi::Array<ObjectRef> {
        const char* type_str;
      ...
      // Cast args
      .def("tirx._cast_args", [](Cast node) -> ffi::Array<ObjectRef> {
...
#define TVM_TIRX_BINOP_SUGAR(lower, NodeTy, sugar_fn)                         \
      .def("tirx._" #lower "_sugar", [](tirx::NodeTy node) -> bool {          \
...
#undef TVM_TIRX_BINOP_SUGAR
      ...
      .def("tirx._div_sugar", [](tirx::Div node) -> bool {
      ...
      // Select args
      .def("tirx._select_args", [](Select node) -> ffi::Array<PrimExpr> {
        return {node->condition, node->true_value, node->false_value};
      })
      // Ramp args
      .def("tirx._ramp_args", [](Ramp node) -> ffi::Array<PrimExpr> {
        return {node->base, node->stride, node->lanes};
      })
      // Broadcast args
      .def("tirx._broadcast_args", [](Broadcast node) -> ffi::Array<PrimExpr> {
        return {node->value, node->lanes};
      })
      // Call callee and args
      .def("tirx._call_callee", [](tirx::Call node) -> ffi::String {
      ...
      .def("tirx._call_args", [](tirx::Call node) -> ffi::Array<ObjectRef> {
        ffi::Array<ObjectRef> result;
      ...
      // Shuffle args
      .def("tirx._shuffle_args", [](Shuffle node) -> ffi::Array<ObjectRef> {
        ffi::Array<ObjectRef> result;
      ...
      .def("tirx._reduce_positional", [](Reduce node) -> ffi::Array<ObjectRef> {
        return {node->combiner};
      })
      .def("tirx._reduce_kwargs", [](Reduce node) -> ffi::Map<ffi::String, ObjectRef> {
      ...
      .def("tirx._load_indices", [](BufferLoad node) -> ffi::Array<ObjectRef> {
      ...
      .def("tirx._store_indices", [](BufferStore node) -> ffi::Array<ObjectRef> {
        if (node->predicate.defined()) return node->indices;
        ffi::Array<ObjectRef> result;
        for (const auto& idx : node->indices) {
          if (const auto* ramp = idx.as<RampNode>()) {
      ...
      .def("tirx._buf_region_indices", [](BufferRegion node) -> ffi::Array<ObjectRef> {
        ffi::Array<ObjectRef> result;
