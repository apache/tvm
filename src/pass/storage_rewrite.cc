...
//
// The current detector check if for every access of dst and src
// is in form: dst[index] = f(src[index]) and index match up
//
// WARNING: the current detection algorithm cannot handle the case
// when same location in an array is written multiple times
//
...
//  A[0] = B[0] + 1
//
// The high level code generator need to ensure that the generated
// code only write each location of target once.
//
// This is true for IR generated by compute schedule.
