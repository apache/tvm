...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def("sym.IntervalSet", MakeIntervalSet);
}
...
ffi::Map<Var, sym::IntSet> AsIntSet(const ffi::Map<Var, Range>& var_dom) {
  ffi::Map<Var, sym::IntSet> result;
  for (auto kv : var_dom) {
    const Var& var = kv.first;
    const Range& range = kv.second;
    result.Set(var, sym::IntSet::FromRange(range));
  }
  return result;
}
...
ffi::Array<IntSet> EstimateRegionUpperBound(const ffi::Array<Range>& region,
  ...
  AnalyzerObj* analyzer_ptr = analyzer.get();
  if (ffi::Optional<ffi::Array<sym::IntSet>> result = EstimateRegionStrictBound(
          /*region=*/region,
          /*var_dom=*/var_dom,
...
TVM_FFI_STATIC_INIT_BLOCK() {
  namespace refl = tvm::ffi::reflection;
  refl::GlobalDef()
      .def("sym.intset_single_point", IntSet::SinglePoint)
      .def("sym.intset_vector", IntSet::Vector)
      .def("sym.intset_interval", IntSet::Interval)
      .def_method("sym.IntervalSetGetMin", &IntSet::min)
      .def_method("sym.IntervalSetGetMax", &IntSet::max)
      .def_method("sym.IntSetIsNothing", &IntSet::IsNothing)
      .def_method("sym.IntSetIsEverything", &IntSet::IsEverything)
      .def("sym.EstimateRegionLowerBound",
      ...
      .def("sym.EstimateRegionStrictBound",
           [](ffi::Array<Range> region, ffi::Map<Var, Range> var_dom, PrimExpr predicate,
              ffi::Optional<Analyzer> opt_analyzer) -> ffi::Optional<ffi::Array<IntSet>> {
             Analyzer analyzer = opt_analyzer.has_value() ? opt_analyzer.value() : Analyzer();
             return EstimateRegionStrictBound(region, var_dom, predicate, analyzer);
           })
      .def("sym.EstimateRegionUpperBound",
           [](ffi::Array<Range> region, ffi::Map<Var, Range> var_dom, PrimExpr predicate,
              ffi::Optional<Analyzer> opt_analyzer) -> ffi::Optional<ffi::Array<IntSet>> {
             Analyzer analyzer = opt_analyzer.has_value() ? opt_analyzer.value() : Analyzer();
             return EstimateRegionUpperBound(region, var_dom, predicate, analyzer);
           })
      .def("sym.PosInf", []() { return SymbolicLimits::pos_inf_; })
      .def("sym.NegInf", []() { return SymbolicLimits::neg_inf_; })
      .def("sym.UnionLowerBound", UnionLowerBound);
}
