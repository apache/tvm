 ...
 public:
  ...
  bool Detect(const PrimExpr& e, LinearEqEntry* ret) {
    *ret = Dispatch(e, e);
    ...
    if (!ret->base.defined()) {
      ret->base = prim::IntImm(var_->ty.as_or_throw<PrimType>(), 0);
    }
    if (!ret->coeff.defined()) {
      ret->coeff = prim::IntImm(var_->ty.as_or_throw<PrimType>(), 0);
    }
    return true;
  }
...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef()
      .def("sym.DetectLinearEquation", DetectLinearEquation)
      .def("sym.DetectClipBound", [](const PrimExpr& e, const ffi::Array<PrimVar>& vars) {
