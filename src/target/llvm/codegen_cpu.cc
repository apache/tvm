...
void CodeGenCPU::Init(const std::string& module_name, llvm::TargetMachine* tm,
  ...
  t_tvm_crt_func_registry_ = llvm::StructType::create({t_char_->getPointerTo(), t_void_p_});
  t_tvm_crt_module_ = llvm::StructType::create({t_tvm_crt_func_registry_});
...
void CodeGenCPU::AddStartupFunction() {
  if (export_system_symbols_.size() != 0) {
    if (target_c_runtime_) {
      ...
      for (auto sym : export_system_symbols_) {
        symbols.emplace_back(sym.first);
        funcs.emplace_back(sym.second);
      }
      llvm::DataLayout layout(module_.get());
      llvm::ArrayType* t_func_registry_ptrs = llvm::ArrayType::get(t_void_p_, funcs.size());
      llvm::GlobalVariable* func_registry_ptrs = new llvm::GlobalVariable(
        *module_, t_func_registry_ptrs, true,
        llvm::GlobalValue::InternalLinkage, llvm::ConstantArray::get(t_func_registry_ptrs, funcs),
        "_tvm_func_registry_ptrs");
      uint64_t align = layout.getTypeAllocSize(t_void_p_);
#if TVM_LLVM_VERSION >= 100
