...
public:
   ...
   * of this state is worse than any state in the heap, then the state will not be pushed.
   */
  void Update(const State& state, float score) {
    ...
    if (static_cast<int>(in_heap_.size()) < max_heap_size_) {
      // Simply push the state to the heap if we have capacity
      heap_.emplace_back(state, score);
      std::push_heap(heap_.begin(), heap_.end(), compare_state());
      in_heap_.insert(state_str);
    } else if (score > heap_.front().second) {
      // Pop the worst state in the heap and push the current one
      std::string old_state_str = heap_.front().first.ToStr();
      in_heap_.erase(old_state_str);
      in_heap_.insert(state_str);

      std::pop_heap(heap_.begin(), heap_.end(), compare_state());
      heap_.back() = StateHeapItem(state, score);
      std::push_heap(heap_.begin(), heap_.end(), compare_state());
    }
    if (score > max_score_) {
      max_score_ = score;
  ...
  /*! \brief Get the maximum score in the heap. -Infinity will be returned if the heap is empty. */
  float GetMaxScore() { return max_score_; }

  /*! \brief Get the minimum score in the heap. -Infinity will be returned if the heap is empty. */
  float GetMinScore() {
    if (heap_.empty()) {
      return -1e30;
    }
