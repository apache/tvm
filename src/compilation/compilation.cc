...
Buffer bufferWithOffsetAlignment(Array<Expr> shape, Type dtype, std::string name, int data_alignment, int offset_factor)
{
	...
	return BufferNode::make(data, dtype, shape, Array<Expr>(), elem_offset, name, "", data_alignment, offset_factor);
}

void get_binds(Array<Tensor> args, std::unordered_map<Tensor, Buffer> binds, Map<Tensor, Buffer> &bindsOut, Array<NodeRef> &argListOut, BuildConfig& config)
{
	bindsOut = binds;
	...
	for (const auto &x : args) {
		if (bindsOut.find(x) == bindsOut.end()) {
			...
			auto buf = bufferWithOffsetAlignment(x->shape, x->dtype, x->op->name, config.data_alignment, config.offset_factor);
			bindsOut.Set(x, buf);
...
Stmt BuildStmt(Schedule sch, Array<Tensor> args, std::unordered_map<Tensor, Buffer> binds, bool loopPartition, Array<NodeRef> &argListOut, BuildConfig& config)
{
...
LoweredFunc Lower(Schedule sch, Array<Tensor> args, std::string name, std::unordered_map<Tensor, Buffer> binds, BuildConfig& config)
{
...
runtime::Module BuildModule(Array<LoweredFunc> funcs, Target& target, Target& targetHost, BuildConfig& config)
{
	...
	if (target.hasKey("gpu") && fdevice.size() == 0) {
		LOG(WARNING) << "Specified target " + target.str() + " but cannot find device code. Did you forget to bind?";
	}
