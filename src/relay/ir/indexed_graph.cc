...
std::unique_ptr<IndexedGraph<Expr>> CreateIndexedGraph(const Expr& expr) {
  ...
  class Creator : public MixedModeVisitor {
   public:
    std::unique_ptr<IndexedGraph<Expr>> CreateGraph(const Expr& expr) {
      graph_ = std::make_unique<IndexedGraph<Expr>>();
      VisitExpr(expr);
  ...
  class Annotator : public ExprFunctor<void(const Expr&)> {
   public:
    explicit Annotator(std::unique_ptr<IndexedGraph<Expr>> graph) : graph_(std::move(graph)) {}
