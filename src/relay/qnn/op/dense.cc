...
Expr QnnDensePackCanonicalize(const Attrs& attrs, const Array<Expr>& new_args,
                              const Array<tvm::relay::Type>& arg_types) {
  LOG(FATAL) << "Canonicalization function for qnn.dense_pack is not implemented";
  return Expr();
}
...
Expr MakeQuantizedDensePack(Expr data, Expr weight, Expr input_zero_point, Expr kernel_zero_point,
  ...
  static const Op& op = Op::Get("qnn.dense_pack");
  return Call(op, {data, weight, input_zero_point, kernel_zero_point, input_scale, kernel_scale},
              Attrs(attrs), {});
}

RELAY_REGISTER_OP("qnn.dense_pack")
    .describe(R"code(Applies a linear transformation: :math:`Y = XW^T`.
...
TVM_REGISTER_GLOBAL("relay.qnn.op._make.dense_pack").set_body_typed(MakeQuantizedDensePack);
