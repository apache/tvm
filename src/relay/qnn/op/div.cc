...
/*!
 * \file src/relay/qnn/op/div.cc
 * \brief QNN divide operator.
 */
 ...
 * \return The sequence of Relay ops for div op.
 */
Expr QnnDivCanonicalize(const Attrs& attrs, const Array<Expr>& new_args,
  ...
  /*
  A tensor division c = a / b can be written in terms of respective
  quantized tensors, scales and zero points as
  S_c * (Q_c - zp_c) = S_a * (Q_a - zp_a) / (S_b * (Q_b - zp_b))

  We can consider the operation (Q_a - zp_a) / (Q_b - zp_b) as a different
  quantized tensor of c, Q', with corresponding scale
  S' = S_a / S_b and
  zp' = 0.
  The quantized division then becomes
  Q_c = S'/S_c Q' + z_c,
  ...
  */
  ...
  auto output = Divide(lhs_shifted, rhs_shifted);
...
// QNN Division operator.
QNN_REGISTER_BINARY_OP("div")
.describe("Elementwise division with broadcasting for quantized tensors.")
