...
class QRealizeIntExprNode : public QRealizeExprNode {
 public:
  ...
  TVM_DLL static QRealizeIntExpr make(Expr data, Expr dom_scale);
...
//     // TODO: saturation
//     // clip(data, pow(2, dtype - shift_factor))
...
Expr QuantizeRealize(const Call& ref_call,
                     ...
                     const NodeRef& ctx) {
  // TODO: consider not dtype_activation
  ...
  if (const auto* n = new_args[0].as<QRealizeIntExprNode>()) {
    ...
    if (idom_scale_imm == odom_scale_imm) {
      ...
      return QRealizeIntExprNode::make(data, dom_scale);
    }
    ...
    if (static_cast<int>(shift_nbit) == shift_nbit) {
      if (shift_nbit > 0) {
        ...
        data = Clip(data, clip_min_imm, clip_max_imm);
        return QRealizeIntExprNode::make(data, dom_scale);
      } else {  // shift_nbit < 0, use left shift
        shift_nbit = - shift_nbit;
        // saturation before to avoid overflow during left shifting
        clip_min_imm = int(clip_min_imm) >> int(shift_nbit);
        clip_max_imm = int(clip_max_imm) >> int(shift_nbit);
        LOG(INFO) << "left shift happens";
        LOG(INFO) << "clip_min: " << clip_min_imm;
        LOG(INFO) << "clip_max: " << clip_max_imm;
        data = Clip(data, clip_min_imm, clip_max_imm);
      ...
      return QRealizeIntExprNode::make(round_data, dom_scale);
    }
  }
...
Expr Conv2dRealize(const Call& ref_call,
  ...
  *attrs = *ref_attrs;
  ...
  return QRealizeIntExprNode::make(ret, dom_scale);
}
...
Expr AddRealize(const Call& ref_call,
  ...
  const auto* rhs = new_args[1].as<QRealizeIntExprNode>();
  if (lhs && rhs) {
    float ldom_scale = GetScalarFromConstant<float>(lhs->dom_scale);
    float rdom_scale = GetScalarFromConstant<float>(rhs->dom_scale);
    CHECK_EQ(ldom_scale, rdom_scale);
    Expr ret = ForwardOp(ref_call, {lhs->data, rhs->data});
    return QRealizeIntExprNode::make(ret, lhs->dom_scale);
  }
...
Expr ClipRealize(const Call& ref_call,
  ...
  if (const auto* n = new_args[0].as<QRealizeIntExprNode>()) {
    ...
    return QRealizeIntExprNode::make(ret, n->dom_scale);
  }
  CHECK(!new_args[0]->derived_from<TempExprNode>());
  return Expr(nullptr);
}
