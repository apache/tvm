...
void DocPrinter::Append(const Doc& doc, const PrinterConfig& cfg) {
  for (const ffi::reflection::AccessPath& p : cfg->path_to_underline) {
...
void DocPrinter::PrintDoc(const Doc& doc) {
  ...
  for (const ffi::reflection::AccessPath& path : doc->source_paths) {
    MarkSpan({start_pos, end_pos}, path);
  }
}

void DocPrinter::MarkSpan(const ByteSpan& span, const ffi::reflection::AccessPath& path) {
  int n = path_to_underline_.size();
  for (int i = 0; i < n; ++i) {
    ffi::reflection::AccessPath p = path_to_underline_[i];
