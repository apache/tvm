...
std::string CodeGenCUDA::Finish() {
  if (enable_fp16_) {
    ...
    decl_stream << "class TVM_ALIGNED(2) half {\n"
                   ...
                   "   // == 0x800000\n"
                   "  static int const shift = fp32FractionBits - fp16FractionBits;"
                   "   // == 13\n"
                   ...
                   "   // flt32 infinity\n"
                   "  static int32_t const maxN = 0x477FFFFF;"
                   "   // max flt32 that's a flt16 normal after >> by shift\n"
                   "  static int32_t const minN = 0x38800000;"
                   "   // min flt16 normal as a flt32\n"
                   "  static int32_t const maxZ = 0x33000000;"
                   "   // max fp32 number that's still rounded to zero in fp16\n"
                   ...
                   // If exp16 == 0 (just into the denorm range),
                   // then significant should be shifted right 1.
