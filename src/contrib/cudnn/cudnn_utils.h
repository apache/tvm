...
#ifndef TVM_SRC_CONTRIB_CUDNN_CUDNN_UTILS_H_
#define TVM_SRC_CONTRIB_CUDNN_CUDNN_UTILS_H_
...
const void* CuDNNDataType::GetConst<0>(cudnnDataType_t type) {
  const static int int_v = 0;
  const static float float_v = 0;
  const static double double_v = 0;
...
const void* CuDNNDataType::GetConst<1>(cudnnDataType_t type) {
  const static int int_v = 1;
  const static float float_v = 1.f;
  const static double double_v = 1.f;
  if (type == CUDNN_DATA_FLOAT || type == CUDNN_DATA_HALF) {
    return static_cast<const void*>(&float_v);
  }
  if (type == CUDNN_DATA_DOUBLE) {
...
struct CuDNNHandle {
  ...
  static CuDNNHandle* ThreadLocal() {
    static thread_local CuDNNHandle inst;
    return &inst; 
  }
};  // CuDNNHandle
 ...
 #endif  // TVM_SRC_CONTRIB_CUDNN_CUDNN_UTILS_H_