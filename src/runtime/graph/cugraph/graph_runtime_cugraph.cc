...
class GraphRuntimeCuGraph : public GraphRuntime {
 public:
  ...
  int EndCapture() {
    cudaGraph_t graph;
    CUDA_CALL(cudaStreamEndCapture(static_cast<cudaStream_t>(capture_stream_),
                                   &graph));
    ...
    LOG(INFO)
        << "Num of nodes in the cuda graph created using stream capture API = "
        << numNodes;
...
PackedFunc GraphRuntimeCuGraph::GetFunction(
...
Module GraphRuntimeCuGraphCreate(const std::string& sym_json,
...
TVM_REGISTER_GLOBAL("tvm.graph_runtime_cugraph.create")
    .set_body([](TVMArgs args, TVMRetValue* rv) {
      ...
      *rv = GraphRuntimeCuGraphCreate(args[0], args[1],
                                      GetAllContext(args, ctx_start_arg),
