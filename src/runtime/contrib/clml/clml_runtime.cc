...
class CLMLRuntime : public JSONRuntimeBase {
 ...
 private:
  ...
  std::shared_ptr<cl_ml_tensor_memory_desc_qcom> CreateDepthToSpaceLayer(CachedLayer* layer,
    ...
    result = h_ClmlIntf->clCreateMLOpDepthToSpaceQCOM(workspace->context, 0, &dtos_desc,
                                                      input->tensor, output->tensor, &op,
                                                      tuning_cache);
  ...
  std::shared_ptr<cl_ml_tensor_memory_desc_qcom> CreateResizeLayer(CachedLayer* layer,
    ...
    result = h_ClmlIntf->clCreateMLOpResizeBilinearQCOM(workspace->context, 0, &resize_desc,
                                                        input->tensor, output->tensor, &op,
                                                        tuning_cache);
