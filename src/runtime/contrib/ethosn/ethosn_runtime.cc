...
Module EthosnModule::LoadFromFile(const std::string& path) {
  std::string data;
...
TVM_REGISTER_GLOBAL("runtime.module.loadfile_ethos-n").set_body([](TVMArgs args, TVMRetValue* rv) {
  *rv = EthosnModule::LoadFromFile(args[0]);
});
