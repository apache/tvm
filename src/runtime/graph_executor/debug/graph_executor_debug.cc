...
class GraphExecutorDebug : public GraphExecutor {
 public:
   ...
   */
  std::string RunIndividual(int number, int repeat, int min_repeat_ms, int max_repeat_ms,
    ...
    if (tkey == "rpc") {
      ...
      for (size_t index = 0; index < op_execs_.size(); ++index) {
        time_sec_per_op[index] =
            RunOpRPC(index, number, repeat, min_repeat_ms, max_repeat_ms, limit_zero_time_iterations,
      ...
      for (size_t index = 0; index < op_execs_.size(); ++index) {
        std::string result_str =
            RunIndividualNode(index, number, repeat, min_repeat_ms, max_repeat_ms, limit_zero_time_iterations,
                              cooldown_interval_ms, repeats_to_cooldown);
  ...
  std::string RunIndividualNode(int node_index, int number, int repeat, int min_repeat_ms, int max_repeat_ms,
                                int limit_zero_time_iterations, int cooldown_interval_ms,
                                int repeats_to_cooldown) {
    std::string tkey = module_->type_key();
    ...
    PackedFunc time_evaluator = profiling::WrapTimeEvaluator(
        ...
        repeat, min_repeat_ms, max_repeat_ms, limit_zero_time_iterations, cooldown_interval_ms,
        repeats_to_cooldown);
    return time_evaluator();
  }

  std::vector<double> RunOpRPC(int index, int number, int repeat, int min_repeat_ms, int max_repeat_ms,
                               int limit_zero_time_iterations, int cooldown_interval_ms,
                               int repeats_to_cooldown) {
...
PackedFunc GraphExecutorDebug::GetFunction(const std::string& name,
  ...
  if (name == "debug_get_output") {
    ...
    return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
      ...
      ICHECK_GE(max_repeat_ms, 0);
      ICHECK_GE(limit_zero_time_iterations, 0);
      ICHECK_GE(cooldown_interval_ms, 0);
      ICHECK_GT(repeats_to_cooldown, 0);
      std::string blob =
          this->RunIndividual(number, repeat, min_repeat_ms, max_repeat_ms, limit_zero_time_iterations,
                              cooldown_interval_ms, repeats_to_cooldown);
      TVMByteArray arr;
    ...
    return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
      ...
      int max_repeat_ms = args[4];
      int limit_zero_time_iterations = args[5];
      int cooldown_interval_ms = args[6];
      int repeats_to_cooldown = args[7];
      ...
      ICHECK_GE(cooldown_interval_ms, 0);
      ICHECK_GT(repeats_to_cooldown, 0);
      std::string blob = this->RunIndividualNode(node_index, number, repeat, min_repeat_ms, max_repeat_ms,
