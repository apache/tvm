...
#define AI_BUFFER_FMT_SET(type_id_, sign_bit_, float_bit_, bits_, fbits_)                     \
  AI_BUFFER_FMT_OBJ(AI_BUFFER_FMT_PACK(float_bit_, 0x1, 24) |                                 \
                    AI_BUFFER_FMT_PACK(sign_bit_, 0x1, 23) | AI_BUFFER_FMT_PACK(0, 0x3, 21) | \
                    AI_BUFFER_FMT_PACK(type_id_, 0xF, 17) | AI_BUFFER_FMT_PACK(0, 0x7, 14) |  \
                    AI_BUFFER_FMT_SET_BITS(bits_) | AI_BUFFER_FMT_SET_FBITS(fbits_))

#define AI_BUFFER_FMT_GET(fmt_) (AI_BUFFER_FMT_OBJ(fmt_) & 0x01FFFFFF)

#define AI_BUFFER_FORMAT(buf_) AI_BUFFER_FMT_GET((buf_)->format)
#define AI_BUFFER_WIDTH(buf_) ((buf_)->width)
#define AI_BUFFER_HEIGHT(buf_) ((buf_)->height)
#define AI_BUFFER_CHANNELS(buf_) ((buf_)->channels)
#define AI_BUFFER_N_BATCHES(buf_) ((buf_)->n_batches)
...
#define AI_BUFFER_META_INFO_INTQ(meta_)                                               \
  ((meta_) && ((meta_)->flags & AI_BUFFER_META_HAS_INTQ_INFO)) ? ((meta_)->intq_info) \
                                                               : NULL  // NOLINT
...
#define AI_BUFFER_SIZE(buf_) (((buf_)->width) * ((buf_)->height) * ((buf_)->channels))

#define AI_BUFFER_BYTE_SIZE(count_, fmt_) ((((count_)*AI_BUFFER_FMT_GET_BITS(fmt_)) + 4) >> 3)
