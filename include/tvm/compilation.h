...
struct Target {
	...
	Target(std::string targetName, DLDeviceType deviceType, int max_num_threads, int thread_warp_size, std::unordered_set<std::string>& keys, std::unordered_set<std::string>& options) {
...
*/
EXPORT Stmt BuildStmt(Schedule sch, Array<Tensor> args, std::unordered_map<Tensor, Buffer> binds, bool loopPartition, Array<NodeRef> &argListOut, BuildConfig& config);
...
*/
EXPORT LoweredFunc Lower(Schedule sch, Array<Tensor> args, std::string name, std::unordered_map<Tensor, Buffer> binds, BuildConfig& config);
...
*/
EXPORT runtime::Module BuildModule(Array<LoweredFunc> funcs, Target& target, Target& targetHost, BuildConfig& config);
