 ...
 public:
  ...
  bool SEqualReduce(const FuncStructInfoNode* other, SEqualReducer equal) const {
    return equal.DefEqual(params, other->params) && equal(ret, other->ret) &&
           equal(pure, other->pure) && equal(derive_func, other->derive_func);
  }
...
class FuncStructInfo : public StructInfo {
 public:
   ...
   * \param pure The purity of the function (true by default).
   * \param span The span of the AST.
   *
   ...
   */
  TVM_DLL FuncStructInfo(Array<StructInfo> params, StructInfo ret, bool pure = true,
                         Span span = Span());
   ...
   * \param pure The purity of the function
   ...
   */
  TVM_DLL static FuncStructInfo OpaqueFunc(StructInfoDeriveFunc derive_func, bool pure = false,
                                           Span span = Span());
   ...
   * \note Defaults to an derive func that always return ObjectStructInfo if not specified.
   */
  TVM_DLL static FuncStructInfo OpaqueFunc(StructInfo ret = ObjectStructInfo(), bool pure = false,
                                           Span span = Span());
