...
AUTOGENERATED_RE = re.compile(r'(^<!--- (?:END )?AUTOGENERATED DO NOT EDIT --->$)', re.MULTILINE)
...
def update_readme(sources):
    ...
    def _to_jenkins_name(src):
        """Produce the varying part of the name of the Jenkins build for a Jenkinsfile.

        e.g. given path/to/foo_bar_jenkinsfile.groovy.j2, returns foo-bar to correspond to
    ...
    lines = (
        [''] + # preserve leading \n
        [BADGE_LINE_TEMPLATE.format(build_name=_to_jenkins_name(n)) for n in sorted(sources)] +
        ['']  # preserve trailing \n
    )
    parts[2] = '\n'.join(lines)
...
if __name__ == "__main__":
    ...
    sources = list(TEMPLATES_DIR.glob("*_jenkinsfile.groovy.j2"))
