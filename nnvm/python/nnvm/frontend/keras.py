...
def _convert_merge(insym, keras_layer, _):
    ...
    for i in range(1, len(insym)):
        if merge_type == 'Add':
            ...
            raise_operator_unimplemented('average merge')
        elif merge_type == 'Maximum':
            raise_operator_unimplemented('maximum merge')
        else:
...
def _convert_dense(insym, keras_layer, symtab):
    ...
    if input_dim > 2:
        ...
        if input_dim != 3 or input_shape[0] != 1 or input_shape[1] != 1:
            raise_attribute_invalid(input_shape, 'input shape', 'dense')
...
def _convert_pooling(insym, keras_layer, symtab):
    ...
    raise_operator_unimplemented('pooling with {}'.format(keras_layer))
    ...
    if padding_type == 'ZeroPadding2D':
        if isinstance(padding, int):
            ...
            if isinstance(padding[0], int):
                ...
                raise_attribute_invalid(str(padding), 'padding', padding_type)
        else:
            raise_attribute_invalid(str(padding), 'padding', padding_type)
    else:
