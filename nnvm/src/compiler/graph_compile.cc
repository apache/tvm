...
nnvm::Graph GraphCompile(const nnvm::Graph& g) {
  ...
  const std::vector<std::string>& target =
      g.GetAttr<std::vector<std::string>>("target");
  std::vector<int> device_types{0};
  if (g.HasAttr("device_type")) {
    device_types = g.GetAttr<std::vector<int>>("device_type");
  }
  ...
  if (tar_func_map.size() > 1) {
    ...
    for (size_t i = 0; i < new_idx.num_nodes(); i++) {
      device_vec[new_idx.entry_id(i, 0)] =
          static_cast<int>(new_idx[i].source->attrs.device_type);
    }
    for (uint32_t nid = 0; nid < new_idx.num_nodes(); nid++) {
      const auto& inode = new_idx[nid];
      for (const auto& e : inode.inputs) {
        device_vec[new_idx.entry_id(e)] =
            static_cast<int>(new_idx[e.node_id].source->attrs.device_type);
      }
    }
