...
constexpr CrcMode choose_crc_mode(uint32_t polynomial, uint8_t width, CrcMode mode_limit) {
  return mode_limit <= CrcMode::TABLE && have_crc_table(polynomial, width) ? CrcMode::TABLE
                                                                           : CrcMode::BITWISE;
}
#endif  // DOXYGEN_ONLY
...
template <uint32_t polynomial = POLY_32BIT_ANSI, uint8_t width = 32,
          CrcMode mode_limit = CrcMode::HARDWARE>
class MbedCRC {
  impl::MbedCRC<polynomial, width, impl::choose_crc_mode(polynomial, width, mode_limit)> crc_impl;

 public:
  /* Backwards compatibility */
  enum CrcMode { TABLE = int(::mbed::CrcMode::TABLE), BITWISE = int(::mbed::CrcMode::BITWISE) };
   ...
   *  @note   Default constructor without any arguments is valid only for supported CRC polynomials.
   * :: crc_polynomial_t MbedCRC <POLY_7BIT_SD, 7> ct; --- Valid POLY_7BIT_SD MbedCRC <0x1021, 16>
   * ct; --- Valid POLY_16BIT_CCITT MbedCRC <POLY_16BIT_CCITT, 32> ct; --- Invalid, compilation
   * error MbedCRC <POLY_16BIT_CCITT, 32> ct (i,f,rd,rr) Constructor can be used for not supported
   * polynomials MbedCRC<POLY_16BIT_CCITT, 16> sd(0, 0, false, false); Constructor can also be used
   * for supported polynomials with different initial/final/reflect values
   *
   */
  constexpr MbedCRC(uint32_t initial_xor, uint32_t final_xor, bool reflect_data,
   ...
   */
  // *INDENT-OFF*
  template <uint32_t poly = polynomial,
            std::enable_if_t<poly == POLY_32BIT_ANSI && width == 32, int> = 0>
  constexpr MbedCRC() : MbedCRC(0xFFFFFFFF, 0xFFFFFFFF, true, true) {}

  template <uint32_t poly = polynomial,
            std::enable_if_t<poly == POLY_16BIT_IBM && width == 16, int> = 0>
  constexpr MbedCRC() : MbedCRC(0, 0, true, true) {}

  template <uint32_t poly = polynomial,
            std::enable_if_t<poly == POLY_16BIT_CCITT && width == 16, int> = 0>
  constexpr MbedCRC() : MbedCRC(0xFFFF, 0, false, false) {}

  template <uint32_t poly = polynomial,
            std::enable_if_t<poly == POLY_7BIT_SD && width == 7, int> = 0>
  constexpr MbedCRC() : MbedCRC(0, 0, false, false) {}

  template <uint32_t poly = polynomial,
            std::enable_if_t<poly == POLY_8BIT_CCITT && width == 8, int> = 0>
  constexpr MbedCRC() : MbedCRC(0, 0, false, false) {}
  // *INDENT-ON*
   ...
   */
  int32_t compute_partial_start(uint32_t* crc) { return crc_impl.compute_partial_start(crc); }
   ...
   *  @return  0  on success or a negative in case of failure.
   */
  int32_t compute_partial_stop(uint32_t* crc) { return crc_impl.compute_partial_stop(crc); }

  /** Get the current CRC polynomial.
   *
   * @return  Polynomial value
   */
  static constexpr uint32_t get_polynomial() { return polynomial; }

  /** Get the current CRC width
   *
   * @return  CRC width
   */
  static constexpr uint8_t get_width() { return width; }
};
...
class MbedCRC {
 ...
 private:
  ...
  static constexpr uint32_t reflect_constant(uint32_t data) {
    ...
    return ((data & 0x00000001) << 31) | ((data & 0x00000002) << 29) | ((data & 0x00000004) << 27) |
           ((data & 0x00000008) << 25) | ((data & 0x00000010) << 23) | ((data & 0x00000020) << 21) |
           ((data & 0x00000040) << 19) | ((data & 0x00000080) << 17) | ((data & 0x00000100) << 15) |
           ((data & 0x00000200) << 13) | ((data & 0x00000400) << 11) | ((data & 0x00000800) << 9) |
           ((data & 0x00001000) << 7) | ((data & 0x00002000) << 5) | ((data & 0x00004000) << 3) |
           ((data & 0x00008000) << 1) | ((data & 0x00010000) >> 1) | ((data & 0x00020000) >> 3) |
           ((data & 0x00040000) >> 5) | ((data & 0x00080000) >> 7) | ((data & 0x00100000) >> 9) |
           ((data & 0x00200000) >> 11) | ((data & 0x00400000) >> 13) | ((data & 0x00800000) >> 15) |
           ((data & 0x01000000) >> 17) | ((data & 0x02000000) >> 19) | ((data & 0x04000000) >> 21) |
           ((data & 0x08000000) >> 23) | ((data & 0x10000000) >> 25) | ((data & 0x20000000) >> 27) |
           ((data & 0x40000000) >> 29) | ((data & 0x80000000) >> 31);
  }
   ...
   * @return Reflected value (full 32-bit value)
   */
  static uint32_t reflect(uint32_t data) { return __RBIT(data); }
   ...
   */
  static MSTD_CONSTEXPR_IF_HAS_IS_CONSTANT_EVALUATED uint_fast32_t
  reflect_byte(uint_fast32_t data) {
   ...
   */
  static constexpr uint32_t get_top_polynomial() { return shift_left(polynomial); }
  ...
  // *INDENT-OFF*
  using crc_table_t =
      std::conditional_t<width <= 8, uint8_t, std::conditional_t<width <= 16, uint16_t, uint32_t>>;
  // *INDENT-ON*
   ...
   */
  static constexpr uint32_t get_crc_mask() { return (uint32_t)((uint32_t)2U << (width - 1)) - 1U; }
   ...
   */
  static MSTD_CONSTEXPR_IF_HAS_IS_CONSTANT_EVALUATED uint32_t reflect_crc(uint32_t data) {
   ...
   */
  static constexpr uint32_t shift_left(uint32_t data) { return data << (32 - width); }
   ...
   */
  static constexpr uint32_t shift_right(uint32_t data) { return data >> (32 - width); }
  ...
  /* Check to see if we can do assembler optimizations */
    ...
    (__ARM_ARCH_7A__ == 1U)
  /* ARM that has Thumb-2 - same unified assembly is good for either ARM or Thumb state (LSRS; IT
   * CS; EORCS reg/imm) */
...
#define MBED_CRC_THUMB1 0
  ...
  std::enable_if_t<mode_ == CrcMode::BITWISE, int32_t> do_compute_partial(const uint8_t* data,
   ...
   */
  template <CrcMode mode_ = mode>
  std::enable_if_t<mode_ == CrcMode::TABLE, int32_t> do_compute_partial(const uint8_t* data,
                                                                        crc_data_size_t size,
                                                                        uint32_t* crc) const {
    uint_fast32_t p_crc = *crc;
...
#endif
    uint32_t
    mbed_tiny_compute_crc32(const void* data, int datalen);
