...
processor = AutoTokenizer.from_pretrained("onnx-community/Llama-3.2-1B")
config = AutoConfig.from_pretrained("onnx-community/Llama-3.2-1B")
...
def convert_posit_to_float(posit_tensor, dtype_str, dev):
    ...
    if "custom[posites2]16" in dtype_str:
        ...
        logits_uint_np = logits_uint16.numpy()[0][-1]  # Extract [0][-1] before conversion
        ...
        logits_uint_np = logits_uint32.numpy()[0][-1]  # Extract [0][-1] before conversion
        return posit32_converter_func(logits_uint_np)
    else:
        return posit_tensor.numpy()[0][-1]  # Extract [0][-1] for float types too
...
def benchmark_dual_models(model1_path, model2_path, dtype1, dtype2, input_data):
    ...
    # Model 1 prefill with independent timer
    ...
    # Model 2 prefill with independent timer
    ...
    logits_model1 = convert_posit_to_float(logits_model1_raw, dtype1, dev)
    
    logits_model2_raw = out_model2[0]
    logits_model2 = convert_posit_to_float(logits_model2_raw, dtype2, dev)
    ...
    while len(generation_tokens_model1) < ITER and next_token_model1 != 151643:
        # Model 1
        input_ids_model1 = next_token_model1.reshape(1, 1)
        tvm_inputs_model1 = [tvm.runtime.tensor(input_ids_model1), tvm.runtime.tensor([attention_mask]), tvm.runtime.tensor([position_ids])]

        start = time.time()
        # out_model1 = decoder_model1["main"](*tvm_inputs0_model1, *kvcache_model1, *tvm_inputs1)
        out_model1 = decoder_model1["main"](*tvm_inputs_model1, *kvcache_model1)
        ...
        tvm_inputs_model2 = [tvm.runtime.tensor(input_ids_model2), tvm.runtime.tensor([attention_mask]), tvm.runtime.tensor([position_ids])]
        out_model2 = decoder_model2["main"](*tvm_inputs_model2, *kvcache_model2)
    ...
    # Stop Model 1 timer immediately after its last decode
    total_generation_time_model1 = prefill_time_model1 + np.sum(decode_times_model1)
    
    # Stop Model 2 timer immediately after its last decode  
