...
def benchmark_tvmruntime(decoder_path, input_data):
    ...
    for input_id in input_ids:
        out1 = decoder["main"](tvm.runtime.tensor([[input_id]]), *kvcache, tvm.runtime.tensor([attention_mask]), tvm.runtime.tensor([position_ids]))
    ...
    while next_token != 151643 and len(generation_tokens) < ITER:
        # Decode
        tvm_inputs0 = [tvm.runtime.tensor(input_ids)]
        tvm_inputs1 = [tvm.runtime.tensor([attention_mask]), tvm.runtime.tensor([position_ids])]
        start = time.time()
        out1 = decoder["main"](*tvm_inputs0, *kvcache, *tvm_inputs1)
...
if __name__ == "__main__":
    ...
    benchmark_tvmruntime(args.decoder,
                         prompt)
    print("Done!")