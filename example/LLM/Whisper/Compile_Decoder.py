...
def compile_model(onnx_path, dtype_converter=None, target_bits=None, use_vectorize=False):
    ...
    # mod = from_onnx(mod)
    mod = from_onnx(mod, {"input_ids": (1, 1), "encoder_hidden_states": (1, 1500, 384)})
...
mod_fp32 = compile_model(f"{base_path}/decoder_model.onnx", use_vectorize=True)
start = time.time()
lib = tvm.relax.build(mod_fp32, target=target)
print("Build time: ", time.time() - start)
lib.export_library("./model/decoder_fp32.so")

# Compile FP16 model
print("Compiling FP16 model...")
mod_fp16 = compile_model(f"{base_path}/decoder_model_fp16.onnx", use_vectorize=True)
start = time.time()
lib_fp16 = tvm.relax.build(mod_fp16, target=target)
print("Build time: ", time.time() - start)
lib_fp16.export_library("./model/decoder_fp16.so")

# Compile Posit models
# print("Compiling Posit32 model...")
# mod_base = onnx.load_model(f"{base_path}/decoder_model.onnx")
# mod_base = from_onnx(mod_base)
# start_time = time.time()
# dtype_mutator = ChangeDatatype("float32", "custom[posites2]32", mod_base)
# mod_posit32 = compile_model(f"{base_path}/decoder_model.onnx", 
...
# start_time = time.time()
# lib_posit32.export_library("./model/decoder_posit32es2.so")
