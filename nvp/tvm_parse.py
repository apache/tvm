...
def visit_stmts(primfn):
    g = nx.DiGraph()
    g = generate_nodes(primfn.body, g, depth=0, ws="", DEBUG=True)
    return g
...
def generate_nodes(stmt, G, depth, ws="", DEBUG=False): # tir.stmt.___
    if isinstance(stmt, (_stmt.For)):
        depth = depth+1
        g = nx.DiGraph()
        Dprint(ws+' '+str(type(stmt))+' Start(%d)'%(depth)+'\t[Control]', DEBUG)
        g = generate_nodes(stmt.body, g, depth, ws+'|', DEBUG)
        Dprint(ws+' '+str(type(stmt))+' End(%d)'%(depth)+'\t[Control]', DEBUG)

        # Color MISC nodes
        g = color_misc_nodes(g, DEBUG)

        # start, end = len(g.nodes), len(g.nodes)+1
        start = max(list(g.nodes))+1
        end = start+1
        g.add_node(start, name='For Start[%d]'%(depth), slot='Control', type='For Start')
        # g.add_node(start, name='For(%s)'%(str(stmt.loop_var)), slot='Control', type='For Start')
        [g.add_edge(start, node) for node in g.nodes if g.in_degree(node)==0 and not node==start] # Connect 'For Start' with 'Src'
        
        g.add_node(end, name='For End[%d]'%(depth), slot='Control', type='For End')
        # g.add_node(end, name='For(%s)'%(str(stmt.loop_var)), slot='Control', type='For End')
        ...
        for idx in range(0, len(acc_node_cnt)-2):
            ...
            g_union.add_node(node_num, name='Seq', slot='Control', type='Seq')
        ...
        g = nx.DiGraph()
        g.add_node(0, name="DUMMY", slot='NOP', type='DUMMY')
...
def visit_STORE(stmt, g, parent, ws, DEBUG):
    ...
    g.add_node(node_num, name=str(stmt), slot='Memory', type='Store')
    ...
    # g = visit_exprs_internal(stmt.value, g, node_num, ws+"|", DEBUG)
...
def visit_EXPR(expr, g, parent, ws, DEBUG): # tir.expr.__
    if hasattr(expr, "indices"): # BufferLoad
        node_num = len(g.nodes)
        Dprint(ws+" "+str(type(expr))+" --> "+str(expr)+"\t[Memory](%d)"%(node_num), DEBUG)
        g.add_node(node_num, name=str(expr), slot='Memory', type='Load')
        g.add_edge(node_num, parent)
        ...
        node_num = len(g.nodes)
        Dprint(ws+" "+str(type(expr))+" --> "+str(expr)+"\t[MISC](%d)"%(node_num), DEBUG)
        try:
            found = re.search('<class \'tvm.tir.expr.(.+?)\'>', str(type(expr))).group(1)
        except AttributeError:
            print("%s NOT supported" %(type(expr)))
            assert 0
        g.add_node(node_num, name=str(expr), slot='MISC', type=found)
        # g.add_node(node_num, name=str(expr), slot='Vector', type=found)
        g.add_edge(node_num, parent)
        ...
        if expr.dtype == 'int32':
            Dprint(ws+" "+str(type(expr))+" --> "+str(expr)+"\t[Scalar](%d)"%(node_num), DEBUG)
            g.add_node(node_num, name=str(expr), slot='Scalar', type='Variable')
            g.add_edge(node_num, parent)
        elif expr.dtype == 'float32':
            Dprint(ws+" "+str(type(expr))+" --> "+str(expr)+"\t[Vector](%d)"%(node_num), DEBUG)
            g.add_node(node_num, name=str(expr), slot='Vector', type='Variable')
            g.add_edge(node_num, parent)
        else:
        ...
        node_num = len(g.nodes)
        Dprint(ws+" "+str(type(expr))+" --> "+str(expr)+"\t[Scalar](%d)"%(node_num), DEBUG)
        g.add_node(node_num, name=str(expr), slot='Scalar', type='Immediate')
        g.add_edge(node_num, parent)
    elif isinstance(expr, (_expr.FloatImm)): # FloatImm
        node_num = len(g.nodes)
        Dprint(ws+" "+str(type(expr))+" --> "+str(expr)+"\t[Vector](%d)"%(node_num), DEBUG)
        g.add_node(node_num, name=str(expr), slot='Vector', type='Immediate')
        g.add_edge(node_num, parent)
    else:
        Dprint("##"*10, DEBUG)
