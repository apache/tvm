...
def save_graph_viz(g, viz, dir='graph.png'):
    ...
    color_map = []
    for node in g.nodes:
        if g.nodes[node]['name'] in ['Kernel Start', 'Kernel End']: color_map.append('black')
        ...
        elif g.nodes[node]['slot'] == 'Control': 
            if g.nodes[node]['type'] == 'Seq': color_map.append('lime')
            else: color_map.append('orange') # For Start/End
        elif g.nodes[node]['slot'] == 'NOP': color_map.append('magenta')
        elif g.nodes[node]['slot'] == 'MISC': color_map.append('grey')
        else: assert 0, 'Slot NOT supported!!'
    if viz == 'type':
