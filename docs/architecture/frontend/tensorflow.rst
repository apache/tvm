...
but unfortunately will leave some remaining. The nodes that should be manually
removed are:
...
should be replaced with constants. For example, a model may accept an input
with shape ``(None,20)``. This should be converted to a shape like ``(1,20)``.
The model should be modified accordingly to ensure that these shapes match
...
currently does not support checkpoint (.ckpt). The graphdef needed by the
...
<https://github.com/tensorflow/models/tree/master/research/slim#exporting-the-inference-graph>`_.
...
argument to ```from_tensorflow```. This dictionary maps input names to input
shapes. Please refer to these `test cases
