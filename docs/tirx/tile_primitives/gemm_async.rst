...
The same predicate must guard the matching ``tcgen05.commit`` (directly or
through ``TCGen05Bar.arrive``). Predicating only the commit is incorrect:
without ``pred=`` on ``gemm_async``, every active lane issues each internal
MMA. The predicate is forwarded through both the dense ``tcgen05.mma`` and
block-scaled ``tcgen05.mma.block_scale`` paths.
