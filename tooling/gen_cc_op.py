...
def _codegen_definition(schema: OpSchema) -> str:
    ...
    def _params(f_filter: Callable[[ParamDef], bool], min_num_args: int) -> str:
        params = ", ".join(
            param.cc_as_relax(param.name)
            for param in schema.params
            if f_filter(
                param,
            )
        ...
        if min_num_args is None:
            return f"{{ {params} }}"
        return f"VariadicArgs(Array<Optional<relax::Expr>>{{ {params} }}, {min_num_args})"

    return """
{signature} {{
  static const Op& op = Op::Get("relax.{op_name}");
  return Call(op, {params}, Attrs(nullptr), {sinfo});
}}
TVM_REGISTER_GLOBAL("{ffi_name}").set_body_typed({creator_name});
TVM_RELAX_REGISTER_OP("{op_name}"){sinfo_inference}{legalization};
""".strip().format(
        ffi_name=schema.ffi_name,
        creator_name=schema.creator_name,
        op_name=schema.name,
        signature=_codegen_signature(schema),
        params=_params(lambda param: not is_sinfo_param(param), min_num_args=schema.min_num_args),
        sinfo=_params(is_sinfo_param, min_num_args=None),
        sinfo_inference=(
            f'.set_attr<FInferStructInfo>("FInferStructInfo", {schema.sinfo_inference})'
            if schema.sinfo_inference
            else ""
        ),
        legalization=(
            f'.set_attr<FLegalize>("FLegalize", {schema.legalization})'
            if schema.legalization
            else ""
        ),
    )
