...
def matmul(M, N, K, block_M, block_N, block_K, trans_A, trans_B, dtypeAB, dtypeC, accum_dtype, num_stages, num_threads):
    ...
    @T.prim_func
    def main(A: T.Buffer(A_shape, dtypeAB), B: T.Buffer(B_shape, dtypeAB),
             C: T.Buffer((M, N), dtypeC)):
        bx, by, _ = T.launch_program(T.ceildiv(N, block_N), T.ceildiv(M, block_M), num_threads=num_threads)

        with T.block():
            A_shared = T.alloc_buffer(A_shared_shape, dtypeAB, scope="shared.dyn")
            B_shared = T.alloc_buffer(B_shared_shape, dtypeAB, scope="shared.dyn")
...
def run_gemm(M, N, K, trans_A, trans_B, dtypeAB, dtypeC, dtypeAccum, block_M, block_N, block_K, num_stages=3, num_threads=128):
    program = matmul(M, N, K, block_M, block_N, block_K, trans_A, trans_B, dtypeAB, dtypeC, dtypeAccum, num_stages, num_threads)
...
def test_pad_aligned_f16f16f16_nn():
    run_gemm(512-8, 1024-32, 768-24, False, False, "float16", "float16", "float16", 128, 256, 32, 2)

def test_pad_f16f16f16_nn():
    run_gemm(512-9, 1024-7, 768-5, False, False, "float16", "float16", "float16", 128, 256, 32, 2)

def test_pad_f16f16f32_nn():
    run_gemm(512+19, 1024+17, 768+15, False, False, "float16", "float16", "float32", 128, 64, 32)
