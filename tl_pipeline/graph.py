...
class Node:
    def __init__(self, node_id: int, inputs: List[Union[Tuple['Node', int], 'Node', None]], name: str, sync_type: str, hw_usage: List[str], time: float, buffer_layer: List[str]):
        self.id = node_id
        self.name = name
        self._out_edges = []
        self._in_edges = []
...
def load_model(fname: str) -> List[Node]:
    ...
    ordered_nodes = []
    for node_id, name, sync_type, hw_usage, time, buffer_layer, is_output, inputs in a:
        input_list = []
        ...
        node = Node(node_id, input_list, name, sync_type, hw_usage, time, buffer_layer)
