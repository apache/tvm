...
TEST(NestedMsg, MapToNestedMsgBySInfo) {
  ...
  EXPECT_TRUE(ffi::StructuralEqual()(arr1[0].LeafValue(), TupleGetItem(TupleGetItem(x, 1), 0)));

  EXPECT_TRUE(arr[2].IsLeaf());
  EXPECT_TRUE(ffi::StructuralEqual()(arr[2].LeafValue(), TupleGetItem(x, 2)));
}
...
TEST(NestedMsg, NestedMsgToExpr) {
  ...
  EXPECT_TRUE(ffi::StructuralEqual()(expr, expected));
  ...
  EXPECT_TRUE(ffi::StructuralEqual()(expr1, t));
}
...
TEST(NestedMsg, TransformTupleLeaf) {
  ...
  EXPECT_TRUE(ffi::StructuralEqual()(
