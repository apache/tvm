...
def _compare_caffe_tvm(caffe_out, tvm_out):
...
def _test_op(data, func_op, op_name, **kwargs):
    ...
    _compare_caffe_tvm(tvm_out, caffe_out)
...
def test_forward_Permute():
    """ Permute """
    _test_permute(np.random.rand(1, 3, 10, 10).astype(np.float32),
                  order=[0, 2, 3, 1])
    _test_permute(np.random.rand(1, 3, 10, 10).astype(np.float32),
                  order=[1, 2, 3, 0])
    _test_permute(np.random.rand(3, 10, 10).astype(np.float32),
                  order=[1, 2, 0])
    _test_permute(np.random.rand(3, 10).astype(np.float32), order=[1, 0])

    print("Testing layer Permute pass!")
...
def _test_priorbox(data, **kwargs):
    ...
    # obtain the .caffemodel file and .prototxt file
    (proto_file, blob_file,
     solver_file) = _gen_filename_str("PriorBox", shape_list, **kwargs)
    _gen_model_files(n, proto_file, blob_file, solver_file)
    # run model in Caffe
    caffe_out = _run_caffe(data, proto_file, blob_file)
    # run model in TVM
...
def test_forward_PriorBox():
    ...
    _test_priorbox(data,
                   prior_box_param=dict(min_size=60.0,
                                        aspect_ratio=2.0,
                                        flip=True,
                                        clip=False,
                                        variance=[0.1, 0.1, 0.2, 0.2],
                                        offset=0.5))
    print("Testing layer PriorBox pass!")
...
def _test_ssdmobilentv1(data):
    ...
    proto_file_url = ("https://github.com/fernchen/CaffeModels/raw/"
                        "master/ssdmobilenet/ssdmobilenet.prototxt")
    blob_file_url = ("https://github.com/fernchen/CaffeModels/raw/"
                        "master/ssdmobilenet/ssdmobilenet.caffemodel")
    proto_file = download_testdata(proto_file_url, 'ssdmobilenetv1.prototxt',
                                    module="model")
    blob_file = download_testdata(blob_file_url, 'ssdmobilenetv1.caffemodel',
                                    module='model')
    # run model in Caffe
    caffe_out = _run_caffe(data, proto_file, blob_file)
    # run model in TVM
...
def test_forward_SSDMobilenetv1():
    ...
    print("Testing network SSDMobilenetv1 pass!")
