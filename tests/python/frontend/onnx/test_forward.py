...
def test_aten():
    ...
    def _convert_to_onnx(model, inputs):
        ...
        torch.onnx.export(model, inputs, file_name,
                        export_params=True, verbose=False, opset_version=10)
        onnx_model = onnx.load(file_name)
        return onnx_model
    ...
    def verify_embedding_bag(num_embedding, embedding_dim, data_shape, num_bags=None):
        ...
        torch_input = dummy_data if dummy_offsets is None else (dummy_data, dummy_offsets)
        print(torch_input)
        onnx_model = _convert_to_onnx(model, torch_input)
        torch_out = model(torch_input)
        for target, ctx in tvm.testing.enabled_targets():
            tvm_out = get_tvm_output_with_vm(onnx_model, tvm_inputs, target, ctx, freeze_params=True, convert_to_static=True)
            tvm.testing.assert_allclose(torch_out.numpy(), tvm_out)


    verify_embedding_bag(10, 3, [2, 10])
    verify_embedding_bag(10, 3, [10], num_bags=1)
