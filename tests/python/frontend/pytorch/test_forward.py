...
def test_grid_sample():
    ...
    data_2D = torch.rand([4, 4, 16, 32]).float()
    grid_2D = torch.rand([4, 8, 8, 2]).float()
    data_3D = torch.rand([4, 4, 4, 16, 32]).float()
    grid_3D = torch.rand([4, 4, 8, 8, 2]).float()
    ...
    for _method in methods:
        for _padding in padding_modes:
            for _align in align_corners:
                ...
                #   gpu result used as baseline in tvm topi.image.grid_sample.
