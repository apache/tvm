...
# OneHot
# ----------------------
def _test_forward_one_hot(indices_shape, depth, out_dtype):
    inp_array1 = np.random.randint(0, 5, size=indices_shape)
    with tf.Graph().as_default():
        in1 = tf.placeholder(shape=inp_array1.shape, dtype=inp_array1.dtype)
        out = tf.one_hot(in1, depth, dtype=out_dtype)
        compare_tf_with_tvm(inp_array1, in1.name, out.name)

def test_forward_one_hot():
    _test_forward_one_hot((3,), 3, "int32")
    _test_forward_one_hot((3,), 3, "float32")
    _test_forward_one_hot((2, 2), 5, "int32")
    _test_forward_one_hot((2, 2), 5, "float32")
    _test_forward_one_hot((3, 2, 4, 5), 6, "int32")
    _test_forward_one_hot((3, 2, 4, 5), 6, "float32")
