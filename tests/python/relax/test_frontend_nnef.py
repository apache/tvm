...
def verify_model(
    ...
    for operation in graph.operations:
        if operation.name == "variable":
            ...
            if any(exc in model_path for exc in ["log", "sqrt", "pow", "batch_norm"]):
                ...
                low = -1.0
            data = np.random.uniform(low=low, size=shape).astype("float32")
    ...
    mod = tvm.relax.frontend.nnef.from_nnef(graph)

    exec = relax.build(mod, target=target)
    vm = relax.VirtualMachine(exec, dev)
    inputs = [tvm.nd.array(arr, device=dev) for arr in inputs.values()]
    out = vm["main"](*inputs)
    ...
    for i, base_out in enumerate(outputs):
        tvm.testing.assert_allclose(out[i], outputs[base_out], rtol=rtol, atol=atol)
...
def test_cts_deconv5x5(target, dev):
    verify_model("deconv5x5", target, dev, rtol=1e-5, atol=1e-4)
...
def test_cts_sum_reduce_spatial(target, dev):
    verify_model("sum_reduce_spatial", target, dev, rtol=1e-5, atol=1e-4)
...
@pytest.mark.skip(reason="Replicate - Edge mode is currently not supported in TVM relax")
@tvm.testing.parametrize_targets
...
def test_cts_deconv5x5_pad2_2(target, dev):
    verify_model("deconv5x5_pad2-2", target, dev, rtol=1e-5, atol=1e-4)
