...
has_gna = tvm.get_global_func("relax.ext.gna", True)

gna_enabled = pytest.mark.skipif(
    not has_gna,
    reason="GNA backend not enabled.",
)
...
def test_gna_patterns_registered():
    ...
    assert expected_patterns.issubset(pattern_names), f"Missing patterns: {expected_patterns - pattern_names}"
...
def test_gna_codegen_smoke():
    ...
    seq = tvm.transform.Sequential([
        FuseOpsByPattern(patterns, bind_constants=False, annotate_codegen=False),
    ...
    inputs = [
        np.random.randn(2, 4).astype("float32"),
        np.random.randn(4, 8).astype("float32")
    ]
    
    ref_out = build_and_run(MatmulReLU, inputs)
    assert ref_out.shape == (2, 8)
