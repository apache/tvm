...
from tvm.relax.backend.contrib.rocblas import partition_for_rocblas
...
has_rocblas = tvm.get_global_func("relax.ext.rocblas", True)

rocblas_enabled = pytest.mark.skipif(
    not has_rocblas,
    reason="ROCBLAS not enabled.",
)

pytestmark = [rocblas_enabled]
...
def get_result_with_relax_rocblas_offload(mod, np_inputs):
    mod = partition_for_rocblas(mod)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # ND x ND
        ((5, 3, 32, 8), (5, 3, 8, 10), True, "none"),
        # ND x 2D
        # ((5, 3, 32, 8), (8, 10), False, "none"),  # not support now
    ],
)
@pytest.mark.parametrize(
    "dtype",
...
def test_matmul_offload(
    ...
    out = get_result_with_relax_rocblas_offload(mod, args)
    ref = build_and_run(mod, args, "llvm", legalize=True)

    tvm.testing.assert_allclose(out, ref, rtol=1e-2, atol=1e-2)


def test_rocblas_partition_matmul_without_bias():
    # rocBLAS does not handle 2D bias (residual input)
...
def test_rocblas_matmul():
    ...
    out = get_result_with_relax_rocblas_offload(Mod, inputs)
