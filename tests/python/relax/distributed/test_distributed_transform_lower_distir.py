...
class LoweredMLP:
    ...
    @R.function
    def foo(x: R.Tensor((128, 128), dtype="float32"), weight1: R.Tensor((128, 128), dtype="float32"), weight2: R.Tensor((128, 128), dtype="float32")) -> R.Tensor((128, 128), dtype="float32"):
        R.func_attr({"num_input": 1})
        cls = LoweredMLP
        gv: R.Tensor((128, 128), dtype="float32") = R.ccl.broadcast_from_worker0(x)
        gv1: R.Tensor((128, 64), dtype="float32") = R.ccl.scatter_from_worker0(weight1, num_workers=2, axis=1)
        gv2: R.Tensor((64, 128), dtype="float32") = R.ccl.scatter_from_worker0(weight2, num_workers=2, axis=0)
        lv0 = R.call_tir(cls.matmul1, (gv, gv1), out_sinfo=R.Tensor((128, 64), dtype="float32"))
        lv1 = R.call_tir(cls.gelu1, (lv0,), out_sinfo=R.Tensor((128, 64), dtype="float32"))
        lv2: R.Tensor((128, 64), dtype="float32") = lv1
        gv_1 = R.call_tir(cls.matmul2, (lv2, gv2), out_sinfo=R.Tensor((128, 128), dtype="float32"))
...
class MLPWithTuple:
    ...
    @T.prim_func(private=True)
    def matmul11(A: T.Buffer((T.int64(64), T.int64(64)), "float32"), B: T.Buffer((T.int64(64), T.int64(128)), "float32"), matmul: T.Buffer((T.int64(64), T.int64(128)), "float32")):
        T.func_attr({"tir.noalias": T.bool(True)})
        # with T.block("root"):
    ...
    @T.prim_func(private=True)
    def matmul2(A: T.Buffer((T.int64(128), T.int64(128)), "float32"), B: T.Buffer((T.int64(128), T.int64(64)), "float32"), matmul: T.Buffer((T.int64(128), T.int64(64)), "float32")):
        T.func_attr({"tir.noalias": T.bool(True)})
        # with T.block("root"):
        for i0, i1, k in T.grid(T.int64(128), T.int64(64), T.int64(128)):
            with T.block("matmul"):
    ...
    @T.prim_func(private=True)
    def split11(A: T.Buffer((128, 64), "float32"), T_split: T.Buffer((64, 64), "float32"), T_split_1: T.Buffer((64, 64), "float32")):
        T.func_attr({"tir.noalias": T.bool(True)})
        # with T.block("root"):
    ...
    def foo(x: R.DTensor((128, 128), "float32", "mesh[0]", "R"), weight_packed: R.Tuple(R.DTensor((128, 128), "float32", "mesh[0]", "S[1]"), R.DTensor((128, 128), "float32", "mesh[0]", "S[0]"))) -> R.DTensor((64, 128), "float32", "mesh[0]", "R"):
        ...
        lv0: R.DTensor((128, 128), "float32", "mesh[0]", "S[1]") = R.dist.call_tir_local_view(cls.matmul2, (x, weight1), out_sinfo=R.DTensor((128, 128), "float32", "mesh[0]", "S[1]"))
        lv1: R.DTensor((128, 128), "float32", "mesh[0]", "S[1]") = R.dist.call_tir_local_view(cls.gelu1, (lv0,), out_sinfo=R.DTensor((128, 128), "float32", "mesh[0]", "S[1]"))
        gv: R.Tuple(R.DTensor((64, 128), "float32", "mesh[0]", "S[1]"), R.DTensor((64, 128), "float32", "mesh[0]", "S[1]")) = R.dist.call_tir_local_view(cls.split11, (lv1,), out_sinfo=[R.DTensor((64, 128), "float32", "mesh[0]", "S[1]"), R.DTensor((64, 128), "float32", "mesh[0]", "S[1]")])
        ...
        gv_1: R.DTensor((64, 128), "float32", "mesh[0]", "R") = R.dist.call_tir_local_view(cls.matmul11, (lv3, weight2), out_sinfo=R.DTensor((64, 128), "float32", "mesh[0]", "R"))
        lv4: R.DTensor((64, 128), "float32", "mesh[0]", "R") = R.ccl.allreduce(gv_1, op_type="sum")
        return lv4
...
class LoweredMLPWithTuple:
    ...
    @R.function
    def foo(x: R.Tensor((128, 128), dtype="float32"), weight_packed: R.Tuple(R.Tensor((128, 128), dtype="float32"), R.Tensor((128, 128), dtype="float32"))) -> R.Tensor((64, 128), dtype="float32"):
        cls = LoweredMLPWithTuple
        gv: R.Tensor((128, 128), dtype="float32") = R.ccl.broadcast_from_worker0(x)
        gv1: R.Tensor((128, 128), dtype="float32") = weight_packed[0]
        gv2: R.Tensor((128, 64), dtype="float32") = R.ccl.scatter_from_worker0(gv1, num_workers=2, axis=1)
        gv3: R.Tensor((128, 128), dtype="float32") = weight_packed[1]
        gv4: R.Tensor((64, 128), dtype="float32") = R.ccl.scatter_from_worker0(gv3, num_workers=2, axis=0)
        lv0 = R.call_tir(cls.matmul2, (gv, gv2), out_sinfo=R.Tensor((128, 64), dtype="float32"))
        lv1 = R.call_tir(cls.gelu1, (lv0,), out_sinfo=R.Tensor((128, 64), dtype="float32"))
        gv_1 = R.call_tir(cls.split11, (lv1,), out_sinfo=[R.Tensor((64, 64), dtype="float32"), R.Tensor((64, 64), dtype="float32")])
        lv2: R.Tensor((64, 64), dtype="float32") = gv_1[0]
        lv3: R.Tensor((64, 64), dtype="float32") = lv2
        gv_1_1 = R.call_tir(cls.matmul11, (lv3, gv4), out_sinfo=R.Tensor((64, 128), dtype="float32"))
