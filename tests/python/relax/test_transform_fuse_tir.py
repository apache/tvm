...
def test_tuple_get_reshape():
    ...
    class Module:
        @T.prim_func
        def split1(
            A: T.Buffer((T.int64(2), T.int64(4096), T.int64(960)), "float16"),
            T_split: T.Buffer((T.int64(2), T.int64(4096), T.int64(320)), "float16"),
            T_split_1: T.Buffer((T.int64(2), T.int64(4096), T.int64(320)), "float16"),
            T_split_2: T.Buffer((T.int64(2), T.int64(4096), T.int64(320)), "float16"),
        ):
            T.func_attr({"op_pattern": 2, "tir.noalias": T.bool(True)})
            # with T.block("root"):
            ...
            for ax0, ax1, ax2 in T.grid(T.int64(2), T.int64(4096), T.int64(320)):
                with T.block("T_split_1"):
                    v_ax0, v_ax1, v_ax2 = T.axis.remap("SSS", [ax0, ax1, ax2])
                    T.reads(A[v_ax0, v_ax1, v_ax2 + T.int64(320)])
                    T.writes(T_split_1[v_ax0, v_ax1, v_ax2])
                    T_split_1[v_ax0, v_ax1, v_ax2] = A[v_ax0, v_ax1, v_ax2 + T.int64(320)]
            for ax0, ax1, ax2 in T.grid(T.int64(2), T.int64(4096), T.int64(320)):
                with T.block("T_split_2"):
                    v_ax0, v_ax1, v_ax2 = T.axis.remap("SSS", [ax0, ax1, ax2])
                    T.reads(A[v_ax0, v_ax1, v_ax2 + T.int64(640)])
                    T.writes(T_split_2[v_ax0, v_ax1, v_ax2])
                    T_split_2[v_ax0, v_ax1, v_ax2] = A[v_ax0, v_ax1, v_ax2 + T.int64(640)]

        @T.prim_func
        def reshape5(
            A: T.Buffer((T.int64(2), T.int64(4096), T.int64(320)), "float16"),
            T_reshape: T.Buffer((T.int64(2), T.int64(4096), T.int64(8), T.int64(40)), "float16"),
        ):
            T.func_attr({"op_pattern": 2, "tir.noalias": T.bool(True)})
            # with T.block("root"):
        ...
        def fused_reshape5(
            ...
            cls = Module
            with R.dataflow():
                lv52: R.Tensor((2, 4096, 320), dtype="float16") = lv2[0]
                lv = R.call_tir(
                    cls.reshape5, (lv52,), out_sinfo=R.Tensor((2, 4096, 8, 40), dtype="float16")
                )
                R.output(lv)
            return lv
        ...
        @R.function
        def main(
            lv40_1: R.Tensor((2, 4096, 960), dtype="float16")
        ) -> R.Tensor((2, 4096, 8, 40), dtype="float16"):
            cls = Module
            with R.dataflow():
                lv41_1 = R.call_tir(
                ...
                lv645 = cls.fused_reshape5(lv41_1)
                R.output(lv645)
            return lv645
