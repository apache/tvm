...
def test_check_for_static_value(specialize_by_tir_var):
    ...
    if specialize_by_tir_var:
        M, K = Before["main"].params[0].struct_info.shape
        _, N = Before["main"].params[1].struct_info.shape
        symbolic_var_map = {M: 128, K: 64, N: 32}
    else:
        symbolic_var_map = {"M": 128, "K": 64, "N": 32}

    After = relax.transform.CheckForSpecialCase(symbolic_var_map)(Before)
