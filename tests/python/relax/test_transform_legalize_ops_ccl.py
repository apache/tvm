...
def test_scatter_from_worker0():
    ...
    class ScatterFromWorker0:
        ...
        def main(x: R.Tensor((10, 10), "float32"))  -> R.Tensor((5, 10), "float32"):
            gv0: R.Tensor((5, 10), "float32") = R.ccl.scatter_from_worker0(x, 2, tensor_dim=0)
            return gv0

    @I.ir_module
    class Expected:
        @T.prim_func(private=True)
        def reshape(A: T.Buffer((T.int64(10), T.int64(10)), "float32"), T_reshape: T.Buffer((T.int64(2), T.int64(5), T.int64(10)), "float32")):
            T.func_attr({"tir.noalias": T.bool(True)})
            # with T.block("root"):
            for ax0, ax1, ax2 in T.grid(T.int64(2), T.int64(5), T.int64(10)):
                with T.block("T_reshape"):
                    v_ax0, v_ax1, v_ax2 = T.axis.remap("SSS", [ax0, ax1, ax2])
                    T.reads(A[(v_ax0 * T.int64(5) + v_ax2 // T.int64(10) + v_ax1) % T.int64(10), v_ax2 % T.int64(10)])
                    T.writes(T_reshape[v_ax0, v_ax1, v_ax2])
                    T_reshape[v_ax0, v_ax1, v_ax2] = A[(v_ax0 * T.int64(5) + v_ax2 // T.int64(10) + v_ax1) % T.int64(10), v_ax2 % T.int64(10)]

        @R.function
        def main(x: R.Tensor((10, 10), dtype="float32")) -> R.Tensor((5, 10), dtype="float32"):
            cls = Expected
            gv = R.call_tir(cls.reshape, (x,), out_sinfo=R.Tensor((2, 5, 10), dtype="float32"))
            gv0 = R.call_dps_packed("runtime.disco.scatter_from_worker0", (gv,), out_sinfo=R.Tensor((5, 10), dtype="float32"))
            return gv0
    # fmt: on
...
def test_scatter_from_local():
    # fmt: off
    @tvm.script.ir_module
    class ScatterFromWorker0:
        @R.function
        def main(x: R.Tensor((10, 10), "float32"))  -> R.Tensor((5, 10), "float32"):
            gv0: R.Tensor((5, 10), "float32") = R.ccl.scatter_from_local(x, 2, tensor_dim=1)
            return gv0
    ...
    class Expected:
        ...
        def main(x: R.Tensor((10, 10), dtype="float32")) -> R.Tensor((10, 5), dtype="float32"):
            ...
            gv0 = R.call_dps_packed("runtime.disco.scatter_from_local", (gv1,), out_sinfo=R.Tensor((10, 5), dtype="float32"))
            return gv0
    # fmt: on
