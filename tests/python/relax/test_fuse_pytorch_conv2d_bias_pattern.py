...
def test_conv2d_bias_relu_fusion():  
    ...
    patterns = [  
        ("conv2d_bias_activation_with_reshape", make_fused_bias_activation_pattern(  
            "relax.nn.conv2d",   
            with_bias=True,   
    ...
    fused_mod = relax.transform.FuseOpsByPattern(  
        patterns, bind_constants=False  
    )(mod)  
  
    # Verify fusion occurred  
    fused_functions = [name for name in fused_mod.functions.keys()   
                      if "fused" in str(name)]  
...
def test_conv2d_bias_relu_fusion_comparison():  
    ...
    old_patterns = [  
        ("conv2d_bias_activation_old", make_fused_bias_activation_pattern(  
            "relax.nn.conv2d",   
            with_bias=True,   
            activation="relax.nn.relu",   
            allow_reshape=False  
    ...
    old_fused_mod = relax.transform.FuseOpsByPattern(  
        old_patterns, bind_constants=False  
    )(mod)  
  
    # Test with allow_reshape=True  
    new_patterns = [  
        ("conv2d_bias_activation_new", make_fused_bias_activation_pattern(  
            "relax.nn.conv2d",   
            with_bias=True,   
            activation="relax.nn.relu",   
            allow_reshape=True  
    ...
    new_fused_mod = relax.transform.FuseOpsByPattern(  
        new_patterns, bind_constants=False  
    )(mod)  
  
    # Both should create fused functions  
    old_fused_functions = [name for name in old_fused_mod.functions.keys()   
                          if "fused" in str(name)]  
    new_fused_functions = [name for name in new_fused_mod.functions.keys()   
                          if "fused" in str(name)]  
...
def test_conv2d_no_fusion_case():  
    ...
    patterns = [  
        ("conv2d_bias_activation", make_fused_bias_activation_pattern(  
            "relax.nn.conv2d",   
            with_bias=True,   
            activation="relax.nn.relu",   
            allow_reshape=True  
