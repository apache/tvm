...
def test_ir_builder_tir_let():
    with IRBuilder() as ib:
        with T.Bind(tir.IntImm("int32", 2)) as v:
            T.evaluate(1)
    # the let binding generated by IRBuilder
    let_actual = ib.get()

    # the expected Bind + Evaluate sequence (using Evaluate(1) to avoid
    # SeqStmt::Flatten stripping the no-op Evaluate(0))
    let_expected = tir.SeqStmt([tir.Bind(T.int32(), tir.IntImm("int32", 2)), tir.Evaluate(1)])
...
def test_ir_builder_tir_allocate():
    ...
    # the expected AllocBuffer
    buffer_var = tir.Var("v", tvm.ir.PointerType(tvm.ir.PrimType("float32"), "local"))
    buf = tir.decl_buffer([10], "float32", data=buffer_var)
    ir_expected = tir.AllocBuffer(buf, tir.Evaluate(1))
...
def test_ir_builder_tir_decl_buffer():
    ...
    # the expected decl_buffer (now emits AllocBuffer instead of Allocate+DeclBuffer)
    buffer = T.Buffer((128, 128), "float32")
    ir_expected = tir.AllocBuffer(buffer, tir.Evaluate(0))
