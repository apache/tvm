...
def test_printer_ptx_more():
    ...
    _assert_print(tir.op.ptx_fence("acq_rel", "gpu"), 'T.ptx.fence("acq_rel", "gpu")')
    _assert_print(tir.op.ptx_fence("sc", "cta"), 'T.ptx.fence("sc", "cta")')
    _assert_print(
        tir.op.ptx_fence_proxy_async("shared::cta"), 'T.ptx.fence.proxy_async("shared::cta")'
    )
    _assert_print(tir.op.ptx_fence_proxy_async("global"), 'T.ptx.fence.proxy_async("global")')
    _assert_print(tir.op.ptx_fence_mbarrier_init(), "T.ptx.fence.mbarrier_init()")
    ...
    _assert_print(
        tir.op.ptx_map_shared_rank(r, 2), 'r = T.handle()\nT.ptx.mapa(r, 2, "", "u64", "uint64")'
    )
...
def test_printer_cuda_more():
    ...
    _assert_print(
        tir.op.cuda_func_call("f", 1, source_code=""), 'T.cuda.func_call("f", 1, source_code="")'
    )
