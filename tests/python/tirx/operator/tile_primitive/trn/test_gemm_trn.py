...
def test_larger_gemm():
    ...
    def gemm() -> None:
        with Tx.kernel():
            A_sbuf = Tx.alloc_buffer((256, 512), "float32", scope="trn.sbuf", layout=A_layout)
            B_sbuf = Tx.alloc_buffer((512, 256), "float32", scope="trn.sbuf", layout=B_layout)
            C_psum = Tx.alloc_buffer((256, 256), "float32", scope="trn.psum", layout=C_layout)
            Tx.gemm(C_psum, A_sbuf, B_sbuf, C_psum)
...
def test_gemm_in_a_loop():
    ...
    def gemm() -> None:
        with Tx.kernel():
            ...
            C_psum = Tx.alloc_buffer((512, 256), "float32", scope="trn.psum", layout=C_layout)
            for i in range(2):
                for k in range(2):
                    Tx.gemm(
...
def test_gemm_with_stride():
    ...
    def gemm() -> None:
        with Tx.kernel():
            A_sbuf = Tx.alloc_buffer((512, 512, 2), "float32", scope="trn.sbuf", layout=A_layout)
            B_sbuf = Tx.alloc_buffer((512, 2, 256), "float32", scope="trn.sbuf", layout=B_layout)
            C_psum = Tx.alloc_buffer((512, 256), "float32", scope="trn.psum", layout=C_layout)
            for i in range(2):
                for k in range(2):
                    Tx.gemm(
...
def test_gemm_different_shape():
    ...
    def gemm() -> None:
        with Tx.kernel():
            A_sbuf = Tx.alloc_buffer((2, 512, 1024), "float32", scope="trn.sbuf", layout=A_layout)
            B_sbuf = Tx.alloc_buffer((1024, 256), "float32", scope="trn.sbuf", layout=B_layout)
            C_psum = Tx.alloc_buffer((512, 256), "float32", scope="trn.psum", layout=C_layout)
            for i in range(2):
                for k in range(2):
...
def test_gemm_too_large_f_size():
    ...
    def gemm() -> None:
        with Tx.kernel():
            A_sbuf = Tx.alloc_buffer((256, 128), "float32", scope="trn.sbuf", layout=A_layout)
            B_sbuf = Tx.alloc_buffer((128, 1024), "float32", scope="trn.sbuf", layout=B_layout)
            C_psum = Tx.alloc_buffer((256, 1024), "float32", scope="trn.psum", layout=C_layout)
            Tx.gemm(C_psum, A_sbuf, B_sbuf, C_psum)
...
def test_gemm_pf_mismatch_fail():
    ...
    def gemm() -> None:
        with Tx.kernel():
            A_sbuf = Tx.alloc_buffer((512, 1024), "float32", scope="trn.sbuf", layout=A_layout)
            B_sbuf = Tx.alloc_buffer((256, 1024), "float32", scope="trn.sbuf", layout=B_layout)
            C_psum = Tx.alloc_buffer((512, 256), "float32", scope="trn.psum", layout=C_layout)
            for i in range(2):
                for k in range(2):
                    Tx.gemm(
...
def test_gemm_transpose_AB():
    ...
    def gemm() -> None:
        with Tx.kernel():
            A_sbuf = Tx.alloc_buffer((1024, 512), "float32", scope="trn.sbuf", layout=A_layout)
            B_sbuf = Tx.alloc_buffer((256, 1024), "float32", scope="trn.sbuf", layout=B_layout)
            C_psum = Tx.alloc_buffer((512, 256), "float32", scope="trn.psum", layout=C_layout)
            for i in range(2):
                for k in range(2):
