...
def build_module(mod, target, params=None, enable_acl=True, tvm_ops=0, acl_partitions=1, offload_concat=False):
    """Build module with option to build for ACL."""
    if isinstance(mod, tvm.relay.expr.Call):
        mod = tvm.IRModule.from_expr(mod)
    with tvm.transform.PassContext(opt_level=3, disabled_pass=["AlterOpLayout"]):
        if enable_acl:
            mod = arm_compute_lib.partition_for_arm_compute_lib(mod, params, offload_concat=offload_concat)
...
def build_and_run(
    ...
    try:
        lib = build_module(mod, device.target, params, enable_acl, tvm_ops, acl_partitions, offload_concat)
...
def verify_codegen(
    ...
    module = build_module(module, target, tvm_ops=tvm_ops, acl_partitions=num_acl_modules, offload_concat=offload_concat)
