...
def get_mm_uint8_intrin(in_m, in_n, in_k):
    ...
    def mm_uint8_intrinsic(a: T.handle, b: T.handle, c: T.handle, offsets: T.handle):
        A = T.match_buffer(
            a, [T.cast(in_n, dtype="int32") * T.cast(in_m, dtype="int32")], dtype="uint8"
        )
        B = T.match_buffer(
            b, [T.cast(in_m, dtype="int32") * T.cast(in_k, dtype="int32")], dtype="uint8"
        )
        C = T.match_buffer(
            c, [T.cast(in_n, dtype="int32") * T.cast(in_k, dtype="int32")], dtype="int32"
        )
        OFFSETS = T.match_buffer(offsets, [2], dtype="uint8")
        with T.block("root"):
            T.reads(
                A[0 : T.cast(in_n, dtype="int32") * T.cast(in_m, dtype="int32")],
                B[0 : T.cast(in_m, dtype="int32") * T.cast(in_k, dtype="int32")],
                OFFSETS[0:2],
            )
            ...
            for i in T.serial(in_n):
                for s in T.serial(blocks):
                    C[
                        T.ramp(((s * 32) + (i * T.cast(in_k, dtype="int32"))), 1, 32)
                    ] = T.call_llvm_intrin(
                        T.llvm_lookup_intrinsic_id("llvm.hexagon.V6.lvsplatw.128B"),
                        T.uint32(1),
                        (
                            (T.cast(OFFSETS[0], dtype="int32") * T.cast(OFFSETS[1], dtype="int32"))
                            * T.cast(in_m, dtype="int32")
                        ),
                        dtype="int32x32",
                    )
                for blok, ro in T.grid(blocks, unrolled_rows):
                    ...
                    a_b_vrmpy_accumulation_unrolled = T.call_llvm_intrin(
                        ...
                        T.reinterpret(
                            B[T.ramp(B_index_unrolled, 1, 128)], dtype="int32x32"
                        ),  # 32 4 byte inputs (Vu) to vrmpy
                        T.reinterpret(
                            A[T.ramp(A_index_unrolled, 1, 4)], dtype="int32"
                        ),  # 4 byte input (Rt) to vrmpy
                    ...
                    a_b_offsets_vrmpy_accumulation_unrolled_b = T.call_llvm_intrin(
                        ...
                        T.call_llvm_intrin(
                            T.llvm_lookup_intrinsic_id("llvm.hexagon.S2.vsplatrb"),
                            T.uint32(1),
                            a_offset,
                            dtype="int32",
                        ),
                    ...
                    a_b_offsets_vrmpy_accumulation_unrolled_a = T.call_llvm_intrin(
                        ...
                        T.call_llvm_intrin(
                            T.llvm_lookup_intrinsic_id("llvm.hexagon.V6.lvsplatw.128B"),
                            T.uint32(1),
                            T.reinterpret(A[T.ramp(A_index_unrolled, 1, 4)], dtype="int32"),
                            dtype="int32x32",
                        ),
                        T.call_llvm_intrin(
                            T.llvm_lookup_intrinsic_id("llvm.hexagon.S2.vsplatrb"),
        ...
        with T.block("root"):
            for i0, i1, i2 in T.grid(in_m, in_n, in_k):
                with T.block("C"):
                    ...
                    C[y, x] = C[y, x] + T.cast(A[y, j] - OFFSETS[0], "int32") * T.cast(
                        B[j, x] - OFFSETS[1], "int32"
                    )
