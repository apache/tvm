...
def opencl_cost_estimator(mod, target):
    try:
        ...
        config = {} if ("clml" == target.kind.name) else {"relay.backend.collage_in_tuning": True,}
        with tvm.transform.PassContext(config=config, opt_level=3):
            relay.backend.te_compiler.get().clear()
            graph, lib, param = relay.build(
                mod, target=target
            )
...
def collage(model):
    ...
    with optional_tuning_records(TUNING_LOG):
        ...
        with ctxt:
            ...
            compile_and_benchmark("collage", partitioned_model, OPENCL, tmp_dir)
...
def just_clml(model):
    ...
    with tvm.transform.PassContext(opt_level=3):
        ...
        logging.info(partitioned_model["mod"])
        logging.info("-------------- END PARTITIONED ----------------")
        compile_and_benchmark("just_clml", partitioned_model, OPENCL, tmp_dir)
