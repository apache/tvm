...
@pytest.mark.parametrize("dtype", ["int32", "uint8", "uint32", "uint64", "bool"])
@pytest.mark.parametrize("equal", [False, True])
def test_conditional_region_zero_comparison(dtype, equal):
    """The implicit else condition must not introduce unsigned coefficients."""
    tir = tvm.tirx
    mask_buffer = tir.decl_buffer((1,), dtype, name="mask_buffer")
    data = tir.decl_buffer((4,), "int32", name="data")
    output = tir.decl_buffer((1,), "int32", name="output")
    mask = tir.Var("mask", dtype)
    condition = mask == 0 if equal else mask != 0
    body = tir.SeqStmt(
        [
            tir.Bind(mask, mask_buffer[0]),
            tir.BufferStore(output, tir.if_then_else(condition, data[0], data[3]), [0]),
        ]
    )
    block = tir.SBlock([], [], [], "conditional_read", body)
    buffers = {buf: buf for buf in (mask_buffer, data, output)}
    for analyze in (
        s_tir.analysis.get_sblock_access_region,
        s_tir.analysis.get_sblock_read_write_region,
    ):
        regions = analyze(block, buffers)
        reads, writes = regions[:2]
        data_region = next(region for region in reads if region.buffer.same_as(data))
        tvm.ir.assert_structural_equal(data_region.region, [Range(0, 4)])
        tvm.ir.assert_structural_equal(writes, [tir.BufferRegion(output, [Range(0, 1)])])


@pytest.mark.parametrize("dtype", ["uint32", "uint64"])
@pytest.mark.parametrize("condition_kind", ["upper", "high_bit", "wraparound"])
def test_unsigned_conditional_region_preserves_both_branches(dtype, condition_kind):
    """Fallback must not prune accesses using signed or non-wrapping semantics."""
    tir = tvm.tirx
    mask_buffer = tir.decl_buffer((1,), dtype, name="mask_buffer")
    data = tir.decl_buffer((4,), "int32", name="data")
    output = tir.decl_buffer((1,), "int32", name="output")
    mask = tir.Var("mask", dtype)
    if condition_kind == "upper":
        condition = mask < 8
    elif condition_kind == "high_bit":
        condition = mask >= tir.const(1 << (int(dtype[4:]) - 1), dtype)
    else:
        condition = mask + tir.const(1, dtype) < mask
    body = tir.SeqStmt(
        [
            tir.Bind(mask, mask_buffer[0]),
            tir.BufferStore(output, tir.if_then_else(condition, data[0], data[3]), [0]),
    ...
    block = tir.SBlock([], [], [], "unsigned_read", body)
    buffers = {buf: buf for buf in (mask_buffer, data, output)}
    reads, _, _ = s_tir.analysis.get_sblock_access_region(block, buffers)
    data_region = next(region for region in reads if region.buffer.same_as(data))
    tvm.ir.assert_structural_equal(data_region.region, [Range(0, 4)])


@pytest.mark.parametrize("dtype", ["uint32", "uint64"])
def test_unsigned_condition_keeps_independent_signed_bound(dtype):
    tir = tvm.tirx
    mask_buffer = tir.decl_buffer((1,), dtype, name="mask_buffer")
    data = tir.decl_buffer((8,), "int32", name="data")
    output = tir.decl_buffer((8,), "int32", name="output")
    mask = tir.Var("mask", dtype)
    i = tir.Var("i", "int32")
    body = tir.SeqStmt(
        [
            ...
            tir.For(
                ...
                tir.ForKind.SERIAL,
                tir.IfThenElse(
                    tir.And(i < 4, mask == 0), tir.BufferStore(output, data[i], [i]), None
                ),
            ),
        ]
    )
    block = tir.SBlock([], [], [], "mixed_condition", body)
    buffers = {buf: buf for buf in (mask_buffer, data, output)}
    reads, writes, _ = s_tir.analysis.get_sblock_access_region(block, buffers)
    data_region = next(region for region in reads if region.buffer.same_as(data))
    tvm.ir.assert_structural_equal(data_region.region, [Range(0, 4)])
    tvm.ir.assert_structural_equal(writes, [tir.BufferRegion(output, [Range(0, 4)])])
