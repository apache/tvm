...
class ResnetStructure:
    ...
    def tvmgen_default_fused_cast_subtract_fixed_point_multiply_add_clip_cast_cast(placeholder: T.handle, placeholder_1: T.handle, T_cast: T.handle) -> None:
        ...
        for ax0_ax1_fused, ax2, ax3_outer, ax3_inner in T.grid(75, 75, 4, 16):
            T.store(T_cast_1.data, ax0_ax1_fused * 4800 + ax2 * 64 + ax3_outer * 16 + ax3_inner, T.cast(T.cast(T.max(T.min(T.q_multiply_shift(T.cast(placeholder_2.data[ax0_ax1_fused * 4800 + ax2 * 64 + ax3_outer * 16 + ax3_inner], "int32") - 94, 1843157232, 31, 1, dtype="int32") + placeholder_3.data[ax3_outer * 16 + ax3_inner], 255), 0), "uint8"), "int16"), True)
    ...
    def tvmgen_default_fused_nn_conv2d_add_fixed_point_multiply_clip_cast_cast_1(placeholder_10: T.handle, placeholder_11: T.handle, placeholder_12: T.handle, T_cast_4: T.handle) -> None:
        ...
        for i0_i1_fused_1, i2_1, i3_1 in T.grid(77, 77, 64):
            T.store(PaddedInput_1, i0_i1_fused_1 * 4928 + i2_1 * 64 + i3_1, T.if_then_else(1 <= i0_i1_fused_1 and i0_i1_fused_1 < 76 and 1 <= i2_1 and i2_1 < 76, placeholder_13.data[i0_i1_fused_1 * 4800 + i2_1 * 64 + i3_1 - 4864], T.int16(0), dtype="int16"), True)
        for ax0_ax1_fused_ax2_fused_1 in T.serial(0, 5625):
            ...
            for ff_1 in T.serial(0, 64):
                ...
                for ry, rx, rc_1 in T.grid(3, 3, 64):
                    T.store(Conv2dOutput_1, ff_1, Conv2dOutput_1[ff_1] + T.cast(PaddedInput_1[T.floordiv(ax0_ax1_fused_ax2_fused_1, 75) * 4928 + ry * 4928 + rx * 64 + T.floormod(ax0_ax1_fused_ax2_fused_1, 75) * 64 + rc_1], "int32") * T.cast(placeholder_14.data[ry * 12288 + rx * 4096 + rc_1 * 64 + ff_1], "int32"), True)
            for ax3_inner_2 in T.serial(0, 64):
                T.store(T_cast_5.data, ax0_ax1_fused_ax2_fused_1 * 64 + ax3_inner_2, T.cast(T.cast(T.max(T.min(T.q_multiply_shift(Conv2dOutput_1[ax3_inner_2] + placeholder_15.data[ax3_inner_2], 1608879842, 31, -7, dtype="int32"), 255), 0), "uint8"), "int16"), True)
    ...
    def tvmgen_default_fused_nn_conv2d_add_fixed_point_multiply_add_clip_cast_cast_subtract_fixed_point_15934180698220515269_(placeholder_16: T.handle, placeholder_17: T.handle, placeholder_18: T.handle, T_add: T.handle) -> None:
        ...
        for i0_i1_fused_2, i2_2, i3_2 in T.grid(75, 75, 64):
            T.store(PaddedInput_2, i0_i1_fused_2 * 4800 + i2_2 * 64 + i3_2, placeholder_19.data[i0_i1_fused_2 * 4800 + i2_2 * 64 + i3_2], True)
            ...
            for ax3_outer_1 in T.serial(0, 4):
                for ff_2 in T.serial(0, 64):
                    ...
                    for rc_2 in T.serial(0, 64):
                        T.store(Conv2dOutput_2, ff_2, Conv2dOutput_2[ff_2] + T.cast(PaddedInput_2[ax0_ax1_fused_ax2_fused_2 * 64 + rc_2], "int32") * T.cast(placeholder_20.data[rc_2 * 256 + ax3_outer_1 * 64 + ff_2], "int32"), True)
    ...
    def tvmgen_default_fused_nn_conv2d_add_fixed_point_multiply_add_clip_cast_cast_subtract_fixed_point_4200876283395191415_(placeholder_22: T.handle, placeholder_23: T.handle, placeholder_24: T.handle, placeholder_25: T.handle, T_cast_6: T.handle) -> None:
        ...
        for i0_i1_fused_3, i2_3, i3_3 in T.grid(75, 75, 64):
            T.store(PaddedInput_3, i0_i1_fused_3 * 4800 + i2_3 * 64 + i3_3, placeholder_29.data[i0_i1_fused_3 * 4800 + i2_3 * 64 + i3_3], True)
            ...
            for ax3_outer_2 in T.serial(0, 4):
                for ff_3 in T.serial(0, 64):
                    ...
                    for rc_3 in T.serial(0, 64):
                        T.store(Conv2dOutput_3, ff_3, Conv2dOutput_3[ff_3] + T.cast(PaddedInput_3[ax0_ax1_fused_ax2_fused_3 * 64 + rc_3], "int32") * T.cast(placeholder_27.data[rc_3 * 256 + ax3_outer_2 * 64 + ff_3], "int32"), True)
        ...
        for ax0_ax1_fused_ax2_fused in T.serial(0, 5625):
            ...
            for ff in T.serial(0, 64):
                ...
                for rc in T.serial(0, 64):
                    T.store(Conv2dOutput, ff, Conv2dOutput[ff] + T.cast(PaddedInput[ax0_ax1_fused_ax2_fused * 64 + rc], "int32") * T.cast(placeholder_8.data[rc * 64 + ff], "int32"), True)
            for ax3_inner_1 in T.serial(0, 64):
                T.store(T_cast_3.data, ax0_ax1_fused_ax2_fused * 64 + ax3_inner_1, T.cast(T.cast(T.max(T.min(T.q_multiply_shift(Conv2dOutput[ax3_inner_1] + placeholder_9.data[ax3_inner_1], 1843106743, 31, -6, dtype="int32"), 255), 0), "uint8"), "int16"), True)
    __tvm_meta__ = None
# fmt: on
