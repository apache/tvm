...
@T.prim_func
def flattened_predicate_func(A: T.Buffer[(32,), "float32"], C: T.Buffer[(32,), "float32"]) -> None:
    for i, j in T.grid(5, 7):
        if i * 7 + j < 32:
            C[i * 7 + j] = A[i * 7 + j] + 1.0
...
@T.prim_func
def flattened_unit_loop_func(A: T.Buffer[(32,), "float32"], C: T.Buffer[(32,), "float32"]) -> None:
    for x, z in T.grid(4, 8):
        C[x * 8 + z] = A[x * 8 + z] + 1.0


@T.prim_func
def compacted_multi_alloc_func(
    A: T.Buffer[(32,), "float32"], D: T.Buffer[(32,), "float32"]
) -> None:
    for i in range(0, 32):
        with T.block() as []:
            T.reads(A[i])
...
@T.prim_func
def flattened_multi_alloc_func(
    A: T.Buffer[(32,), "float32"], D: T.Buffer[(32,), "float32"]
) -> None:
    for i in range(0, 32):
