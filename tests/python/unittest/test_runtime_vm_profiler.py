...
def test_basic(dev, target):
    ...
    vmp = profiler_vm.VirtualMachineProfiler(des_exe, dev)

    data = np.random.rand(1, 1, 28, 28).astype("float32")
    res = vmp.profile(tvm.nd.array(data), func_name="main")
    assert "softmax" in str(res)


def test_vm_reshape_and_copy():
    """Test VM reshape and copy"""
    dev = tvm.gpu()
    ...
    with tvm.transform.PassContext(opt_level=3):
        exc = relay.vm.compile(mod, "llvm")
    assert "reshape_tensor" in exc.bytecode
    vmp = profiler_vm.VirtualMachineProfiler(exc, dev)
    vmp.profile(tvm.nd.array(x_np))
