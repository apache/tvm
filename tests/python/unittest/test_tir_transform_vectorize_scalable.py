...
def test_vectorize_loop():
    ...
    with ib.for_range(0, n) as i:
        with ib.for_range(0, 4, kind="vectorize_scalable") as j:
    ...
    stmt = tvm.tir.transform.VectorizeLoopScalable()(mod)["main"].body
    print(stmt)
    assert isinstance(stmt, tvm.tir.For)
    assert isinstance(stmt.body, tvm.tir.For)
    assert isinstance(stmt.body.body.index, tvm.tir.Ramp)
    assert isinstance(stmt.body.body.value, tvm.tir.Broadcast)
...
def test_vectorize_with_if():
    ...
    ib = tvm.tir.ir_builder.create()
    A = ib.pointer("float32", name="A")
    with ib.for_range(0, 4, kind="vectorize_scalable") as i:
    ...
    assert isinstance(stmt.then_case, tvm.tir.For)
    assert isinstance(stmt.then_case.body.index, tvm.tir.Ramp)
    assert isinstance(stmt.then_case.body.value, tvm.tir.Add)
    assert stmt.then_case.body.value.dtype == "float32x4"
...
def test_vectorize_let():
    ...
    print(stmt)
    assert isinstance(stmt, tvm.tir.For)
    assert isinstance(stmt.body, tvm.tir.LetStmt)
    assert stmt.body.value.dtype == "float32x4"
...
def test_vectorize_if_then_else():
    ...
    assert not isinstance(stmt.body.body, tvm.tir.For)
    assert isinstance(stmt.body.body.value.args[2], tvm.tir.Broadcast)
