...
def test_tensor_core_float():
    ...
    t = s[BS].fuse(xi, yi)
    to, ti = s[BS].split(t, factor=warp_size)
    s[BS].bind(tx, thread_y)
    s[BS].bind(ty, thread_z)
    s[BS].bind(ti, thread_x)
