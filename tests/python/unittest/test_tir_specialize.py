...
def test_specialize_matmul():
    ...
    assert_structural_equal_gs(func, matmul_128)
    # partially specialized
    func = matmul.specialize({n: 128})
    assert_structural_equal_gs(func, matmul_m_128)
    # symbolic specialized
    func = matmul.specialize({n: tvm.tir.Var("x", "int32") * 8})
    assert_structural_equal_gs(func, matmul_m_8x)
...
def test_specialize_elemwise():
    ...
    assert_structural_equal_gs(func, element_wise_128_64)
    # partially specialized
    func = element_wise.specialize({c: tvm.tir.decl_buffer((128, C.shape[1]))})
    assert_structural_equal_gs(func, element_wise_128_n)
...
def test_specialize_mem_copy():
    ...
    assert_structural_equal_gs(func, mem_copy_16_16_8_4)
    func = mem_copy.specialize({n: 16, m: 16, p: 8, q: 4})
    assert_structural_equal_gs(func, mem_copy_16_16_8_4)
    # partially specialized
    func = mem_copy.specialize({q: n})
    assert_structural_equal_gs(func, mem_copy_m_n_p_n)
...
def test_specialize_with_const_folding():
    ...
    assert_structural_equal_gs(func, param_in_arith_exprs_n_16)
