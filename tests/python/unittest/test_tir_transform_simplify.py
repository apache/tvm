...
def test_basic_likely_elimination():
    ...
    W = te.placeholder(shape=(n + 1,), dtype="int64", name="w")
...
def test_complex_likely_elimination():
    def cumsum(X):
        ...
        """
        (m,) = X.shape
        s_state = te.placeholder((m + 1,), dtype="int64", name="state")
        s_init = te.compute((1,), lambda _: tvm.tir.const(0, "int64"))
    ...
    indices_ph = te.placeholder((i,), name="indices", dtype="int64")
    lengths_ph = te.placeholder((n,), name="lengths", dtype="int64")
...
class TestNestedVarCondition(BaseBeforeAfter):
    ...
    def before(A: T.Buffer((16,), "float32"), n: T.int64):
        for i in T.serial(16):
            if i == n:
                if i == n:
                    A[i] = 0.0

    def expected(A: T.Buffer((16,), "float32"), n: T.int64):
        for i in T.serial(16):
            if i == n:
                A[i] = 0.0
...
class TestAlteredBufferContents(BaseBeforeAfter):
    ...
    def before(A: T.Buffer((1,), "int64"), n: T.int64):
        if A[0] == n:
            A[0] = A[0] + 1
            if A[0] == n:
                A[0] = 0
...
class TestNegationOfCondition(BaseBeforeAfter):
    ...
    def expected(A: T.Buffer((16,), "int64")):
        for i in T.serial(16):
            if i == 5:
                A[i] = 1
...
class TestNegationOfVarCondition(BaseBeforeAfter):
    ...
    def before(A: T.Buffer((16,), "int64"), n: T.int64):
        for i in T.serial(16):
            if i == n:
                if i != n:
                    A[i] = 0
    ...
    def expected(A: T.Buffer((16,), "int64"), n: T.int64):
        for i in T.serial(16):
            if i == n:
                A[i] = 1
...
class TestLiteralConstraintSplitBooleanAnd(BaseBeforeAfter):
    ...
    def before(A: T.Buffer((16, 16), "int64"), n: T.int64):
        for i, j in T.grid(16, 16):
            if i == n and j == n:
                if i == n:
                    A[i, j] = 0

    def expected(A: T.Buffer((16, 16), "int64"), n: T.int64):
        for i, j in T.grid(16, 16):
            if i == n and j == n:
                A[i, j] = 0
...
class TestCeilLog2Int(BaseBeforeAfter):
    ...
    @T.prim_func
    def before(A: T.Buffer(1, "int64")):
        A[0] = T.cast(
            T.ceil(T.log2(T.cast(14, "float64"), dtype="float64"), dtype="float64"), dtype="int64"
        )

    @T.prim_func
    def expected(A: T.Buffer(1, "int64")):
        A[0] = 4
...
class TestLeftShiftLowerBound(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(16, "float32")):
        for i in T.serial(16):
            if T.shift_left(1, i, dtype="int64") >= 1:
                A[i] = 0.0
...
class TestLeftShiftUpperBound(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(16, "float32")):
        for i in T.serial(16):
            if T.shift_left(31, i, dtype="int64") <= 1015808:
                A[i] = 0.0
...
class TestLeftShiftOfNegativeValue(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(16, "float32")):
        for i in T.serial(16):
            if -64 <= T.shift_left(-i, 4, dtype="int64"):
                A[i] = 0.0
...
class TestLeftShiftByNegativeValue(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(16, "float32")):
        for i in T.serial(16):
            if T.shift_left(16, -i, dtype="int64") <= 16:
                A[i] = 0.0
...
class TestRemoveTransitivelyProvableCondition(BaseBeforeAfter):
    ...
    i, j, k = [tvm.tir.Var(name, "int64") for name in "ijk"]
    zero = tvm.tir.IntImm("int64", 0)
...
class TestSuppressTransitivelyProvableCondition(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(1, "bool"), i: T.int64, j: T.int64, k: T.int64):
        if i < j and j < k:
            A[0] = i < k
...
class TestRewriteAsAndOfOrsWithSimplificationBetweenGroups(BaseBeforeAfter):
    ...
    def expected(A: T.Buffer(1, "bool"), i: T.int64, j: T.int64, k: T.int64):
...
class TestConditionalFloorMod(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(1, "bool"), i: T.int64):
        if T.floormod(0 - i, 2) == 0:
            A[0] = T.floormod(i, 2) == 0

    def expected(A: T.Buffer(1, "bool"), i: T.int64):
        if T.floormod(i, -2) == 0:
            A[0] = True
...
class TestSimplifyRHSOfBooleanAndUsingLHS(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(1, "bool"), n: T.int64):
        A[0] = n < 5 and n < 10

    def expected(A: T.Buffer(1, "bool"), n: T.int64):
        A[0] = n < 5
...
class TestSimplifyRHSOfBooleanAndUsingLHSWithoutConst(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(1, "bool"), n: T.int64, m: T.int64):
        A[0] = n < m + 5 and n < m + 10

    def expected(A: T.Buffer(1, "bool"), n: T.int64, m: T.int64):
        A[0] = n < m + 5
...
class TestProvableConditionWithOffset(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(1, "bool"), i: T.int64, j: T.int64):
        if i < j:
            A[0] = i < j + 1

    def expected(A: T.Buffer(1, "bool"), i: T.int64, j: T.int64):
        if i < j:
            A[0] = True
...
class TestMostRestrictiveConditional(BaseBeforeAfter):
    ...
    i, j, k = [tvm.tir.Var(name, "int64") for name in "ijk"]
    tir_int = tvm.tir.IntImm("int64", 0)
...
class TestAlteredBufferContents(BaseBeforeAfter):
    ...
    def expected(A: T.Buffer((1,), "int64"), n: T.int64):
        if A[0] == n:
            A[0] = A[0] + 1
            A[0] = 10
...
class TestPossiblyAlteredBufferContents(BaseBeforeAfter):
    ...
    def before(A: T.Buffer((1,), "int64"), n: T.int64, m: T.int64):
        if A[0] == n:
            if m == 0:
                A[0] = A[0] + 1
...
class TestKeepExpressionSimplifyUsingBufferValue(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(1, "int64"), B: T.Buffer(1, "int64")):
        A[0] = 0
        B[0] = A[0]
...
class TestSimplifyConditionalInLoopUsingBufferValue(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(16, "int64"), B: T.Buffer(16, "int64")):
        for i in T.serial(16):
            A[i] = i
    ...
    def expected(A: T.Buffer(16, "int64"), B: T.Buffer(16, "int64")):
        for i in T.serial(16):
            A[i] = i
...
class TestNoSimplifyUsingLoopDependentBufferValue(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(16, "int64"), B: T.Buffer(1, "int64")):
        B[0] = 0
        for i in T.serial(16):
            if B[0] < 10:
                B[0] = A[i] * 2 + B[0]
...
class TestSimplifyRampIndexBroadcastValue(BaseBeforeAfter):
    ...
    def expected(A: T.Buffer(4, "int64")):
        A[T.ramp(0, 1, 4)] = T.broadcast(0, 4)
...
class TestSimplifyUsingPartiallyProvenBufferValueGather(BaseBeforeAfter):
    ...
    def before(A: T.Buffer(24, "int64"), B: T.Buffer(24, "int64"), F: T.Buffer(3, "int64")):
    ...
    def expected(A: T.Buffer(24, "int64"), B: T.Buffer(24, "int64"), F: T.Buffer(3, "int64")):
        for i in T.serial(24):
            T.evaluate(T.assume(((3 <= i) and (i < 17)) or A[i] == 0))
