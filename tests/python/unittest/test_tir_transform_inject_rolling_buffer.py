...
class PreRollingBuffer:
    def main(A: ty.handle, tensor: ty.handle) -> None:
        # function attr dict
        tir.func_attr({"global_symbol": "main", "tir.noalias": True})
        # buffer definition
        tensor_2 = tir.buffer_decl([1, 10, 12, 16], dtype="int8", elem_offset=0, align=128, offset_factor=1)
        A_1 = tir.match_buffer(A, [1, 12, 14, 16], dtype="int8", elem_offset=0, align=128, offset_factor=1)
        tensor_1 = tir.match_buffer(tensor, [1, 8, 8, 16], dtype="int8", elem_offset=0, align=128, offset_factor=1)
        # body
        ...
        for ax1_outer in tir.serial(0, 2):
            ...
            for ax1 in tir.serial(0, 6):
                for ax2 in tir.serial(0, 12):
                    for ax3 in tir.serial(0, 16):
                        tensor_2[0, (ax1 + (ax1_outer*4)), ax2, ax3] = tir.int8(0)
                        for dh in tir.serial(0, 3):
                            for dw in tir.serial(0, 3):
                                tensor_2[0, (ax1 + (ax1_outer*4)), ax2, ax3] = tir.max(tensor_2[0, (ax1 + (ax1_outer*4)), ax2, ax3], A_1[0, ((ax1 + (ax1_outer*4)) + dh), (ax2 + dw), ax3])
            for ax1_inner in tir.serial(0, 4):
                for ax2_inner in tir.serial(0, 8):
                    for ax3_inner in tir.serial(0, 16):
                        tensor_1[0, (ax1_inner + (ax1_outer*4)), ax2_inner, ax3_inner] = tir.int8(0)
                        for dh_1 in tir.serial(0, 3):
                            for dw_1 in tir.serial(0, 5):
                                tensor_1[0, (ax1_inner + (ax1_outer*4)), ax2_inner, ax3_inner] = tir.max(tensor_1[0, (ax1_inner + (ax1_outer*4)), ax2_inner, ax3_inner], tensor_2[0, ((ax1_inner + (ax1_outer*4)) + dh_1), (ax2_inner + dw_1), ax3_inner])
    __tvm_meta__ = None
...
class PostRollingBuffer:
    def main(A: ty.handle, tensor: ty.handle) -> None:
        ...
        # body
        tir.realize(tensor_1[0:1, 0:8, 0:8, 0:16], "")
        tir.realize(tensor_2[0:1, 0:6, 0:12, 0:16], "")
        for ax1_outer in tir.serial(0, 2):
            for ax1 in tir.serial(0, 6):
                for ax2 in tir.serial(0, 12):
                    for ax3 in tir.serial(0, 16):
                        if ((ax1_outer < 1) or (ax1 >= 2)):
                            tensor_2[0, tir.floormod((ax1 + (ax1_outer*4)), 6), ax2, ax3] = tir.int8(0)
                        for dh in tir.serial(0, 3):
                            for dw in tir.serial(0, 3):
                                if ((ax1_outer < 1) or (ax1 >= 2)):
                                    tensor_2[0, tir.floormod((ax1 + (ax1_outer*4)), 6), ax2, ax3] = tir.max(tensor_2[0, tir.floormod((ax1 + (ax1_outer*4)), 6), ax2, ax3], A_1[0, ((ax1 + (ax1_outer*4)) + dh), (ax2 + dw), ax3])
            for ax1_inner in tir.serial(0, 4):
                for ax2_inner in tir.serial(0, 8):
                    for ax3_inner in tir.serial(0, 16):
                        tensor_1[0, (ax1_inner + (ax1_outer*4)), ax2_inner, ax3_inner] = tir.int8(0)
                        for dh_1 in tir.serial(0, 3):
                            for dw_1 in tir.serial(0, 5):
                                tensor_1[0, (ax1_inner + (ax1_outer*4)), ax2_inner, ax3_inner] = tir.max(tensor_1[0, (ax1_inner + (ax1_outer*4)), ax2_inner, ax3_inner], tensor_2[0, tir.floormod(((ax1_inner + (ax1_outer*4)) + dh_1), 6), (ax2_inner + dw_1), ax3_inner])
    __tvm_meta__ = None
# fmt: on
