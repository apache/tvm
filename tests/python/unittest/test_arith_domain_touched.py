...
def test_domain_touched():
    ...
    a_domain_r = tvm.arith.DomainTouched(ir, a.op, True, False)
    assert str(a_domain_r) == "[range(min=-1, ext=100),range(min=-1, ext=m)]"
    a_domain_w = tvm.arith.DomainTouched(ir, a.op, False, True)
    assert str(a_domain_w) == "[range(min=0, ext=100),range(min=0, ext=m)]"
    a_domain_rw= tvm.arith.DomainTouched(ir, a.op, True, True)
    assert str(a_domain_rw) == "[range(min=-1, ext=101),range(min=-1, ext=(m + 1))]"
    b_domain_r = tvm.arith.DomainTouched(ir, b.op, True, False)
    assert str(b_domain_r) == "[range(min=-1, ext=100),range(min=1, ext=m)]"
    b_domain_w = tvm.arith.DomainTouched(ir, b.op, False, True)
