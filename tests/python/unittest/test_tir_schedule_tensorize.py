...
def test_tensorize_matmul():
    ...
    assert_structural_equal_gs(tensorized_matmul, s.mod["main"])
...
def test_tensorize_batch_matmul():
    ...
    s.reorder(io, jo, ko, ii, ji, ki)
    s.tensorize(ii, "test_mma_intrin")
    assert_structural_equal_gs(tensorized_batch_matmul_mma, s.mod["main"])
...
def test_tensorize_dot_product():
    ...
    assert_structural_equal_gs(tensorized_batch_matmul_dot_product, s.mod["main"])
    verify_trace_roundtrip(sch=s, mod=func)
...
def test_tensorize_outer_product():
    ...
    assert_structural_equal_gs(tensorized_batch_matmul_outer_product, s.mod["main"])
    verify_trace_roundtrip(sch=s, mod=func)
...
def test_tensorize_with_annotation():
    ...
    assert_structural_equal_gs(annotated_tensorized_matmul, s.mod["main"])
    verify_trace_roundtrip(sch=s, mod=func)
...
def test_tensorize_matmul_mixed_dtype():
    ...
    s.tensorize(ii, "test_mma_intrin")
    assert_structural_equal_gs(s.mod["main"], tensorized_matmul_int64_shape)
