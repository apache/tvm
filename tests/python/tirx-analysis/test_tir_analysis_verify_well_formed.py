...
@pytest.mark.parametrize("legacy_s_tir", [False, True])
def test_mixed_module_parser_checks_both_dialects(legacy_s_tir):
    @I.ir_module
    class Mixed:
        @T.prim_func(s_tir=True)
    ...
    restored = tvm.script.from_source(Mixed.script(), s_tir=legacy_s_tir)
    tvm.ir.assert_structural_equal(restored, Mixed)
    assert tvm.s_tir.analysis.verify_well_formed(restored)
