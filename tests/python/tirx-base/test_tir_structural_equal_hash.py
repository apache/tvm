...
def test_exprs():
    ...
    # Defintition remap
    assert consistent_equal(tvm.tirx.Let(vx, 1, vx - 1), tvm.tirx.Let(vy, 1, vy - 1))
    # Default same address free var remap
    assert consistent_equal(tvm.tirx.Let(vx, 1, vx // vz), tvm.tirx.Let(vy, 1, vy // vz))
...
def test_buffer_storage_scope():
    ...
    func0 = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_local_0})
    func1 = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_local_1})
    func2 = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_global})
    func3 = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_empty})
...
def test_buffer_map_mismatch():
    ...
    func_0 = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_0})
    func_0_clone = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_0_clone})
    func_1 = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_1})
...
def test_buffer_map_length_mismatch():
    ...
    func_0 = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_0})
    func_1 = tvm.tirx.PrimFunc([x], tvm.tirx.Evaluate(x), buffer_map={x: buffer_0, y: buffer_1})
