...
def clml_preprocessing(mod):
    for _var in mod.get_global_vars():
        ...
        if("Compiler" in new_fn.attrs.keys() and new_fn.attrs["Compiler"] != "clml"):
            ...
            mod[_var.name_hint] = _function.Function(list(new_params),
                                                     new_body,
                                                     fn.ret_type,
                                                     fn.type_params,
                                                     fn.attrs)
...
def opencl_cost_estimator(mod, target):
    ...
    cost = vm_factory.benchmark(ctx, repeat=5, number=20, min_repeat_ms=0, func_name=func_name, **main_args)
    return cost.mean
    ...
    with optional_tuning_records(TUNING_LOG):
        ...
        with ctxt:
            ...
            mod = tvm.relay.transform.CollagePartition(
                config, cost_estimator=cost_estimator
            )(mod)
