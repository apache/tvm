...
def test_grid_sample():
    def verify_grid_sample(data_shape, grid_shape, method="bilinear", padding_mode="zeros", align_corners=True):
        dtype = "float32"
        data = relay.var("data", relay.ty.TensorType(data_shape, dtype))
        ...
        if len(data_shape) == 4:
            ...
            tensor_type = relay.TensorType((batch, channel, out_depth, out_height, out_width), dtype)

        y = relay.image.grid_sample(
            data, grid, method=method, layout=layout, padding_mode=padding_mode, align_corners=align_corners
        )
        yy = run_infer_type(y)
        ...
        ref_res = tvm.topi.testing.grid_sample_python(
            data_np, grid_np, method=method, padding_mode=padding_mode, align_corners=align_corners
        )
    ...
    data_2D_shape = (4, 4, 16, 32)
    grid_2D_shape = (4, 8, 8, 2)
    data_3D_shape = (4, 4, 4, 16, 32)
    grid_3D_shape = (4, 4, 8, 8, 2)
    ...
    for _method in methods:
        for _padding in padding_modes:
            for _align in align_corners:
                ...
                # 3D "bicubic"(tricubic) is not surpported in pytorch
                if _method != "bicubic":
