...
def test_resnet_pool_uses_specified_convert_layout():
    relay.op.get("nn.global_max_pool2d").reset_attr("FTVMConvertOpLayout")

    @tvm.ir.register_op_attr("nn.global_max_pool2d", "FTVMConvertOpLayout")
    def convert_global_maxpool2d(attrs, inputs, tinfos, desired_layouts):
        # stick by convertng layout and out_layout to use NHWC and NHWC,
        #   respectively, as specified in the transforms.ConvertLayout() function's arguments later
        new_attrs = dict(attrs)
        new_attrs["layout"] = str(desired_layouts[0])
