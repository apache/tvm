...
def test_dyn_upsampling3d_run():
    def verify_upsampling3d(dshape, scale_h, scale_w, layout, method, align_corners=False):
        ...
        if layout == "NCDHW":
            ...
            (n, h, w, c, d) = dshape
            x_data = np.random.uniform(size=(n, h, w, c, d)).astype("float32")
    ...
    verify_upsampling3d((1, 16, 32, 32), 2.0, 2.0,"NCHW", "nearest_neighbor")
    verify_upsampling3d((1, 16, 32, 32), 2.0, 2.0, "NCHW", "bilinear", True)
    verify_upsampling3d((1, 16, 32, 32), 2.0, 2.0, "NHWC", "nearest_neighbor")
    verify_upsampling3d((1, 16, 32, 32), 2.0, 2.0,"NHWC", "bilinear", True)
"""
#tests upsampling type inference with scale_h passed in as a constant and scale_w as a variable
def test_dyn_upsampling3d_infer_type_const():
    ...
    z = relay.nn.upsampling3d(data, scale_d, 2.0, scale_w)
    zz = run_infer_type(z)
