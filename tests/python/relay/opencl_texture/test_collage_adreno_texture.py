...
def compile_and_benchmark(label, model, targets, tmp_dir):
    ...
    m.run()
    time_f = m.module.time_evaluator("run", ctx, repeat=MEASURE_REPEAT, number=MEASURE_NUMBER)
    profile = time_f()
...
def collage(model):
    ...
    with optional_tuning_records(TUNING_LOG):
        ...
        # options [compiler.NoFusion | compiler.TVMFusion | compiler.ArbitraryFusion]
        config = {
        ...
        ctxt = tvm.transform.PassContext(config=config, opt_level=3)
        ...
        with ctxt:
            ...
            return compile_and_benchmark("collage", partitioned_model, OPENCL, tmp_dir)
...
def just_clml(model):
    ...
    with tvm.transform.PassContext(opt_level=3):
        ...
        logging.info(partitioned_model["mod"])
        logging.info("-------------- END PARTITIONED ----------------")
        return compile_and_benchmark("just_clml", partitioned_model, OPENCL, tmp_dir)
...
def just_tvm(model):
    ...
    with optional_tuning_records(TUNING_LOG):
        with tvm.transform.PassContext(opt_level=3):
            return compile_and_benchmark("just_tvm", model, OPENCL, tmp_dir)
