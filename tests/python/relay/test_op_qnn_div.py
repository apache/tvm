...
def generate_golden_output(x_recovered, y_recovered, scale, zp):
    div = x_recovered / y_recovered
    output = np.around(div / scale + zp)
...
def test_tflite_same_io_qnn_params():
    ...
    x_datas = [
        np.array((200, 201, 202, 203)).reshape((1, 4)),
        np.array((55, 56, 57, 58)).reshape((1, 4)),
        np.array((25, 153, 1, 165)).reshape((1, 4)),
    ]
    y_datas = [
        np.array((1, 1, 1, 1)).reshape((1, 4)),
        np.array((10, 12, 3, 3)).reshape((1, 4)),
        np.array((45, 99, 1, 200)).reshape((1, 4)),
    ]
...
def test_saturation():
    # Same params
    data_dtype = "uint8"
    lhs_scale = rhs_scale = output_scale = 125
    ...
    y_data = np.array((255, 255, 128, 1)).reshape((1, 4))
    ...
    x_data = np.array((255, 1, 1, 0)).reshape((1, 4))
    y_data = np.array((255, 255, 127, 1)).reshape((1, 4))
    ...
    y_data = np.array((255, 128, 64, 1)).reshape((1, 4))
