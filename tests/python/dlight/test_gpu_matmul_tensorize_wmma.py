...
def test1():
    ...
    def expected(X: T.Buffer((256, 256), "float16"), W: T.Buffer((256, 256), "float16"), compute: T.Buffer((256, 256), "float16")):
        ...
        X_reindex_shared_dyn = T.alloc_buffer((2, 8, 8, 2, 16, 16), "float16", scope="shared.dyn")
        X_reindex_shared_dyn_wmma_matrix_a = T.alloc_buffer((2, 8, 8, 2, 16, 16), "float16", scope="wmma.matrix_a")
        W_reindex_shared_dyn = T.alloc_buffer((2, 8, 8, 2, 16, 16), "float16", scope="shared.dyn")
        W_reindex_shared_dyn_wmma_matrix_b = T.alloc_buffer((2, 8, 8, 2, 16, 16), "float16", scope="wmma.matrix_b")
        compute_reindex_shared_dyn = T.alloc_buffer((2, 2, 8, 8, 16, 16), "float16", scope="shared.dyn")
        compute_reindex_shared_dyn_wmma_accumulator = T.alloc_buffer((2, 2, 8, 8, 16, 16), "float16", scope="wmma.accumulator")
        for ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused in T.thread_binding(4, thread="blockIdx.x"):
            for ax1_0_2_init in T.thread_binding(2, thread="threadIdx.z"):
                for ax2_0_2_init in T.thread_binding(2, thread="threadIdx.y"):
                    for ax1_0_3_init, ax2_0_3_init in T.grid(4, 4):
                        with T.block("compute_o_init"):
                            v0_o = T.axis.spatial(1, 0)
                            v1_o = T.axis.spatial(16, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused // 2 * 8 + ax1_0_2_init * 4 + ax1_0_3_init)
                            v2_o = T.axis.spatial(16, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused % 2 * 8 + ax2_0_2_init * 4 + ax2_0_3_init)
                            T.reads()
                            T.writes(compute_reindex_shared_dyn_wmma_accumulator[v1_o // 8, v2_o // 8, v1_o % 8, v2_o % 8, 0:16, 0:16])
                            with T.block("compute_init_o"):
                                v1_i_init_o = T.axis.spatial(1, 0)
                                v2_i_init_o = T.axis.spatial(1, 0)
                                T.reads()
                                T.writes(compute_reindex_shared_dyn_wmma_accumulator[v1_o // 8, v2_o // 8, v1_o % 8, v2_o % 8, 0:16, 0:16])
                                C = T.match_buffer(compute_reindex_shared_dyn_wmma_accumulator[v1_o // 8, v2_o // 8, v1_o % 8, v2_o % 8, 0:16, 0:16], (16, 16), "float16", strides=("C_s0", "C_s1"), scope="wmma.accumulator", offset_factor=16)
                                T.tvm_fill_fragment(C.data, 16, 16, 16, C.elem_offset // C.strides[0] // 16 * (C.strides[0] // 16) + C.elem_offset % C.strides[0] // 16, T.float32(0))
            for ax3_0_0 in T.unroll(8):
                            ...
                            for ax0_ax1_fused_3 in T.thread_binding(32, thread="threadIdx.x"):
                                for ax0_ax1_fused_4 in T.vectorized(8):
                                    with T.block("X_reindex_shared.dyn"):
                                        ...
                                        v1 = T.axis.spatial(256, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused // 2 * 128 + (ax0_ax1_fused_0 * 1024 + ax0_ax1_fused_1 * 512 + ax0_ax1_fused_2 * 256 + ax0_ax1_fused_3 * 8 + ax0_ax1_fused_4) // 32)
                                        v2 = T.axis.spatial(256, ax3_0_0 * 32 + (ax0_ax1_fused_0 * 1024 + ax0_ax1_fused_1 * 512 + ax0_ax1_fused_2 * 256 + ax0_ax1_fused_3 * 8 + ax0_ax1_fused_4) % 32)
                                        T.reads(X[v1, v2])
                                        T.writes(X_reindex_shared_dyn[v1 // 128, v2 // 32, v1 % 128 // 16, v2 % 32 // 16, v1 % 16, v2 % 16])
                                        X_reindex_shared_dyn[v1 // 128, v2 // 32, v1 % 128 // 16, v2 % 32 // 16, v1 % 16, v2 % 16] = X[v1, v2]
                for ax0_ax1_fused_0 in range(4):
                    for ax0_ax1_fused_1 in T.thread_binding(2, thread="threadIdx.z"):
                        for ax0_ax1_fused_2 in T.thread_binding(2, thread="threadIdx.y"):
                            for ax0_ax1_fused_3 in T.thread_binding(32, thread="threadIdx.x"):
                                for ax0_ax1_fused_4 in T.vectorized(8):
                                    with T.block("W_reindex_shared.dyn"):
                                        ...
                                        v2 = T.axis.spatial(256, ax3_0_0 * 32 + (ax0_ax1_fused_0 * 1024 + ax0_ax1_fused_1 * 512 + ax0_ax1_fused_2 * 256 + ax0_ax1_fused_3 * 8 + ax0_ax1_fused_4) % 32)
                                        T.reads(W[v1, v2])
                                        T.writes(W_reindex_shared_dyn[v1 // 128, v2 // 32, v1 % 128 // 16, v2 % 32 // 16, v1 % 16, v2 % 16])
                                        W_reindex_shared_dyn[v1 // 128, v2 // 32, v1 % 128 // 16, v2 % 32 // 16, v1 % 16, v2 % 16] = W[v1, v2]
                for ax1_0_2 in T.thread_binding(2, thread="threadIdx.z"):
                    for ax2_0_2 in T.thread_binding(2, thread="threadIdx.y"):
                        for ax3_0_1 in range(2):
                            for ax0 in range(4):
                                with T.block("X_reindex_shared.dyn_wmma.matrix_a_o"):
                                    v0_o = T.axis.spatial(2, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused // 2)
                                    v1_o = T.axis.spatial(8, ax3_0_0)
                                    v2_o = T.axis.spatial(8, ax1_0_2 * 4 + ax0)
                                    v3_o = T.axis.spatial(2, ax3_0_1)
                                    v4_o = T.axis.spatial(1, 0)
                                    v5_o = T.axis.spatial(1, 0)
                                    T.reads(X_reindex_shared_dyn[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16])
                                    T.writes(X_reindex_shared_dyn_wmma_matrix_a[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16])
                                    A = T.match_buffer(X_reindex_shared_dyn[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16], (16, 16), "float16", strides=("A_s0", "A_s1"), scope="shared.dyn", offset_factor=16)
                                    C = T.match_buffer(X_reindex_shared_dyn_wmma_matrix_a[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16], (16, 16), "float16", strides=("C_s0", "C_s1"), scope="wmma.matrix_a", offset_factor=16)
                            ...
                            for ax0 in range(4):
                                with T.block("W_reindex_shared.dyn_wmma.matrix_b_o"):
                                    v0_o = T.axis.spatial(2, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused % 2)
                                    v1_o = T.axis.spatial(8, ax3_0_0)
                                    v2_o = T.axis.spatial(8, ax2_0_2 * 4 + ax0)
                                    v3_o = T.axis.spatial(2, ax3_0_1)
                                    v4_o = T.axis.spatial(1, 0)
                                    v5_o = T.axis.spatial(1, 0)
                                    T.reads(W_reindex_shared_dyn[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16])
                                    T.writes(W_reindex_shared_dyn_wmma_matrix_b[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16])
                                    A = T.match_buffer(W_reindex_shared_dyn[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16], (16, 16), "float16", strides=("A_s0", "A_s1"), scope="shared.dyn", offset_factor=16)
                                    C = T.match_buffer(W_reindex_shared_dyn_wmma_matrix_b[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16], (16, 16), "float16", strides=("C_s0", "C_s1"), scope="wmma.matrix_b", offset_factor=16)
                            ...
                            for ax1_0_3, ax2_0_3 in T.grid(4, 4):
                                with T.block("compute_o_update"):
                                    v0_o = T.axis.spatial(1, 0)
                                    v1_o = T.axis.spatial(16, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused // 2 * 8 + ax1_0_2 * 4 + ax1_0_3)
                                    v2_o = T.axis.spatial(16, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused % 2 * 8 + ax2_0_2 * 4 + ax2_0_3)
                                    v3_o = T.axis.reduce(16, ax3_0_0 * 2 + ax3_0_1)
                                    T.reads(compute_reindex_shared_dyn_wmma_accumulator[v1_o // 8, v2_o // 8, v1_o % 8, v2_o % 8, 0:16, 0:16], X_reindex_shared_dyn_wmma_matrix_a[v1_o * 16 // 128, v3_o * 16 // 32, (v1_o * 16 - 128 * (v1_o * 16 // 128)) // 16, (v3_o * 16 - 32 * (v3_o * 16 // 32)) // 16, 0:16, 0:16], W_reindex_shared_dyn_wmma_matrix_b[v2_o * 16 // 128, v3_o * 16 // 32, (v2_o * 16 - 128 * (v2_o * 16 // 128)) // 16, (v3_o * 16 - 32 * (v3_o * 16 // 32)) // 16, 0:16, 0:16])
                                    T.writes(compute_reindex_shared_dyn_wmma_accumulator[v1_o // 8, v2_o // 8, v1_o % 8, v2_o % 8, 0:16, 0:16])
                                    with T.block("compute_o"):
                                        v1_i_o = T.axis.spatial(1, 0)
                                        v2_i_o = T.axis.spatial(1, 0)
                                        v3_i_o = T.axis.reduce(1, 0)
                                        T.reads(compute_reindex_shared_dyn_wmma_accumulator[v1_o // 8, v2_o // 8, v1_o % 8, v2_o % 8, 0:16, 0:16], X_reindex_shared_dyn_wmma_matrix_a[v1_o // 8, v3_o // 2, v1_o % 8, v3_o % 2, 0:16, 0:16], W_reindex_shared_dyn_wmma_matrix_b[v2_o // 8, v3_o // 2, v2_o % 8, v3_o % 2, 0:16, 0:16])
                                        T.writes(compute_reindex_shared_dyn_wmma_accumulator[v1_o // 8, v2_o // 8, v1_o % 8, v2_o % 8, 0:16, 0:16])
                                        A = T.match_buffer(X_reindex_shared_dyn_wmma_matrix_a[v1_o // 8, v3_o // 2, v1_o % 8, v3_o % 2, 0:16, 0:16], (16, 16), "float16", strides=("A_s0", "A_s1"), scope="wmma.matrix_a", offset_factor=16)
                                        B = T.match_buffer(W_reindex_shared_dyn_wmma_matrix_b[v2_o // 8, v3_o // 2, v2_o % 8, v3_o % 2, 0:16, 0:16], (16, 16), "float16", strides=("B_s0", "B_s1"), scope="wmma.matrix_b", offset_factor=16)
                                        C = T.match_buffer(compute_reindex_shared_dyn_wmma_accumulator[v1_o // 8, v2_o // 8, v1_o % 8, v2_o % 8, 0:16, 0:16], (16, 16), "float16", strides=("C_s0", "C_s1"), scope="wmma.accumulator", offset_factor=16)
            ...
            for ax0, ax1 in T.grid(1, 1):
                for ax2_0 in T.thread_binding(2, thread="threadIdx.z"):
                    for ax3_0 in T.thread_binding(2, thread="threadIdx.y"):
                        for ax2_1, ax3_1 in T.grid(4, 4):
                            with T.block("compute_reindex_shared.dyn_wmma.accumulator_o"):
                                v0_o = T.axis.spatial(2, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused // 2 + ax0)
                                v1_o = T.axis.spatial(2, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused % 2 + ax1)
                                v2_o = T.axis.spatial(8, ax2_0 * 4 + ax2_1)
                                v3_o = T.axis.spatial(8, ax3_0 * 4 + ax3_1)
                                v4_o = T.axis.spatial(1, 0)
                                v5_o = T.axis.spatial(1, 0)
                                T.reads(compute_reindex_shared_dyn_wmma_accumulator[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16])
                                T.writes(compute_reindex_shared_dyn[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16])
                                A = T.match_buffer(compute_reindex_shared_dyn_wmma_accumulator[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16], (16, 16), "float16", strides=("A_s0", "A_s1"), scope="wmma.accumulator", offset_factor=16)
                                C = T.match_buffer(compute_reindex_shared_dyn[v0_o, v1_o, v2_o, v3_o, 0:16, 0:16], (16, 16), "float16", strides=("C_s0", "C_s1"), scope="shared.dyn", offset_factor=16)
                        ...
                        for ax0_ax1_fused_3 in T.thread_binding(32, thread="threadIdx.x"):
                            for ax0_ax1_fused_4 in T.vectorized(8):
                                with T.block("compute_reindex_shared.dyn"):
                                    v0 = T.axis.spatial(1, 0)
                                    v1 = T.axis.spatial(256, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused // 2 * 128 + (ax0_ax1_fused_0 * 1024 + ax0_ax1_fused_1 * 512 + ax0_ax1_fused_2 * 256 + ax0_ax1_fused_3 * 8 + ax0_ax1_fused_4) // 128)
                                    v2 = T.axis.spatial(256, ax0_ax1_0_0_ax2_0_0_ax1_0_1_ax2_0_1_fused % 2 * 128 + (ax0_ax1_fused_0 * 1024 + ax0_ax1_fused_1 * 512 + ax0_ax1_fused_2 * 256 + ax0_ax1_fused_3 * 8 + ax0_ax1_fused_4) % 128)
                                    T.reads(compute_reindex_shared_dyn[v1 // 128, v2 // 128, v1 % 128 // 16, v2 % 128 // 16, v1 % 16, v2 % 16])
                                    T.writes(compute[v1, v2])
                                    compute[v1, v2] = compute_reindex_shared_dyn[v1 // 128, v2 // 128, v1 % 128 // 16, v2 % 128 // 16, v1 % 16, v2 % 16]
    # fmt: on
    before_mod = tvm.IRModule.from_expr(before.without_attr("global_symbol"))
    with Target("nvidia/geforce-rtx-2080-ti"):
        after_mod = dl.ApplyDefaultSchedule(dl.gpu.Matmul())(before_mod)
    after_mod.show()
