...
def verify_conv2d_nhwc(batch, in_channel, in_size, num_filter, kernel, stride,
    ...
    # w_shape = (kernel, kernel, num_filter, in_channel)
    ...
    def get_ref_data():
        if dtype == 'float16':
            ...
            a_np = np.random.randint(low=-7, high=7, size=a_shape).astype(np.int32)
            b_np = np.random.randint(low=-7, high=7, size=bias_shape).astype(np.int32)
            w_np = np.random.randint(low=-7, high=7, size=w_shape).astype(np.int32)
            dw_np = topi.testing.dilate_python(w_np, (1, 1, dilation, dilation))
        elif dtype == 'int8':
    ...
    if dtype == 'int4':
        ...
        w_np_tvm = w_np.reshape((kernel,
                    kernel,
                    in_channel // wmma_k,
                    wmma_k,
                    num_filter // wmma_n,
                    wmma_n)).transpose((0,1,2,4,5,3))
        a_np = convert_int32_into_int4(a_np_tvm)
        # b_np = convert_int32_into_int4(b_np)
        w_np = convert_int32_into_int4(w_np_tvm)
    ...
    def check_device(device):
        ...
        if add_bias:
            ...
            func.time_evaluator(func.entry_name, ctx, number=50, repeat=20)(a, w, c)
            # evaluator(a, w, c)
            print('Time cost of this operator: %f ms' % (evaluator(a, w, c).mean * 1000))

        rtol = 1e-3
        # print(c.asnumpy().shape, c_np.sum())
        tvm.testing.assert_allclose(c.asnumpy().transpose((0,4,1,2,3,5)).reshape(c_np.shape), c_np, rtol=rtol)
        ...
        # log_filename = "conv2d_int4_nhwc_tensorcore.log"
