...
def verify_indicator(shape, dtype):
    ...
    def check_device(device):
        ...
        ctx = tvm.context(device, 0)
        out = tvm.nd.array(np.zeros(shape, dtype="float32"), ctx)
        tvm_l = tvm.nd.array(np_l, ctx)
        tvm_r = tvm.nd.array(np_r, ctx)

        f = tvm.build(s_less, [A, B, C], device, name="less")
        f(tvm_l, tvm_r, out)
        np.testing.assert_allclose(out.asnumpy(), np.less(np_l, np_r).astype("float32"), rtol=1e-5)

        f = tvm.build(s_greater, [D, E, F], device, name="greater")
        f(tvm_l, tvm_r, out)
        np.testing.assert_allclose(out.asnumpy(), np.greater(np_l, np_r).astype("float32"), rtol=1e-5)
