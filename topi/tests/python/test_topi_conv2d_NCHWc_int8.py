...
def verify_conv2d_NCHWc_int8(batch, in_channel, in_size, num_filter, kernel, stride, padding, dilation=1):
    in_height = in_width = in_size

    A = tvm.placeholder((batch, in_height, in_width, in_channel), name='A', dtype='uint8')
    W = tvm.placeholder((kernel, kernel, in_channel, num_filter, kernel, kernel), name='W', dtype='int8')
...
def test_conv2d_NCHWc():
    ...
    with DefaultFallback():
        verify_conv2d_NCHWc_int8(1, 256, 32, 256, 7, 7, 0)
