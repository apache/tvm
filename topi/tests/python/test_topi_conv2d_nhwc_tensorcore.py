...
def verify_conv2d_nhwc(batch, in_channel, in_size, num_filter, kernel, stride,
    ...
    def get_ref_data():
        if dtype == 'float16':
            ...
            a_np = np.random.randint(low=1, high=7, size=a_shape)
            b_np = np.random.randint(low=1, high=7, size=bias_shape)
            w_np = np.random.randint(low=1, high=7, size=w_shape)
    ...
    def check_device(device):
        ...
        #                padding, dilation)
        # task = autotvm.create('conv2d_nhwc_tensorcore.cuda', args=[A, W, stride, padding, dilation, dtype, out_dtype], target=device)
