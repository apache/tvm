...
from cuda_depthconv_bn_relu_fusion import depthconv_bn_relu_fusion
from scipy import signal
import time

def test_depthconv_bn_relu_fusion():
    ...
    BNparams = tvm.placeholder((in_channel * channel_multiplier, 4), name='BNparams')
    Stride = tvm.nd.array(np.array([stride_h, stride_w]))
    
    schedule, Output = depthconv_bn_relu_fusion(Input, Filter, Stride, padding, BNparams)
    ...
    def check_device(device):
        ...
        bn_params_np = np.vstack((data_mean, data_var, gamma, beta)).T
        ...
        conv_scipy = np.zeros((out_batch, out_channel, out_height, out_width), dtype=Output.dtype)
        relu_scipy = np.zeros((out_batch, out_channel, out_height, out_width), dtype=Output.dtype)
        if padding == 'SAME':
            ...
            for i in range(out_batch):
                for j in range(out_channel):
                    conv_scipy[i,j,:,:] = signal.convolve2d(input_np[i, j/channel_multiplier,:,:], np.rot90(filter_np[j/channel_multiplier,j%channel_multiplier,:,:], 2), 
                        mode='same')[index_h:in_height:stride_h, index_w:in_width:stride_w] 
                    relu_scipy[i,j,:,:] = np.maximum((conv_scipy[i,j,:,:]-bn_params_np[j,0])/(np.sqrt(bn_params_np[j,1]+1e-5))*bn_params_np[j,2]+bn_params_np[j,3], 0) 
        if padding == 'VALID':
            for i in range(out_batch):
                for j in range(out_channel):
                    conv_scipy[i,j,:,:] = signal.convolve2d(input_np[i, j/channel_multiplier,:,:], np.rot90(filter_np[j/channel_multiplier,j%channel_multiplier,:,:], 2), 
                    ...
                    relu_scipy[i,j,:,:] = np.maximum((conv_scipy[i,j,:,:]-bn_params_np[j,0])/(np.sqrt(bn_params_np[j,1]+1e-5))*bn_params_np[j,2]+bn_params_np[j,3], 0)  
        np.testing.assert_allclose(output_tvm.asnumpy(), relu_scipy, atol=1e-5, rtol=1e-5)
        print "success"
