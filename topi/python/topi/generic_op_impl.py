...
def _tensor_bop_impl(lhs, rhs, name, elementwise_bop, broadcast_bop, orig_bop):
    ...
    orig_bop: function
        Binary operator before overloading, for rule (3).
...
def _make_bop(name, elementwise_bop, broadcast_bop, orig_bop):
    """Wrapper function to make a specific overloaded operator."""
    return lambda lhs, rhs: _tensor_bop_impl(lhs,
                                             rhs,
                                             name,
                                             elementwise_bop,
                                             broadcast_bop,
...
__op_priority__ = tvm.expr.__op_priority__ + 1
...
def _bind_generic_ops():
    ...
    if __op_priority__ > tvm.generic.__op_priority__:
        ...
        tvm.generic.add = _make_bop("add",
                                    lambda x, y: x + y,
                                    _broadcast.broadcast_add,
                                    tvm.generic.add)
        tvm.generic.subtract = _make_bop("subtract",
                                         lambda x, y: x - y,
                                         _broadcast.broadcast_sub,
                                         tvm.generic.subtract)
        tvm.generic.multiply = _make_bop("multiply",
                                         lambda x, y: x * y,
                                         _broadcast.broadcast_mul,
                                         tvm.generic.multiply)
        tvm.generic.divide = _make_bop("divide",
                                       lambda x, y: x / y,
