...
def batch_norm(input, moving_mean, moving_var, eps):
    """Batch normalization inference operator in NCHW layout.
    ...
    assert len(input.shape) == 4, "only support 4-dim batch norm"

    return tvm.compute(input.shape, lambda b, c, h, w: (input[b, c, h, w] - moving_mean[c]) / \
