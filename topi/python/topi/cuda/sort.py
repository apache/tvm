...
from tvm import api
from topi.sort import argsort, topk
...
def topk_ir(data, axis, is_ascend):
    """Low level IR to do nms sorting on the GPU, same usage as tvm.contrib.sort.argsort on the CPU.
...
def topk_gpu(data, k=1, axis=-1, ret_type="both", is_ascend=False, dtype="int64"):
    ...
    out : tvm.Tensor
        The output of this function.
    """
    assert ret_type == "value"
    values_buf = api.decl_buffer(data.shape, data.dtype, "values_buf", data_alignment=8)
    values_out = identity(data)
    out_buf = api.decl_buffer(data.shape, dtype, "out_buf", data_alignment=8)
    tvm.extern([data.shape],
                             [values_out],
                             lambda ins, outs: topk_ir(
                                 ins[0], axis, is_ascend),
                             in_buffers=[values_buf],
                             out_buffers=[],
                             name="topk_gpu",
                             tag="topk_gpu")
    # if k < 1:
    #     if ret_type == "both":
    #         return [sorted_data, indices_out]
    #     if ret_type == "value":
    #         return
    return values_out
