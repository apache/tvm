...
def sort_pre_ir(data, index, sizes_out, axis, axis_mul_before, axis_mul_after):
...
def sort_pre_ir_data(data, index, sizes_in, data_out, index_out, axis, axis_mul_before, axis_mul_after):
    ib = tvm.ir_builder.create()
...
def sort_oet_ir(data, index, new_data, new_index, loc, out_index, axis_mul_before, axis_mul_after, axis, is_descend):
    max_threads = int(tvm.target.current_target(allow_none=False).max_num_threads)
    tx = tvm.thread_axis("threadIdx.x")
    bx = tvm.thread_axis("blockIdx.x")
    ib = tvm.ir_builder.create()
        ...
        with ib.for_range(0, fshape, name = "k", for_type="unroll") as k:
            with ib.if_scope(tvm.all(k % 2 == tid % 2, tid < fshape)):
                with ib.if_scope(k % 2 == 0):
                    with ib.if_scope(tvm.all(tid + 1 < fshape, (p_data[tid] < p_data[tid+1]) == is_descend)):
                ...
                with ib.else_scope():
                    with ib.if_scope(tvm.all(tid + 1 < fshape, (data_new[tid] < data_new[tid+1]) == is_descend)):
            ...
            with ib.if_scope(tvm.all(k % 2 != tid % 2, tid < fshape)):
                with ib.if_scope(k % 2 == 0):
                    ...
                    with ib.else_scope():
                        ...
                        index_out[tid] = index_new[tid]
                with ib.else_scope():
                    with ib.if_scope(tvm.all(tid > 0, (data_new[tid-1] < data_new[tid]) == is_descend)):
                        p_data[tid] = data_new[tid-1]
                        index_new[tid] = index_out[tid-1]
                    with ib.else_scope():
                        p_data[tid] = data_new[tid]
    ...
    with ib.if_scope(axis_mul_before * axis_mul_after > 1):
        with ib.if_scope(tid < axis_mul_before * axis_mul_after):
            ...
            with ib.for_range(0, data.shape[axis], name = "k") as k:
                ...
                with ib.else_scope():
                    start = sizes[tid-1]
                p_out[base_idx + k * axis_mul_after] = tvm.select(k < p_index[tid], index_new[k+start], k)
    with ib.else_scope():
...
def sort_gpu(data, data_buf, index, index_buf, output_buf, axis, is_descend):
    ...
    new_data_buf = api.decl_buffer(dshape, data.dtype, "data_new", data_alignment = 8)

    loc = \
        tvm.extern([(dshape,)],
                   [data, index],
                   lambda ins, outs: sort_pre_ir(
                       ins[0], ins[1], outs[0], axis, axis_mul_before, axis_mul_after),
                   dtype=[index.dtype],
    ...
    data_new, index_new = \
        tvm.extern([(dshape,), (fshape,)],
                   ...
                   lambda ins, outs: sort_pre_ir_data(
                       ins[0], ins[1], ins[2], outs[0], outs[1], axis, axis_mul_before, axis_mul_after),
    ...
    index_out = \
        tvm.extern([(fshape,)],
                   ...
                   lambda ins, outs: sort_oet_ir(
                       ins[0], ins[1], ins[2], ins[3], ins[4], outs[0], axis_mul_before, axis_mul_after, axis, is_descend),
                   dtype=[index.dtype],
                   in_buffers=[data_buf, index_buf, new_data_buf, new_index_buf, loc_buf],
