...
# dictionary that maps alu opcodes to their corresponding value
ALU = {"MIN":VTA_ALU_OPCODE_MIN, "MAX":VTA_ALU_OPCODE_MAX, "ADD":VTA_ALU_OPCODE_ADD, "SHR":VTA_ALU_OPCODE_SHR, "EMPTY":None}
	...
	def __generate_insn(self):
		...
		for i in range(self.ins_size):
			...
			if action == "1DLOAD":
				...
				self.insn_buf[index] = get_alu_insn(ALU.get(item), vec_len, use_imm, imm_val, self.uop_compression, 
				 0, 0, 0, 1)
				cmp_ind += 1
