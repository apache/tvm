...
)
{
  ...
  val idConst = 0
  val userConst = userCode
  val burstConst = 1
  val lockConst = 0
  val cacheConst = cacheCode
  val protConst = protCode
  val qosConst = 0
  val regionConst = 0
}
...
// IntelAXILiteClient and IntelAXIMaster bundles are needed
// for wrapper purposes, because the package RTL tool in Intel Quartus
// only allows certain name formats

class IntelAXILiteClient(params: AXIParams) extends AXIBase(params) {
  val awvalid = Input(Bool())
  val awready = Output(Bool())
  val awaddr = Input(UInt(params.addrBits.W))
...
class IntelAXIMaster(params: AXIParams) extends AXIBase(params) {
  val awvalid = Output(Bool())
  val awready = Input(Bool())
  val awaddr = Output(UInt(params.addrBits.W))
  val awid = Output(UInt(params.idBits.W))
