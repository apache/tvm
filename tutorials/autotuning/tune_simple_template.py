...
The whole workflow is illustrated by a matrix mutiplication example.
"""
...
# as the parameterization of our exiting schedule code.
#
...
#
# If you want the schedule code to be portable acorss a wider range of input shapes
# and target hardware, it is better to define a set of candidate values and 
# pick the best one according to the measurment results on target hardware.
#
...
#    we obtain it in a differet way. With this argument, this function is no longer
#    a deterministic schedule code. Instead, we can pass different configurations to
#    this function and get differnet schedules, so this function is a "template".
#
...
#    To archieve this, we make :code:`cfg` be either
...
#    list of possbile values. These two konbs are independent, so they
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# In the previous template, we manually list all possbile values for a knob.
# This is the lowest level API to define the space.
# However, we also provide another set of API to make the space definition
# easier and smarter. It is recommonded to use this set of high level API.
# 
...
# :any:`ConfigSpace.define_annotate` for annotation like unroll, vectrization,
# thread binding.
...
#  using factors of 32, then there are 6 possbile values for 
...
# Begin tuning
# ^^^^^^^^^^^^
# Here we continue our matrix mutplication example.
