...
===========================
**Author**: `Steven S. Lyubomirsky <https://homes.cs.washington.edu/~sslyu/>`_
...
- Using the :code:`RELAY_REGISTER_OP` macro in C++ to
...
The file :code:`src/relay/op/tensor/elemwise.cc` provides
examples of the first two steps, while
:code:`python/tvm/relay/op/tensor.py` gives examples of the
last.
...
For example, see :code:`src/relay/op/type_relations.h` and their
implementations. E.g., :code:`BroadcastRel` takes two input types and an
...
It may be necessary to add another type relation to :code:`type_relations.h`
if the existing ones do not capture the behavior of the desired operator.

The :code:`RELAY_REGISTER_OP` macro in C++ allows a developer
...
The below example is from :code:`elemwise.cc` and uses a broadcasting
add for tensors:
"""
...
# are not supported, so it suffices to use :code:`Op::Get` to fetch
...
# through :code:`TVM_REGISTER_API` should be wrapped in a separate
...
# to bundle them, as in :code:`python/tvm/relay/op/tensor.py`, where
...
# :code:`concat` operator is registered as taking only one operator,
