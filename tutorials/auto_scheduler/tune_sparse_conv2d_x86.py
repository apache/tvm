...
def sparse_conv2d_bsr_compute(data, weight_data, weight_indices, weight_indptr):
    '''
        Y       = X       * W^T
        Y[m,  h,  w,  n] = X[m,  h,  w,  k] * W[1, 1, k, n]
        Y[n, oh, ow, oc] = X[n, ih, iw, ic] * W[1, 1, ic, oc]
        NHWC
        HWIO
    '''
    (m, h, w, k) = get_const_tuple(data.shape)
    ...
    def _compute_block(i, h, w, nb_j, j):
    ...
    bsrmm_block = te.compute(
        (m, h, w, num_blocks, bs_r), _compute_block,
...
def sparse_conv(N, OH, OW, OC, IH, IW, IC, w_data_shape, w_indices_shape, w_indptr_shape, dtype):
    X = te.placeholder(shape=(N, IH, IW, IC), dtype=dtype)
    ...
    B = te.placeholder(shape=(N, OH, OW, OC), dtype=dtype)

    out = sparse_conv2d_bsr_compute(
        topi.nn.relu(X), W_data, W_indices, W_indptr
    )
    out = te.compute((N, OH, OW, OC), lambda i, j, k, l: out[i, j, k, l] + B[i, j, k, l], name="BiasAdd")
...
X_np = np.random.randn(N, IH, IW, IC).astype("float32")
X_np = np.maximum(np.zeros((N, IH, IW, IC), dtype="float32"), X_np)  # Relu
W_sp_np = random_bsr_matrix(IC, OC, BS_R, BS_C, density=density, dtype="float32")
W_np = W_sp_np.todense()


# Register the sparse data to special buffer
prefix = "sparse_dense_bsr_%d_%d_%d_%d_%d_%.2f_" % (N, OH, OW, BS_R, BS_C, density)
...
def meet_condition_func(search_policy, state, stage_id):
    ...
    if state.stages[stage_id].op.tag in [
        "sparse_conv2d_sp_bsrmm", "sparse_conv2d_sp_bsrmm_block"
    ]:
...
def apply_func(search_policy, state, stage_id):
    ...
    if s0.stages[stage_id].op.tag == "sparse_conv2d_sp_bsrmm_block":
        return [s0.state_object, stage_id - 1]

    sparse_dense = s0.stages[stage_id].op
    sparse_dense_block = s0.stages[stage_id - 1].op
    assert sparse_dense.tag == "sparse_conv2d_sp_bsrmm"
    assert sparse_dense_block.tag == "sparse_conv2d_sp_bsrmm_block"
    ...
    i, h, w, nb_j, j, row_offset, c = s0[sparse_dense_block].iters
    m, x, y, n = s0[consumer].iters

    i0, i1, i2 = s0.split(sparse_dense_block, i, [None, None])
    m0, m1 = s0.follow_split(consumer, m, len(s0.transform_steps) - 1, 1)
    h0, h1, h2 = s0.split(sparse_dense_block, h, [None, None])
    x0, x1 = s0.follow_split(consumer, x, len(s0.transform_steps) - 1, 1)
    w0, w1, w2 = s0.split(sparse_dense_block, w, [None, None])
    y0, y1 = s0.follow_split(consumer, y, len(s0.transform_steps) - 1, 1)
    j0, j1 = s0.split(sparse_dense_block, nb_j, [None])
    n0, n1 = s0.follow_split(consumer, n, len(s0.transform_steps) - 1, 1)
    s0.reorder(sparse_dense_block, [i0, h0, w0, j0, i1, h1, w1, j1, row_offset, i2, h2, w2, j, c])
    s0.reorder(consumer, [m0, x0, y0, n0, m1, x1, y1, n1])
