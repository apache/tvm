...
timing_number = 10
timing_repeat = 10
unoptimized = np.array(
        timeit.Timer(
            lambda: module.run()).repeat(repeat=timing_repeat, number=timing_number)
        )*1000/timing_number
unoptimized = { "mean": np.mean(unoptimized),
...
optimized = { "mean": np.mean(optimized),
