...
# this function will pint the schedule in C like format
# let's skip the detail for now
def print_schedule(s, ops):
    s = s.normalize()
    bounds = tvm.schedule.InferBound(s)
    stmt = tvm.schedule.ScheduleOps(s, bounds)
    stmt = tvm.lower(s, ops, with_api_wrapper=False, max_auto_unroll_step=0)
    print(stmt)
