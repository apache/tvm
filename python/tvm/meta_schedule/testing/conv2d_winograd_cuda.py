...
# pylint: disable=invalid-name,no-member,line-too-long,too-many-nested-blocks,no-self-argument,no-self-use,unused-argument


@T.prim_func
def conv2d_winograd_cuda(
    placeholder: T.Buffer[(1, 14, 14, 128), "float32"],
    placeholder_1: T.Buffer[(6, 6, 128, 128), "float32"],
    conv2d_winograd: T.Buffer[(1, 12, 12, 128), "float32"],
) -> None:
    ...
    for i0, i1, i2, i3 in T.grid(1, 16, 16, 128):
        with T.block("data_pad"):
            ...
            data_pad[i0_1, i1_1, i2_1, i3_1] = T.if_then_else(
                0 <= i1_1 and i1_1 < 14 and 0 <= i2_1 and i2_1 < 14,
        ...
        with T.block("input_tile"):
            ...
            T.reads(
                [
                    data_pad[
                        T.floordiv(p, 9),
                        ((T.floordiv(T.floormod(p, 9), 3) * 4) + eps),
                        ((T.floormod(p, 3) * 4) + nu),
                        ci,
                    ]
                ]
        ...
        with T.block("data_pack"):
            ...
            T.reads(
                [
                    ...
                    B[
                        T.min(r_a, r_b) : (
                            T.min(r_a, r_b) + ((T.max(r_a, r_b) + 1) - T.min(r_a, r_b))
                        ),
                        T.min(eps_1, nu_1) : (
                            T.min(eps_1, nu_1) + ((T.max(eps_1, nu_1) + 1) - T.min(eps_1, nu_1))
                        ),
                    ],
                ]
            )
        ...
        with T.block("inverse"):
            ...
            T.reads(
                [
                    ...
                    A[
                        T.min(r_a_1, r_b_1) : (
                            T.min(r_a_1, r_b_1) + ((T.max(r_a_1, r_b_1) + 1) - T.min(r_a_1, r_b_1))
                        ),
                        T.min(vh, vw) : (T.min(vh, vw) + ((T.max(vh, vw) + 1) - T.min(vh, vw))),
                    ],
                ]
            )
            ...
            T.reads(
                [
                    inverse[
                        T.floormod(h, 4),
                        T.floormod(w, 4),
                        (((n * 9) + (T.floordiv(h, 4) * 3)) + T.floordiv(w, 4)),
                        co_2,
                    ]
                ]
            )
