...
class Droplet:
    ...
    def __init__(
        self, json_file, workload_file, target, log, pvalue=0.05
    ) -> None:
        ...
        self.visited, self.batch = set([0]), max(
            os.cpu_count(), len(self.dims)
        )
    ...
    def num_to_bin(self, value, factor=1):
        bin_format = (
            str(0) * (len(self.dims) - len(bin(value)[2:])) + bin(value)[2:]
        )
    ...
    def speculation(self):
        ...
        while (
            len(self.next) < self.batch
            and self.execution < self.total_execution
        ):
