...
class Space:
    ...
    def add_space(
        self, space_list: list, element_list: list, limit=10000
    ) -> List[int]:
            ...
            if opt == "Annotate":
                ...
                for i in range(len(tile_val)):
                    ...
                    if create:
                        ...
                        config[0][1][tile_idx][1][i] = self.get_value(
                            key, values[idx]
                        )
    ...
    def run(
        ...
        for i, cfg in enumerate(json_file_list):
            try:
                ...
                record.trace.apply_to_schedule(
                    sch, remove_postproc=remove_postproc
                )
                mods.append(sch.mod)
        ...
        for i, record in enumerate(records):
            try:
                inp = ms.runner.RunnerInput(
                    ...
                    args_info=ms.arg_info.TensorInfo.from_prim_func(
                        mods[i]["main"]
                    ),
                )
                runner_res = runner.run([inp])[0].result()
                results[i] = [
                    v.value for v in runner_res.run_secs]  # type: ignore
            except Exception:  # pylint: disable=broad-except, invalid-name
                results[i] = [1e10]
                continue
