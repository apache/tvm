...
from .ai_runner import HwIOError, InvalidMsgError, NotInitializedMsgError, AiRunnerError, InvalidParamError
...
class AiPbMsg(AiRunnerDriver):
    ...
    def _waiting_answer(self, timeout=10000, msg_type=None, state=None):
        ...
        while cont:  # to manage the "log" msg
            ...
            if resp.reqid != self._req_id:
                raise InvalidMsgError('SeqID is not valid - {} instead {}'
                                          .format(resp.reqid, self._req_id))
    ...
    def _to_runtime(self, model_info):
        ...
        name_rt = 'X-CUBE-AI' if model_info.tool_api_version != model_info.api_version else\
             model_info.tool_revision
        return {
            'name': name_rt,
    ...
    def invoke_sample(self, s_inputs, **kwargs):
        ...
        for idx, buffer_desc in enumerate(model.inputs):
            ...
            self._send_buffer(input, buffer_desc, is_last=is_last)
