...
def generate_registry_args(parser, registry, default=None):
    ...
    parser.add_argument(
        ...
        help=f"{registry.registry_name.title()} to compile the model with",
        required=False,
        default=default,
    )
...
def _reconstruct_registry_options(args, registry, name):
    ...
    for option, option_type in registry.list_registered_options(name).items():
        if option_type in INTERNAL_TO_NATIVE_TYPE:
            var_name = f"{registry.registry_name}_{name}_{option.replace('-', '_')}"
...
def reconstruct_registry_entity(args, registry):
    ...
    name = getattr(args, registry.registry_name)
    if name is None:
        return None

    if name not in possible_names:
        raise TVMCException(f'{registry.registry_name.title()} "{name}" is not defined')
