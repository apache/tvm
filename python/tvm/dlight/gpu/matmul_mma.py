...
class MatmulTensorizationMMA(GPUScheduleRule):
    ...
    def apply_config(  # pylint: disable=too-many-locals,missing-docstring
        ...
        main_block = reduction_blocks[0]
        cache_write_required = True # main_block not in output_blocks, it should skip the analysis of reindex
        ...
        if (
            ...
            # factor = 4  # should be divisible by block_idy
        ...
        def fetch_to_shared(block, idx, vec_len):
        ...
        sch.annotate(b_g2s, ann_key="permuted_layout", ann_val=(not intrin_info.smooth_b))
        ...
        def get_index_map_3d(index_map):
        ...
        def get_index_map(index_map, is_smooth=False):
            if is_smooth:
                return get_index_map_5d(index_map)
            return get_index_map_3d(index_map)

        sch.transform_layout(A_mat, ("write", 0), get_index_map(index_map_a))
        sch.transform_layout(B_mat, ("write", 0), get_index_map(index_map_b, intrin_info.smooth_b))
        sch.transform_layout(store, ("read", 0), get_index_map(index_map_c))
        ...
        try:
            ...
            sch.reorder(i0, j0, i1, j1)
            bb = sch.blockize(i1)
            sch.annotate(bb, ann_key="permuted_layout", ann_val=(not intrin_info.smooth_b))
...
class MatmulTensorizationMMAWithDequantizeInfo(GPUScheduleRule):
    ...
    def sch_shared_memory_prefetch_with_config(
        ...
        def decode_fetch_to_shared(
            block, idx, vec_len
        ):
            # step1. create memory hierarchy
            # global -> local -> shared
            block_shared = sch.cache_read(block, idx, shared_scope)
            sch.compute_at(block_shared, k0)
            # TODO(lei): the factor shoule be analyzed more deeper.
            B_shared_jj, B_shared_vi, B_shared_vj = sch.split(
                sch.get_loops(block_shared)[-1], factors=[None, 1, 8])
            ...
            block_shared_local_local = sch.cache_read(block_shared_local, 0, "local")
            # global -> prefetch_shared -> vector load -> dequantzed_local -> shared
            block_shared_local_local_shared = sch.cache_read(
                block_shared_local_local,
                0, 
                shared_scope
            )
            sch.compute_at(block_shared_local, B_shared_vi)
            sch.compute_at(block_shared_local_local, B_shared_vi)

            sch.annotate(block_shared, ann_key="permuted_layout", ann_val=(not intrin_info.smooth_b))
            union_len = (2 + 2) # if smooth_b, the value should be -(2 + 4)
        ...
        if stage > 1:
            sch.annotate(k0, ann_key="software_pipeline_stage", ann_val=[0, 0, stage - 1, stage - 1])
    ...
    def apply_config(  # pylint: disable=too-many-locals,missing-docstring
        ...
        if config.arch.target.kind.name == "cuda" and check_sm_version(config.arch.target.arch) == 80:
