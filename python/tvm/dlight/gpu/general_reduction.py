...
class GeneralReduction(GPUScheduleRule):
    ...
    def apply(  # pylint: disable=too-many-locals
        ...
        # tile/parallelise, and the code below would crash because `iters` is 
        # an empty tuple.
        # Special case: the whole reduction collapsed to a *scalar*.
        # Add a unit thread loop so the final write happens inside a valid
        # GPU thread environment and passes `VerifyMemory`.
        # ------------------------------------------------------------------
        if num_last_block_iter == 0:
            # Put every block (both the running reductions and the final
            # scalar write) inside a trivial GPU thread to satisfy
            # VerifyMemory.  The very first block gets a `blockIdx.x`
            # wrapper so that kernels still have a unique block scope.
            for i, info in enumerate(block_infos):
                loop_rv = sch.add_unit_loop(info.block_rv)   # extent = 1
                if i == 0:
