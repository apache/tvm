    ...
    def make_reverse(self, input_name: str, input_iter: List[tir.PrimExpr]):
        ...
        for ax in self.op.axis:
            if ax.var in results:
                ...
                output_indices.append(tir.Var("undefined") % ax.dom.extent)
...
class DependencyAnalysis(object):
    def __init__(self, deps):
        self.deps = deps
        self.name2dep = {dep.block_analyzer.get_output_buffers(dep.block)[0].name: dep for dep in self.deps}
    ...
    def construct_dependency_target(self, targets: Tuple[str]):
        ...
        for dep in self.deps:
            for name in dep.dependent_region:
                ...
                if dep.block_analyzer.get_block_name(dep.block) in targets: continue
                if dep.block_analyzer.get_block_name(dep.block) not in mapping:
                    mapping[dep.block_analyzer.get_block_name(dep.block)] = [output_indices]
                elif not region_exist_in_list(output_indices, mapping[dep.block_analyzer.get_block_name(dep.block)]):
                    mapping[dep.block_analyzer.get_block_name(dep.block)].append(output_indices)

        for dep in reversed(self.deps):
            indices_list = mapping[dep.block_analyzer.get_output_buffers(dep.block)[0].name]
    ...
    def infer(self, shape: Dict[str, List[arith.ConstIntBound]], rstep: Dict[str, int]={}, targets=None):
        ...
        for ax in self.reduce_axes:
            ...
            if ax.var.name in rstep:
                bound = arith.ConstIntBound(int(_dom_min), int(_dom_min + min(ax.dom, rstep[ax.var.name]) - 1))
            else:
                bound = arith.ConstIntBound(int(_dom_min), int(_dom_min+ ax.dom - 1))
...
def _extract_dependent_region(block_analyzer, block: BlockRV) -> Dict[str, List[tir.PrimExpr]]:
    ...
    def fvisit(x):
        if not isinstance(x, tir.BufferLoad): return
        if x.buffer.name not in dependent_region: return
        index = []
