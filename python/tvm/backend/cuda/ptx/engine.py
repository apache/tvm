...
def _emit(entry, filled, operands, pred=None, preserve_dst=False, undefined_dst=False):
    ...
    if preserve_dst and undefined_dst:
        raise ValueError(
            f"{entry.name}: preserve_dst=True and undefined_dst=True are mutually exclusive"
        )
    if pred is not None:
        if entry.has_dst and not (preserve_dst or undefined_dst):
            raise ValueError(
                ...
                f'"=" output constraint would discard its prior value); pass '
                f"preserve_dst=True to retain the old value or undefined_dst=True when the "
                f"inactive value is intentionally undefined"
            )
        if undefined_dst and not entry.has_dst:
            raise ValueError(f"{entry.name}: undefined_dst=True requires a written destination")
        pred = _coerce_pred(entry, pred)
    elif preserve_dst or undefined_dst:
        policy = "preserve_dst" if preserve_dst else "undefined_dst"
        raise ValueError(f"{entry.name}: {policy}=True requires pred=...")
...
class _InstrChain:
    ...
    def __call__(self, *args, pred=None, preserve_dst=False, undefined_dst=False):
