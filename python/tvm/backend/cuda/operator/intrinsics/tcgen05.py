...
def _mma_block_scaled_parts(*args):
    """Args layout: (d_tmem_addr, a_operand, b_desc[, sp_tmem_addr], i_desc,
    enable_input_d, sfa_tmem_addr, sfb_tmem_addr[, pred],
    kind, scale_vec_size, sparse, use_a_tmem, cta_group, has_pred)."""
    attrs = args[-6:]
    kind = parse_str(attrs[0])
...
def _dispatch_tcgen05_mma_block_scaled(
    ...
    attr_args = [kind, scale_vec_size, sparse, use_a_tmem_b, cta_group_i, int(pred is not None)]
