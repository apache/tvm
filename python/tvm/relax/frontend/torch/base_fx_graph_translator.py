...
class BaseFXGraphImporter(metaclass=abc.ABCMeta):
    ...
    def _scaled_dot_product_attention(self, node: fx.Node) -> relax.Var:
        ...
        # TVM's nn.attention requires 4D inputs (batch, seq_len, num_heads, head_dim)
        # For 2D inputs (seq_len, head_dim), we need to reshape to 4D first
        if query_ndim == 2:
            # 2D input: (seq_len, head_dim) -> expand to (1, seq_len, 1, head_dim)
            ...
            query = self.block_builder.emit(relax.op.expand_dims(query_3d, axis=2))
            key = self.block_builder.emit(relax.op.expand_dims(key_3d, axis=2))
            value = self.block_builder.emit(relax.op.expand_dims(value_3d, axis=2))
            
            # No permutation needed for 2D inputs after expanding to 4D
            # After attention, squeeze back to 2D: (1, seq_len, 1, head_dim) -> (seq_len, head_dim)
            def transpose_and_reshape_back(tensor):
                # Squeeze num_heads dimension (axis 2)
                tensor_3d = self.block_builder.emit(relax.op.squeeze(tensor, axis=[2]))
                # Squeeze batch dimension (axis 0)
                return self.block_builder.emit(relax.op.squeeze(tensor_3d, axis=[0]))
        elif query_ndim == 4:
