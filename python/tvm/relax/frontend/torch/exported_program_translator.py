...
class TorchFXImporter:
    ...
    def _maximum(self, node: fx.node.Node) -> relax.Expr:
        lhs, rhs = self.retrieve_args(node)
        if isinstance(lhs, relax.Var) or isinstance(rhs, relax.Var):
    ...
    def _convolution(self, node: fx.node.Node) -> relax.Var:
        ...
        if isinstance(weight, relax.Expr):
        ...
        if groups > 1 and in_channels == 1:
            ...
            weight = relax.transform.reshape(weight, new_weight_shape)

        kernel_size = weight_shape[2:]
        use_bias = isinstance(bias, relax.Expr)
        ...
        if use_transpose:
            if len(kernel_size) == 3:
                conv_op = relax.op.nn.conv3d_transpose
            elif len(kernel_size) == 2:
                conv_op = relax.op.nn.conv2d_transpose
            else:
                conv_op = relax.op.nn.conv1d_transpose
            output_padding = tuple(inputs[7])
            additional_arguments["output_padding"] = output_padding

        else:
            if len(kernel_size) == 3:
                conv_op = relax.op.nn.conv3d
            elif len(kernel_size) == 2:
                conv_op = relax.op.nn.conv2d
            else:
                conv_op = relax.op.nn.conv1d
        ...
        if groups > 1 and len(kernel_size) == 1 and not use_transpose:
            ...
            padding = (0,) + padding
            dilation = (1,) + dilation
            data = relax.op.expand_dims(data, axis=2)
            weight = relax.op.expand_dims(weight, axis=2)
            data_layout = "NCHW"
            kernel_layout = "OIHW"
        ...
        if is_grouped_conv1d:
            ...
            res = relax.op.squeeze(res, axis=[2])
        return res

    def _linear(self, node: fx.node.Node) -> relax.Var:
        inputs = self.retrieve_args(node)
        bias = inputs[2]
        a_shape = self.infer_shape(inputs[0])
        b_shape = self.infer_shape(inputs[1])
        if len(a_shape) == 2 and len(b_shape) == 2:
            mm_out = relax.op.nn.dense(inputs[0], inputs[1])
        elif len(b_shape) == 1:
            mm_out = self.matmul([inputs[0], inputs[1]])
        else:
            mm_out = self.matmul(
                [inputs[0], relax.op.transpose(inputs[1], axes=(1, 0))]
            )
        if isinstance(bias, relax.Expr):
            bias_ndims = len(self.infer_shape_with_prelude(bias))
            if bias_ndims == 1:
                return relax.op.nn.bias_add(mm_out, bias, axis=-1)
            mm_dtype = self.infer_type_with_prelude(mm_out).dtype
            return self.block_builder.emit(relax.op.linear(x, weight, bias, mm_dtype))
        return mm_out
    ...
    def _layer_norm(self, node: fx.node.Node) -> relax.Var:
        ...
        if len(args) > 2:
            ...
            gamma = relax.const(torch.ones_like(normalized_shape), x.struct_info.dtype)

        if len(args) > 3:
            beta = args[3]
        else:
            beta = relax.const(torch.zeros_like(normalized_shape), x.struct_info.dtype)
        ...
        mean = relax.op.mean(x, axes)
        rstd = relax.op.rsqrt(relax.op.variance(x)+relax.const(eps))
        tuple_res = tvm.relax.Tuple((output, mean, rstd))
        return self.block_builder.emit(tuple_res)
    ...
    def infer_shape(self, expr: relax.Expr):
        return [int(val) for val in expr.struct_info.shape.values]
    ...
    def create_convert_map(self):
        ...
        self.convert_map: Dict[Union[nn.Module, str], Callable[[fx.node.Node], relax.Var]] = {
            # core aten ops
            "aten::sin": lambda node: self.block_builder.emit(relax.op.sin(self.env[node.args[0]])),
            "aten::cos": lambda node: self.block_builder.emit(relax.op.cos(self.env[node.args[0]])),
            "aten::tan": lambda node: self.block_builder.emit(relax.op.tan(self.env[node.args[0]])),
            "aten::asin": lambda node: self.block_builder.emit(relax.op.asin(self.env[node.args[0]])),
            "aten::acos": lambda node: self.block_builder.emit(relax.op.acos(self.env[node.args[0]])),
            "aten::atan": lambda node: self.block_builder.emit(relax.op.atan(self.env[node.args[0]])),
            "aten::sinh": lambda node: self.block_builder.emit(relax.op.sinh(self.env[node.args[0]])),
            "aten::cosh": lambda node: self.block_builder.emit(relax.op.cosh(self.env[node.args[0]])),
            "aten::tanh": lambda node: self.block_builder.emit(relax.op.tanh(self.env[node.args[0]])),
            "aten::asinh": lambda node: self.block_builder.emit(relax.op.asinh(self.env[node.args[0]])),
            "aten::acosh": lambda node: self.block_builder.emit(relax.op.acosh(self.env[node.args[0]])),
            "aten::atanh": lambda node: self.block_builder.emit(relax.op.atanh(self.env[node.args[0]])),
            "aten::exp": self._exp,
            "aten::add.Tensor": self._add,
            "aten::mul.Scalar": self._mul,
            "aten::mul.Tensor": self._mul,
            "aten::sub.Tensor": self._sub,
            "aten::sigmoid": self._sigmoid,
            "aten::sqrt": self._sqrt,
            "aten::round": self._round,
            "aten::arange.start_step": self._arange,
            "aten::sum.dim_IntList": self._sum,
            "aten::convolution": self._convolution,
            "aten::addmm": self._addmm,
            "aten::mm": self._matmul,
            "aten::bmm": self._matmul,
            "aten::cat": self._cat,
            "aten::expand": self._expand,
            "aten::permute": self._permute,
            "aten::split_with_sizes": self._split,
            "aten::cumsum": self._cumsum,
            "aten::squeeze.dims": self._squeeze,
            "aten::unsqueeze": lambda node: self.block_builder.emit(
                relax.op.expand_dims(self.env[node.args[0]], node.args[1])
            ),
            "aten::view": self._reshape,
            "aten::argmax": self._argmax_argmin(relax.op.argmax),
            "aten::argmin": self._argmax_argmin(relax.op.argmin),
            "aten::_softmax": self._softmax,
            "aten::_log_softmax": self._log_softmax,
            "aten::native_dropout": lambda node: self.env[node.args[0]],
            "aten::clamp": self._clamp,
            "aten::relu": lambda node: self.block_builder.emit(relax.op.nn.relu(self.env[node.args[0]])),
            "aten::gelu": self._gelu,
            "aten::avg_pool2d": self._avg_pool2d,
            "aten::_adaptive_avg_pool2d": self._adaptive_avg_pool2d(is_module=False),
            "aten::native_layer_norm": self._layer_norm,
            "aten::full": self._full,
            "aten::mean.dim": self._mean,
            "aten::rsqrt": self._rsqrt,
            "aten::neg": self._neg,
            "aten::maximum": self._maximum,
            "aten::_to_copy": lambda x: x,
            "aten::hardtanh": lambda x: x,
            "aten::max_pool2d_with_indices": lambda x: x,
            "aten::embedding": lambda x: x,
            "aten::native_group_norm": lambda x: x,
            "aten::select.int": lambda x: x,
            "aten::slice.Tensor": lambda x: x,
            "aten::le.Scalar": lambda x: x,
            "aten::ge.Scalar": lambda x: x,
            "aten::scalar_tensor": lambda x: x,
            "aten::where.self": lambda x: x,
        }

    ### [QIT ExportedProgram]
    def get_parameter(self,
                      node: torch.fx.Node,
                      exported_program: torch.export.ExportedProgram) -> torch.Tensor:
        param = None
        if node.name in exported_program.graph_signature.inputs_to_parameters:
            param = exported_program.state_dict[
                exported_program.graph_signature.inputs_to_parameters[node.name]
            ].data
        if node.name in exported_program.graph_signature.inputs_to_buffers:
            param = exported_program.state_dict[
                exported_program.graph_signature.inputs_to_buffers[node.name]
            ]
    ...
    def from_exported_program(
        ...
        for node in graph.nodes:
            if node.op == "placeholder":
                ...
                dtype = str(node.meta['tensor_meta'].dtype).split('.')[-1]
                self.env[node] = relax.Var(node.name,
                                           relax.TensorStructInfo(shape, dtype))
                parameter = self.get_parameter(node, model)
                if parameter is not None:
                    input_params.append(self.env[node])
                else:
                    inputs.append(self.env[node])
        ...
        with self.block_builder.function(name=func_name, params=inputs.copy()):
            ...
            with self.block_builder.dataflow():
                ...
                for node in graph.nodes:
                    param = self.get_parameter(node, model)
                    if param is not None:
                        ...
                        if not keep_params_as_input:
                            ...
                            self.env[node] = self.params[param]
                        # else:
                ...
                for node in graph.nodes:
                    ...
                    #     self.env[node] = inputs.pop(0)
                        ...
                        self.env[node] = self.block_builder.emit(relax.TupleGetItem(self.env[node.args[0]], node.args[1]))
...
def from_exported_program(
    ...
    *,
    ...
    Examples
    --------
    Users can use the FX tracer or torch.export.export() to export
    a ExportedProgram from a PyTorch model. The following codes show
    ...
    .. code-block:: python
        ...
        import numpy as np
        import torch
        from tvm.relax.frontend.torch_fx import from_exported_program
        ...
        # Use the torch.export.export() to export the PyTorch model to ExportedProgram.
        try:
            graph_module = torch.export.export(torch_model, *input_tensors)
        except:
            raise RuntimeError("Failed to export the PyTorch model to ExportedProgram.")

        # Use the importer to import the PyTorch model to Relax.
        mod: tvm.IRModule = from_exported_program(graph_module, input_info)
    ...
    .. code-block:: python
        ...
        torch.export.export(model).graph.print_tabular()
    ...
    """
    return TorchFXImporter().from_exported_program(
