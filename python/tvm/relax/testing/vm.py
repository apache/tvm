...
@tvm.register_global_func("tvm.test.vm.move")
def move(src):
    return src


@tvm.register_global_func("tvm.test.vm.add")
def add(a, b):
    ret = a.numpy() + b.numpy()
    return tvm.runtime.tensor(ret)


@tvm.register_global_func("tvm.test.vm.mul")
def mul(a, b):
    ret = a.numpy() * b.numpy()
    return tvm.runtime.tensor(ret)


@tvm.register_global_func("tvm.test.vm.equal_zero")
def equal_zero(a):
    ret = np.all((a.numpy() == 0))
    return tvm.runtime.tensor(ret)


@tvm.register_global_func("tvm.test.vm.subtract_one")
def subtract_one(a):
    ret = np.subtract(a.numpy(), 1)
    return tvm.runtime.tensor(ret)


@tvm.register_global_func("tvm.test.vm.identity")
...
@tvm.register_global_func("tvm.test.vm.add_scalar")
def add_scalar(a, b):
    return a + b


@tvm.register_global_func("tvm.test.vm.get_device_id")
...
@tvm.register_global_func("tvm.test.vm.check_if_defined")
