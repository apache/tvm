...
"""Pattern table for rocBLAS backend"""
import operator
...
def _is_supported_dtype(lhs_dtype, rhs_dtype):
    """Check if dtypes in the given workload are supported by rocBLAS BYOC."""
...
def _check_matmul(context: PatternCheckContext) -> bool:
    ...
    if "bias" in context.annotated_expr:
        ...
        if not isinstance(bias_batches, (tvm.tir.expr.IntImm, int)) or int(bias_batches) > 1:
            # rocBLAS only supports bias vector
            return False

    # rocBLASLt does not seem to support batched GEMM with one of matrices having
...
register_patterns(
    [
        ...
        # (
        #     "rocblas.matmul_transposed_bias",
        #     *make_matmul_pattern(
        #         with_bias=True,
        #         transposed_rhs=True,
        #     ),
        ...
        # ),
        # (
        #     "rocblas.matmul_transposed_bias_relu",
        #     *make_matmul_pattern(
        #         with_bias=True,
        #         activation="relax.nn.relu",
        #         transposed_rhs=True,
        ...
        # ),
        # (
        #     "rocblas.matmul_transposed_bias_gelu",
        #     *make_matmul_pattern(
        #         with_bias=True,
        #         activation="relax.nn.gelu",
        #         transposed_rhs=True,
...
def partition_for_rocblas(mod):
    """
    Partition the input module into rocBLAS-supported subgraphs.
    ...
    mod: tvm.IRModule
        ...
        offloaded to the rocBLAS backend.
    """

    patterns = get_patterns_with_prefix("rocblas")
