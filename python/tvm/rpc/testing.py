...
@tvm.register_global_func("tvm.rpc.test.addone")
def _addone(x):
    return x + 1


@tvm.register_global_func("tvm.rpc.test.strcat")
def _strcat(name, x):
    return f"{name}:{x}"


@tvm.register_global_func("tvm.rpc.test.except")
def _remotethrow(name):
    raise ValueError(f"{name}")


@tvm.register_global_func("tvm.rpc.test.runtime_str_concat")
def _strcat(x, y):
    return x + y


@tvm.register_global_func("tvm.rpc.test.remote_tensor_func")
def _remote_tensor_func(y):
    x = np.ones((3, 4))
    np.testing.assert_equal(y.numpy(), x)


@tvm.register_global_func("tvm.rpc.test.add_to_lhs")
def _add_to_lhs(x):
    return lambda y: x + y


@tvm.register_global_func("tvm.rpc.test.remote_return_nd")
def _my_module(name):
