...
class CommandLineExit(Exception):
    """Command line exit exception.

    Attributes:
      exit_token: An exit token object, object to cause the UI loop exit.
    """
    ...
    @property
    def exit_token(self):
        """Get the exit token object cause the UI loop exit.

        Returns:
          ('object') An exit token object, object to cause the UI loop exit.
        """
...
def rich_text_lines_frm_line_list(rich_text_list, annotations=None, additional_attr=None):
    ...
    lines = []
    font_attr_segs = {}
    for i, richline in enumerate(rich_text_list):
        if isinstance(richline, RichLine):
            lines.append(richline.text)
            if richline.font_attr_segs:
...
class RichTextLines(object):
    ...
    def font_attr_segs(self):
        """Get the rich text line font attributes segments.
    ...
    def annotations(self):
        """Get the rich text line annotations.
    ...
    def num_lines(self):
        """Get the size of the rich text line list.

        Returns:
          ('int')Size of rich text line list.
        """
...
class CommandHandlerRegistry(object):
    ...
    def get_help(self, cmd_prefix=None):
        ...
        if not cmd_prefix:
            ...
            for sort_cmd_prefix in sorted_prefixes:
                lines = self._get_help_for_command_prefix(sort_cmd_prefix)
                lines.append("")
                lines.append("")
...
class TabCompletionRegistry(object):
    ...
    def get_completions(self, context_word, prefix):
        ...
        return comp_items, _common_prefix(comp_items)
...
class CommandHistory(object):
    ...
    _HISTORY_FILE_NAME = ".tvmdbg_history"
...
class MenuItem(object):
    ...
    @property
    def caption(self):
        """Get a caption of MenuItem.

        Returns:
          ('string') Caption of the menu item.
        """
    ...
    @property
    def content(self):
        """Get a content of MenuItem.

        Returns:
          ('string') Content of the menu item. For a menu item that triggers
            a command, for example, content is the command string.
        """
    ...
    def is_enabled(self):
        ...
        Returns:
          ('bool') The menu item enable status.
        """
...
class Menu(object):
    ...
    def num_items(self):
        """Get a size of of MenuItems under Menu.

        Returns:
          ('int') The size of MenuItem under Menu.
        """
        return len(self._items)

    def captions(self):
        """Get a captions of MenuItems under current Menu.

        Returns:
          ('list') List of menu item captions.
        """
