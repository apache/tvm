...
def register_ndarray(cls, fcreate):
    """Register a subclass of TVM NDArray.
    ...
    ----------
    cls : class
        The class object to be registered as subclass.
    ...
    Note
    ----
    The registered class is requires one property: _tvm_handle and
    a class attribute _array_type_index.

    - ```_tvm_handle``` returns integer represents the address of the handle.
    - ```_array_type_index``` gives integer represents type code of the subclass,
      which should exactly match the type trait defined in C++ backend.
