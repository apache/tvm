...
def compare_numpy_tvm(inputs, output, target, dev, compute, schedule):
    ...
    te_out = tvm.nd.array(np.zeros(output.shape).astype(output.dtype), device=dev)
    with tvm.target.Target(target):
        out = compute(*te_inputs)
        s = schedule([out])
        func = tvm.build(s, te_inputs + [out])
        arys = [tvm.nd.array(x, device=dev) for x in inputs]
        func(*(arys + [te_out]))
