...
def deformableV2_conv2d_nchw_python(
    ...
    mask_np : numpy.ndarray
        4-D with shape [batch, deforambale_groups * filter_height * filter_width * 2,
                        out_height, out_width]
    ...
    print("this is input shape:", a_np.shape)
    ...
    for n, h, w in itertools.product(range(batch), range(out_height), range(out_width)):
        ...
        mask = mask_np[n, :, h, w].reshape(deformable_groups, kernel_h, kernel_w)
        ...
        for c, kh, kw in itertools.product(range(in_channel), range(kernel_h), range(kernel_w)):
            dg = c // ic_per_dgroup
            index_h = index_h_base + offset[dg, ..., 0] # y dir
            index_w = index_w_base + offset[dg, ..., 1] # x dir

            y, x = index_h[kh, kw], index_w[kh, kw]
            if y < 0 or y >= in_height or x < 0 or x >= in_width:
                continue
            a_deform[n, c, h, w, kh, kw] = _bilinear(n, c, y, x) * mask[dg, kh, kw]
...
def deformableV2_conv2d_nhwc_python(
    a_np, offset_np, mask_np, w_np, stride, padding, dilation, deformable_groups, groups
):
    ...
    b_np = deformableV2_conv2d_nchw_python(
        a_np, offset_np, mask_np, w_np, stride, padding, dilation, deformable_groups, groups
    )
