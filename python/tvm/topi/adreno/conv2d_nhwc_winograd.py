...
def schedule_conv2d_nhwc_winograd(cfg, outs):
    return schedule_conv2d_nhwc_winograd_impl(cfg, outs, tag="cast_from_acc16")

@autotvm.register_topi_schedule("conv2d_nhwc_winograd_acc32.image2d")
def schedule_conv2d_nhwc_winograd_acc32(cfg, outs):
    return schedule_conv2d_nhwc_winograd_impl(cfg, outs, tag="cast_from_acc32")

@autotvm.register_topi_compute("conv2d_nhwc_winograd_without_weight_transform.image2d")
def conv2d_nhwc_winograd_without_weight_transform(cfg, data, kernel, strides, padding, dilation, out_dtype):
    args={"shared" : False, "accumulator" : "float16"}
    return conv2d_nhwc_winograd_comp(
...
def conv2d_nhwc_winograd_without_weight_transform_acc32(cfg, data, kernel, strides, padding, dilation, out_dtype):
    args={"shared" : False, "accumulator" : "float32"}
    return conv2d_nhwc_winograd_comp(
        cfg, data, kernel, strides, padding, dilation, out_dtype, args=args, pre_computed=True
    )
...
def schedule_conv2d_nhwc_winograd_without_weight_transform(cfg, outs):
    return schedule_conv2d_nhwc_winograd_impl(cfg, outs, tag="cast_from_acc16", pre_computed=True)
...
def schedule_conv2d_nhwc_winograd_without_weight_transform_acc32(cfg, outs):
    return schedule_conv2d_nhwc_winograd_impl(cfg, outs, tag="cast_from_acc32", pre_computed=True)

def schedule_conv2d_nhwc_winograd_impl(cfg, outs, tag, pre_computed=False):
...
def conv2d_nhwc_winograd_comp(cfg, data, kernel, strides, padding, dilation, out_dtype, args, pre_computed):
    ...
    if len(data.shape) == 4:
        ...
        if autotvm.GLOBAL_SCOPE.in_tuning == True:
            ...
            convert_from4d = True
            data = pack_input(data, "NHWC", N, in_channel_chunks, in_channel_block, in_channel_tail, H, W)
            if not pre_computed:  # kernel tensor is raw tensor, do strict check
    ...
    input_tile = te.compute(
        (alpha, alpha, CI, P, CB),
        lambda eps, nu, c, p, cb: data_pad[idxdiv(p, (nH * nW))][idxmod(idxdiv(p, nW), nH) * m + eps][idxmod(p, nW) * m + nu][c][cb],
        name="d",
    )
    ...
    if convert_from4d and autotvm.GLOBAL_SCOPE.in_tuning == False:
        output = te.compute(
            (N, H, W, out_channels),
            lambda n, h, w, c: inverse[c // CB][n * nH * nW + idxdiv(h, m) * nW + idxdiv(w, m)][idxmod(h, m)][idxmod(w, m)][c % CB].astype(out_dtype),
            name="output",
        ...
        output = te.compute(
            (N, H, W, CO, COB),
            lambda n, h, w, co, cob: inverse[co][n * nH * nW + idxdiv(h, m) * nW + idxdiv(w, m)][idxmod(h, m)][idxmod(w, m)][cob].astype(out_dtype),
            name="output",
            tag="cast_from_acc" + args["accumulator"][-2:],
        )
...
def schedule_conv2d_winograd(cfg, s, output, pre_computed):
    ...
    p, c, eps, nu, cb = s[data_pack].op.axis
    fused = s[data_pack].fuse(p, c, eps, nu)
