...
def _grid_sample_2d(
    ...
    def _bicubic_sample(n, c, h, w):
        A = -0.75  # 0.75 is used in pytorch, it's maybe different in other frameworks
    ...
    return te.compute(
        (batch, in_channel, out_height, out_width), interpolation, tag="grid_sample"
    )
...
def _grid_sample_3d(
    ...
    grid : tvm.Tensor
        5-D with shape [batch, 2, out_depth, out_height, out_width]

        or for pytorch
        5-D with shape [batch, out_depth, out_height, out_width, 2]

    method : str
        The interpolation method 'nearest', 'bilinear' are supported.
    ...
    def _get_pixel_value(n, c, d, h, w):
        return te.if_then_else(
            te.all(d >= 0, h >= 0, w >= 0, d < in_depth,
                   h < in_height, w < in_width),
            data[n, c, d, h, w],
            tir.const(0.0, dtype=data.dtype),
        )
...
def grid_sample(
    ...
    The shape of the output will be
        4D: (data.shape[0], data.shape[1], grid.shape[2], grid.shape[3]),
        5D: (data.shape[0], data.shape[1], grid.shape[2], grid.shape[3], grid.shape[4]),
    or for pytorch
        4D: (data.shape[0], data.shape[1], grid.shape[1], grid.shape[2]),
        5D: (data.shape[0], data.shape[1], grid.shape[1], grid.shape[2], grid.shape[3]).
    ...
    method : str
        ...
        4D:
            'nearest', 'bilinear', 'bicubic' are supported.
        5D:
            'nearest', 'bilinear' are supported.
