...
def _grid_sample_2d(
    ...
    Given :math:`data` an :math:`grid` assuming NCHW layout, then the output is computed by
    ...
    The out-boundary points will be padded with:
        zeros if padding_mode is `zeros`, or
...
def grid_sample(
    ...
    The shape of the output will be
        4D: (data.shape[0], data.shape[1], grid.shape[2], grid.shape[3]), or
    ...
    method : str
        ...
        4D: `nearest`, `bilinear`, `bicubic` are supported.
        5D: `nearest`, `bilinear`(trilinear) are supported.
