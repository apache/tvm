...
def max_pool2d_nchw(Input, Filter, stride, dilation, padding, pool_type, out_dtype=None):
    ...
    # maxpool stage
    kh = te.reduce_axis((0, filter_h), name="kh")
    kw = te.reduce_axis((0, filter_w), name="kw")
    ...
    Reg = te.compute(
        ...
        lambda b, c, h, w:
        te.max(
            (Input[b, c, h * stride_h + kh * dilation_h, w * stride_w + kw * dilation_w,].astype(out_dtype)
            ), axis=[kh, kw],
...
def avg_pool2d_nchw(Input, Filter, stride, dilation, padding, pool_type, out_dtype=None):
    ...
    Output = te.compute(
        (batch, out_channel, out_height, out_width),
        lambda b, c, h, w:
        (Reg[b, c, h, w].astype("int")>>(dilated_kernel_h*dilated_kernel_w)).astype(out_dtype)
