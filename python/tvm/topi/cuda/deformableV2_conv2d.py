...
@autotvm.register_topi_compute("deformableV2_conv2d_nchw.cuda")
def deformableV2_conv2d_nchw(
    cfg, data, offset, mask, kernel, strides, padding, dilation, deformable_groups, groups, out_dtype
):
    """Deformable Conv2d."""
    return nn.deformableV2_conv2d_nchw(
        data, offset, mask, kernel, strides, padding, dilation, deformable_groups, groups, out_dtype
    )


@autotvm.register_topi_schedule("deformableV2_conv2d_nchw.cuda")
def schedule_deformableV2_conv2d_nchw(cfg, outs):
    ...
    def _callback(op):
        if op.tag == "deformableV2_conv2d_nchw":
