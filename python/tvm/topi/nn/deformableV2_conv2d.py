...
def deformableV2_conv2d_nchw(
    data, offset, mask, kernel, strides, padding, dilation, deformable_groups, groups, out_dtype
):
    """DeformableV2 conv2D operator in NCHW layout.
    ...
    data_deform = te.compute(
        ...
        lambda n, c, kh, kw, y, x: _bilinear(
            ...
            + offset[
                ...
                c // ic_per_dgroup * (kernel_w * kernel_h) + (kh * kernel_w + kw), 
                y, 
                x,],
        tag="data_deform",
    )
    return te.compute(
        ...
        tag="deformableV2_conv2d_nchw",
    )


def deformableV2_conv2d_nhwc(
    data, offset, mask, kernel, strides, padding, dilation, deformable_groups, groups, out_dtype
):
