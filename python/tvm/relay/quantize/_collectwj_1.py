...
def collect_wj_1(mod_quantize, dataset=None):
    assert dataset
    ...
    QCheckpoint_1_dir = root_dir_name + "QSiSo"
    ...
    if not os.path.exists(QCheckpoint_1_dir):
        os.mkdir(QCheckpoint_1_dir)
    ...
    for i in range(len(dataset)):
        ...
        for j in range(num_check_point_outputs):
            ...
            np.save(QCheckpoint_1_dir + "/" + "CheckPoint_{}".format(i*(num_check_point_outputs) + j), check_point_tmp)
            check_point_max.append(np.max(check_point_tmp))
            check_point_min.append(np.min(check_point_tmp))
    check_point_maxmin.append(np.array(check_point_max).max())
    check_point_maxmin.append(np.array(check_point_min).min())

    print("check_point collect done")



    return check_point_maxmin
