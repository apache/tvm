...
AEEResult __QAIC_HEADER(launcher_rpc_load)(remote_handle64 handle, const char* module_path,
  ...
  if (TheModel) {
    ...
    LOG(ERROR) << "ERROR: " << __func__ << ": model already loaded, unload first";
...
AEEResult __QAIC_HEADER(launcher_rpc_set_input)(remote_handle64 handle, int input_idx,
  ...
  if (!TheModel) {
    // No model created.
    LOG(ERROR) << "ERROR: " << __func__ << ": no model created";
    return AEE_EBADSTATE;
  }
...
AEEResult __QAIC_HEADER(launcher_rpc_run)(remote_handle64 handle, uint64_t* pcycles,
  ...
  switch (res) {
    ...
    case QURT_HVX_RESERVE_NOT_SUCCESSFUL:
      LOG(ERROR) << "ERROR: error reserving HVX: " << res;
      return AEE_EFAILED;
    default:
      break;
  }
  ...
  if (lck != 0) {
    LOG(ERROR) << "ERROR: error locking HVX: " << lck;
    return AEE_EFAILED;
  }
  ...
  *usecs = us_end - us_begin;
  ...
  if (unl != 0) {
    LOG(ERROR) << "ERROR: error unlocking HVX: " << unl;
    return AEE_EFAILED;
  }
  // Release HVX.
  int rel = qurt_hvx_cancel_reserve();
  if (rel != 0) {
    LOG(ERROR) << "ERROR: error canceling HVX reservation: " << rel;
    return AEE_EFAILED;
  }
