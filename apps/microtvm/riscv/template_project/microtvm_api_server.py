...
class Handler(server.ProjectAPIHandler):
        ...
        return server.ServerInfo(
            ...
            project_options=[
                ...
                server.ProjectOption(
                    ...
                    type="str",
                    default="/opt/riscv",
                    help="Sets the value of toolchain path.",
                ),
                server.ProjectOption(
                    "target",
                    optional=["build", "open_transport"],
                ...
                server.ProjectOption(
                    ...
                    type="str",
                    default="rv64gc",
                    help="Sets the value of ISA.",
                ),
                server.ProjectOption(
                    "vlen",
                    optional=["open_transport"],
                    type="int",
                    help="Number of bits in each vector register if V-Extension is available.",
                ),
                server.ProjectOption(
                    "elen",
                    optional=["open_transport"],
                    type="int",
                    help="Size of largest element in bits if V-Extension is available.",
                ),
            ],
        )
    ...
    def build(self, options):
        ...
        mabi = options.get("mabi")
        cmake_args.append(f"-DTOOLCHAIN_PATH={toolchain_path}")
        cmake_args.append(f"-DTARGET={target}")
        cmake_args.append(f"-DMARCH={march}")
        cmake_args.append(f"-DMABI={mabi}")
    ...
    def open_transport(self, options):
        ...
        target = options.get("target")
        isa = options.get("isa")
        vlen = options.get("vlen")
        elen = options.get("elen")
        spike_extra = []
        if vlen and elen:
        ...
        spike_args = [
            ...
            f"--isa={isa}",
            *spike_extra,
            os.path.join(toolchain_path, target, "bin/pk"),
        ]
