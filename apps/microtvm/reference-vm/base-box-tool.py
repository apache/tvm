...
def attach_virtualbox(uuid, vid_hex=None, pid_hex=None, serial=None):
  ...
  for dev in usb_device:
    ...
    if (vid_hex == dev_vid_hex and pid_hex == dev_pid_hex and
        (serial is None or serial == dev["SerialNumber"])):
      rule_args = ["VBoxManage", "usbfilter", "add", "--target", uuid,
                   "--vendorid", vid_hex, "--productid", pid_hex]
  ...
  raise Exception(f"Device with vid={vid_hex}, pid={pid_hex}, serial={serial!r} not found:\n{usb_devices!r}")


def attach_parallels(uuid, vid_hex=None, pid_hex=None, serial=None):
  usb_devices = json.loads(subprocess.check_output(["prlsrvctl", "usb", "list", "-j"], encoding="utf-8"))
  for dev in usb_devices:
    _, dev_vid_hex, dev_pid_hex, _, _, dev_serial = dev["System name"].split("|")
    if (vid_hex == dev_vid_hex and pid_hex == dev_pid_hex and
        (serial is None or serial == dev_serial)):
...
def build_command(args):
  generate_packer_config(os.path.join(THIS_DIR, args.platform, "base-box", "packer.json"),
                         args.provider.split(",") or ALL_PROVIDERS)
  subprocess.check_call(["packer", "build", "packer.json"],
                        cwd=os.path.join(THIS_DIR, args.platform, "base-box"))
...
def test_command(args):
  ...
  with open(test_config_file) as f:
    ...
    for key, expected_type in REQUIRED_TEST_CONFIG_KEYS.items():
      assert key in test_config and isinstance(test_config[key], expected_type), (
  ...
  for provider_name in providers:
    ...
    for dirpath, _, filenames in os.walk(user_box_dir):
      ...
      if any(rel_path == scp or rel_path.startswith(f"{scp}{os.path.sep}")
    ...
    with open(release_test_vagrantfile, "w") as f:
      for line in lines:
        ...
        box_package = os.path.join(base_box_dir, f"output-packer-{provider_name}", "package.box")
        f.write(f"{m.group(1)} = \"{os.path.relpath(box_package, release_test_dir)}\"\n")
        found_box_line = True

    if not found_box_line:
      _LOG.error("testing provider %s: couldn't find config.box.vm = line in Vagrantfile; unable to test",
                 provider_name)
      continue

    subprocess.check_call(["vagrant", "up", f"--provider={provider_name}"],
                          cwd=release_test_dir)
    try:
      with open(os.path.join(release_test_dir, ".vagrant", "machines", "default", provider_name, "id")) as f:
        machine_uuid = f.read()
      ATTACH_USB_DEVICE[provider_name](
        machine_uuid, vid_hex=test_config["vid_hex"], pid_hex=test_config["pid_hex"], serial=args.test_device_serial)
    ...
    if not all(provider_passed[p] for p in provider_passed.keys()):
      sys.exit("some providers failed release test: " +
...
#  subprocess.check_call(["vagrant", "cloud", "version", "create", f"tlcpack/microtvm-{args.platform}", args.version])
  ...
  for provider_name in args.provider.split(","):
    subprocess.check_call(
      ["vagrant", "cloud", "publish", "-f", f"tlcpack/microtvm-{args.platform}",
       args.version, provider_name,
       os.path.join(THIS_DIR, args.platform, "base-box", f"output-packer-{provider_name}/package.box")])
...
def parse_args():
  parser = argparse.ArgumentParser(description="Automates building, testing, and releasing a base box")
  ...
  parser.add_argument("--test-device-serial",
                      help="If given, attach the test device with this USB serial number")
  parser.add_argument("--version",
                      help="Version to release. Must be specified with release.")
